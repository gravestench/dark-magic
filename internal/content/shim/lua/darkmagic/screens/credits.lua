...
local function pages(text, count)
    ...
    for line in (text .. "\n"):gmatch("(.-)\n") do
        ...
        if lines == count then result[#result + 1], page, lines = table.concat(page, "\n"), {}, 0 end
    end
    if #page > 0 then result[#result + 1] = table.concat(page, "\n") end
    return result
end
...
return {
    create = function(self)
        ...
        if not text then text = assert(locale.text("darkmagic.credits.unavailable")) end
        self.pages, self.page = pages(text, screen.lines_per_page), 1
        if render.assets_available() then self.text = render.create("hud", self.root); self:draw_page() end
        self.cursor = cursor.new(self.root, manifest.cursor, manifest.palettes)
    end,
    draw_page = function(self)
        self.text:set_text(font.table, font.sheet, manifest.palettes[font.palette], self.pages[self.page],
            {red=205, green=190, blue=155, max_width=700, align="center"})
        self.text:set_position(400, 300)
    end,
    update = function(self)
        self.cursor:update()
        local previous = self.page
        if input.pressed("down") or input.pressed("right") or input.pressed("confirm") then self.page = math.min(#self.pages, self.page + 1) end
        if input.pressed("up") or input.pressed("left") then self.page = math.max(1, self.page - 1) end
        if self.text and previous ~= self.page then self:draw_page() end
