...
return {
    create = function(self)
        ...
        local function add(id, activate)
            ...
            local control = {id=id, label=label_text,
                x=definition.x, y=definition.y, width=definition.width, height=definition.height,
                on_activate=activate}
            if render.assets_available() then
                local left, right, label = render.create("hud", self.root), render.create("hud", self.root), render.create("hud", self.root)
                local palette = manifest.palettes[definition.palette]
                local function draw_frames(frames) left:set_dc6(definition.sheet, palette, 0, frames[1]); right:set_dc6(definition.sheet, palette, 0, frames[2]) end
                ...
                control.on_state = function(_, state) if state == "focused" or state == "hover" then draw_frames(definition.down_frames) else draw_frames(definition.up_frames) end end
            end
            self.controls:add(control)
        end
        add("host", function() self.intent = { mode="host" } end)
        add("join", function()
            self.dialog = dialog.text_entry(self.root, screen.dialog, font,
                manifest.palettes[screen.dialog.palette], manifest.palettes[font.palette],
                assert(locale.text("darkmagic.tcpip.address")), "", function(address)
                    if address ~= "" then self.intent = { mode="join", address=address } end
                end)
        end)
