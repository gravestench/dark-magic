...
local function follow(entity)
    local relationship = ecs.get(entity, "d2.world.camera_follow")
    local target_position = ecs.get(relationship:get("target"), "d2.world.position")
    local camera_position = ecs.get(entity, "d2.world.position")
...
function M.register()
    ecs.system({
        id = "d2.world.camera_follow",
        phase = "presentation",
        query = { all = { "d2.world.position", "d2.world.camera_follow" } },
        read = { "d2.world.camera_follow" },
        write = { "d2.world.position" },
