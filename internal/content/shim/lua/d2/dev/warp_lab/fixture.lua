-- Construct and own Warp Lab's three authoritative entities.

local ecs = require("engine.ecs/v1")
local components = require("d2.dev.warp_lab.components")
local system = require("d2.dev.warp_lab.system")
local M = {}

function M.create(a,b,spawn)
    components.register()
    system.register()
    local fixture = {entities = {}}
    local function portal_position(point)
        local entity=ecs.create({
            ["d2.world.position"]={x=point.x,y=point.y},
        })
        fixture.entities[#fixture.entities + 1] = entity
        return entity
    end
    fixture.portal_a = portal_position(a)
    fixture.portal_b = portal_position(b)
    ecs.set(fixture.portal_a, "d2.lab.warp.portal", {
        id = "warp-lab:a", pair = fixture.portal_b,
        label = "WESTERN PORTAL", radius = 1.5,
    })
    ecs.set(fixture.portal_b, "d2.lab.warp.portal", {
        id = "warp-lab:b", pair = fixture.portal_a,
        label = "EASTERN PORTAL", radius = 1.5,
    })
    fixture.player = ecs.create({
        ["d2.world.position"] = {x = spawn.x, y = spawn.y},
        ["d2.lab.warp.actor"] = {speed = 12, direction = 0},
        ["d2.lab.warp.state"] = {event = "ready: click a portal", warp_count = 0},
    })
    fixture.entities[#fixture.entities + 1] = fixture.player
    return fixture
end

function M.intent(fixture,portal_id,route)
    local portal = portal_id == "warp-lab:a" and fixture.portal_a or fixture.portal_b
    ecs.remove(fixture.player, "d2.lab.warp.move_intent")
    ecs.set(fixture.player, "d2.lab.warp.intent", {portal = portal, route = route or "", waypoint = 1})
    ecs.get(fixture.player, "d2.lab.warp.state"):set("event", "interaction intent: " .. portal_id)
end

function M.move(fixture, x, y, route)
    ecs.remove(fixture.player, "d2.lab.warp.intent")
    ecs.set(fixture.player, "d2.lab.warp.move_intent", {
        x = x, y = y, route = route or "", waypoint = 1,
    })
    ecs.get(fixture.player, "d2.lab.warp.state"):set("event", "ground movement intent")
end

function M.destroy(fixture)
    if not fixture then return end
    for _, entity in ipairs(fixture.entities) do
        if entity:exists() then ecs.destroy(entity) end
    end
end

return M
