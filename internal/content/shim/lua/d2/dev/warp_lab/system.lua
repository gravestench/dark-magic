...
function M.register()
    ecs.system({
        id = "d2.lab.warp.resolve_move", phase = "movement",
        query = {all = {"d2.lab.warp.actor", "d2.lab.warp.move_intent", "d2.world.position"}},
        read = {"d2.lab.warp.actor", "d2.lab.warp.move_intent"},
        write = {"d2.world.position", "d2.lab.warp.actor", "d2.lab.warp.move_intent"},
        update = function(context, entities, commands)
            for _, entity in ipairs(entities) do
                local actor = ecs.get(entity, "d2.lab.warp.actor")
                local intent = ecs.get(entity, "d2.lab.warp.move_intent")
                ...
                if remaining <= 0.1 then
                    commands:remove(entity, "d2.lab.warp.move_intent")
                end
            end
        end,
    })
    ecs.system({
        id="d2.lab.warp.resolve_intent", phase="movement",
        query={all={"d2.lab.warp.actor","d2.lab.warp.intent","d2.lab.warp.state","d2.world.position"}},
        read={"d2.lab.warp.actor","d2.lab.warp.intent","d2.lab.warp.portal"},
        write={"d2.lab.warp.state","d2.world.position","d2.lab.warp.actor","d2.lab.warp.intent"},
        update=function(context,entities,commands)
            for _, entity in ipairs(entities) do
                local actor = ecs.get(entity, "d2.lab.warp.actor")
                local intent = ecs.get(entity, "d2.lab.warp.intent")
                local position = ecs.get(entity, "d2.world.position")
                local state = ecs.get(entity, "d2.lab.warp.state")
                local target_entity = intent:get("portal")
                local target_portal = ecs.get(target_entity, "d2.lab.warp.portal")
                local target_position = ecs.get(target_entity, "d2.world.position")
                if not target_portal or not target_position then
                    state:set("event", "intent rejected: unknown portal")
                    commands:remove(entity, "d2.lab.warp.intent")
                else
                    ...
                    if math.sqrt(dx * dx + dy * dy) <= target_portal:get("radius") then
                        ...
                        local destination_portal = ecs.get(destination_entity, "d2.lab.warp.portal")
                        local destination_position = ecs.get(destination_entity, "d2.world.position")
