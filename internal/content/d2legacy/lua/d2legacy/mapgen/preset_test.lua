...
return test.suite({
    ...
    tier = "fast",
    cases = {
        test.case("owns_preset_generation_policy", {
            test.run(function()
                local rows = {
                ...
                test.assert(first.checksum == again.checksum, [=[first.checksum == again.checksum]=])
                test.assert(
                    first.kind == "preset" and first.bounds.width == 40 and first.bounds.height == 30,
                    [=[first.kind == "preset" and first.bounds.width == 40 and first.bounds.height == 30]=]
                )
                test.assert(
                    first.stamps[1].preset_def == 7 and first.stamps[1].populate,
                    [=[first.stamps[1].preset_def == 7 and first.stamps[1].populate]=]
                )
                test.assert(
                ...
                test.assert(
                    first.stamps[1].role:match("^act1%-town:exit%-"),
                    [=[first.stamps[1].role:match("^act1%-town:exit%-")]=]
                )
            end),
        }),
