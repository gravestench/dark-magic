...
local skill_selector = require("d2legacy.ui.skill_selector")
...
function M.create(root, definition, palettes, commands)
    ...
    add_status_control(hud, "health", globes.health, assert(locale.text("d2legacy.hud.health")), hud.tips.health)
    add_status_control(hud, "mana", globes.mana, assert(locale.text("d2legacy.hud.mana")), hud.tips.mana)
    add_status_control(hud, "stamina", definition.stamina, assert(locale.text("d2legacy.hud.stamina")), hud.tips.stamina)
    add_status_control(hud, "experience", definition.experience, assert(locale.text("d2legacy.hud.experience")), hud.tips.experience)
    ...
    local run_tip = tooltip.create(hud.root, assert(locale.text("d2legacy.hud.walk")), run.x + run.width / 2, run.y, {})
    ...
    local menu_tip = tooltip.create(hud.root, assert(locale.text("d2legacy.minipanel.open")), menu.x + menu.width / 2, menu.y, {})
    ...
    hud.controls:add({
        ...
        on_activate = function()
            ...
            menu_tip:set_text(assert(locale.text(hud.menu_open and "d2legacy.minipanel.close" or "d2legacy.minipanel.open")))
...
function M.snapshot(hud, stats)
    ...
    if hud.running ~= running then
        ...
        hud.run_tip:set_text(assert(locale.text(running and "d2legacy.hud.run" or "d2legacy.hud.walk")))
    end
    ...
    hud.tips.health:set_text(string.format("%s: %d / %d", assert(locale.text("d2legacy.hud.health")), health, max_health))
    hud.tips.mana:set_text(string.format("%s: %d / %d", assert(locale.text("d2legacy.hud.mana")), mana, max_mana))
    hud.tips.stamina:set_text(string.format("%s: %d / %d", assert(locale.text("d2legacy.hud.stamina")), stamina, max_stamina))
    hud.tips.experience:set_text(string.format("%s: %d / %d", assert(locale.text("d2legacy.hud.experience")), experience, next_experience))
end
