...
local roster_composite = require("d2legacy.characters.composite")
...
local function for_mode(mode)
    ...
    return {
        create = function(self)
            -- The authority adapter returns the same immutable character records
            -- whether they came from local profile storage or a Realm account.
            self.characters = roster.characters()
            ...
            if #self.characters == 0 then
                ...
                scenes.replace(roster.create_scene)
                return
            end
            ...
            self.show_error = function(message)
                ...
                text.set(self.error_node, "network_error", message or "", 220, "center")
                self.error_node:set_position(313, 500)
            end
            ...
            local function launch_selected()
                ...
                roster.activate(self, self.selected_id)
            end
                        ...
                        if character then
                            ...
                            slot.label:set_position(
                                screen.grid.x
                                    ...
                                    + label_width / 2,
                                screen.grid.y
                                    + row * screen.grid.row_step
                                    + screen.grid.text_offset.y
                                    + label_height / 2
                            )

                            -- Roster portraits are always ordinary player COF/DCC
                            -- composites. Imported characters use their exact saved
                            -- equipment snapshot; fresh characters resolve the class
                            -- default town-idle composite. The large class-selection
                            -- DC6 actors belong exclusively to character creation.
                            local composite = roster_composite.recipe(character)
                ...
                delete = function()
                    ...
                    self.modal = dialog.confirm(
                        ...
                        function(confirmed)
                            ...
                            local deleted, err, immediate = roster.delete(self, selected.id)
                            if not deleted then
                                self.roster_error = err
                                return
                            end
            ...
            for _, id in ipairs({ "new", "delete", "exit", "ok" }) do
                ...
                ui_button.create(self.root, self.controls, id, definition, assert(locale.text(definition.label)), {
                    layer = "hud",
                    enabled = id ~= "delete" or roster.can_delete,
        ...
        update = function(self, elapsed)
            if roster.update(self) then
                ...
                self.selected_id = self.characters[1].id
                self.scrollbar.max = page_count(#self.characters, self.page_size)
                self.page = math.min(self.page, self.scrollbar.max)
                self.scrollbar.value = self.page
                self.refresh_page()
                self.update_title(self.characters[1])
            end
            if self.roster_error ~= self.displayed_roster_error then
                self.displayed_roster_error = self.roster_error
                self.show_error(self.roster_error)
            end
