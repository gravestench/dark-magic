...
return {
    create = function(self)
        ...
        self.games = list.create(self.root, self.controls, "games", {
            ...
            item_label = function(item)
                return string.format("%s   %d/%d", item.name, item.players, item.maximum_players)
            end,
            ...
            normal_style = "realm_lobby_gold",
            hover_style = "realm_lobby_text",
            selected_style = "realm_lobby_text",
        })
        ...
        lobby_ui.add_navigation(self.root, self.controls, {
            create = function()
                scenes.replace("realm_game_create")
            end,
            join = function()
                self.controls:set_focus("game_name")
            end,
            quit = function()
                begin_logout(self)
            end,
        ...
        button.create(
            ...
            self.controls,
            "cancel_game",
            lobby_button("panel_cancel_button", 432, 399),
            "CANCEL",
            {
                normal_style = "realm_lobby_button",
                hover_style = "realm_lobby_button",
                on_activate = function()
                    clear_game_selection(self)
                end,
            }
        )
    ...
    update = function(self, elapsed)
        ...
        if selected.game_id then
            detail = string.format(
                "%s\n%s\n%d/%d PLAYERS",
                selected.name or "",
                string.upper(selected.difficulty or "normal"),
                selected.players or 0,
                selected.maximum_players or 8
            )
        end
        ...
        for _, player in ipairs((state.selected_game or {}).players or {}) do
            player_lines[#player_lines + 1] =
                string.format("%s  LEVEL %d %s", player.name or "", player.level or 1, string.upper(player.class or ""))
        end
        ...
        elseif state.phase == "game_connected" then
        ...
        if state.phase == "lobby" and self.pending_game_detail then
            if realm.select_game(self.pending_game_detail) then
                self.pending_game_detail = nil
            end
        elseif state.phase == "lobby" and self.pending_join then
            local request = self.pending_join
            if realm.join_game(request.reference, request.password) then
        ...
        if self.last_refresh >= 1 and state.phase == "lobby" then
            self.last_refresh = 0
            realm.refresh()
        end
