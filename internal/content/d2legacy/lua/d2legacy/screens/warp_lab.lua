...
local function planned_route(self, global_x, global_y, stop_radius)
    local position = self.ecs.get(self.fixture.player, "d2legacy.world.position")
    ...
    if not path then
        self.ecs.get(self.fixture.player, "d2legacy.lab.warp.state"):set(
...
function lab:update_actor(mode, elapsed, stamp)
    local position = self.ecs.get(self.fixture.player, "d2legacy.world.position")
    local actor = self.ecs.get(self.fixture.player, "d2legacy.lab.warp.actor")
...
function lab:update_portals()
    ...
    for _, item in ipairs(portals) do
        local position = self.ecs.get(item.entity, "d2legacy.world.position")
        local x, y = position:get("x"), position:get("y")
        ...
        if item.stamp == self.active then
            local player = self.ecs.get(self.fixture.player, "d2legacy.world.position")
...
function lab:create()
    ...
    self.fixture_module = require("d2legacy.dev.warp_lab.fixture")
...
function lab:update(elapsed)
    ...
    local moving = self.ecs.get(self.fixture.player, "d2legacy.lab.warp.intent") ~= nil
        or self.ecs.get(self.fixture.player, "d2legacy.lab.warp.move_intent") ~= nil
    ...
    local state = self.ecs.get(self.fixture.player, "d2legacy.lab.warp.state")
