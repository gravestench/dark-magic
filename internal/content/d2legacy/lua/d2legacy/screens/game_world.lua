...
local function destroy_players(self)
    for _, player in pairs(self.remote_players or {}) do
        if player.node and player.node:exists() then
            player.node:destroy()
        end
    end
    self.remote_players = {}
...
local function destroy_missiles(self)
    for _, missile in pairs(self.missiles or {}) do
        if missile.node and missile.node:exists() then
...
local function place_region_node(self, node, x, y, width, height)
    node:set_position(x - self.world_canvas_width / 2 + width / 2, y - self.world_canvas_height / 2 + height / 2)
end

local function refresh_collision_overlay(self, hero_x, hero_y)
    if not self.collision_visible or not self.world or not self.map then
        return
    end
    local cell_x, cell_y = math.floor(hero_x / 5), math.floor(hero_y / 5)
    local key = string.format("%d:%d", cell_x, cell_y)
    if key == self.collision_region_key then
        return
    end
    ...
    local x, y, width, height = self.collision_node:set_world_collision_region(
        self.world,
        math.floor(hero_x) - radius,
        math.floor(hero_y) - radius,
        math.floor(hero_x) + radius + 1,
        math.floor(hero_y) + radius + 1
    )
    place_region_node(self, self.collision_node, x, y, width, height)
end

local function refresh_tile_overlay(self, hero_x, hero_y)
    if not self.tile_debug_visible or not self.world or not self.map then
        return
    end
    local cell_x, cell_y = math.floor(hero_x / 5), math.floor(hero_y / 5)
    local key = string.format("%d:%d", cell_x, cell_y)
    if key == self.tile_debug_region_key then
        return
    end
...
local function create_cross(layer, parent, z, r, g, b)
    ...
    horizontal:fill_rect(17, 3, r, g, b, 255)
    vertical:fill_rect(3, 17, r, g, b, 255)
    horizontal:set_z(z)
    vertical:set_z(z)
...
local function set_cross(marker, visible, x, y)
    marker.horizontal:set_visible(visible)
    marker.vertical:set_visible(visible)
    if visible then
        marker.horizontal:set_position(x, y)
        marker.vertical:set_position(x, y)
    end
end
...
local function update_debug_legend(self)
    ...
    text.set(
        ...
        string.format(
            ...
            self.origins_visible and "[green]ON[/]" or "[red]off[/]"
        ),
        760,
        "center"
    )
...
local function update_players(self, elapsed)
    ...
    for _, snapshot in ipairs(self.gameplay_world.player_snapshots(controlled_player_id(), network_connected())) do
        if snapshot.level_id == self.world_level_id then
            ...
            visible[key] = true
            local player = self.remote_players[key]
            if not player then
                player = { node = render.create("world", self.map.root) }
                player.node:set_visible(false)
                self.remote_players[key] = player
            end
            local composite = player_composite.resolve(snapshot, nil)
            ...
            player.node:set_z(self.world:entity_depth(snapshot.x, snapshot.y))
        end
    end
    for key, player in pairs(self.remote_players) do
        if not visible[key] then
            player.node:destroy()
            self.remote_players[key] = nil
        end
    end
end

local function finish_monster_preload(monster, composite)
    if not monster.pending_job then
        return
    end
    ...
    if composite and status.failed == 0 and pending.key == composite.key then
        ...
        monster.node:set_cof_animation(
            pending.cof,
            pending.palette,
            pending.direction,
            pending.components,
...
local function update_monster(self, monster, snapshot, elapsed)
    ...
    if composite then
        monster.events = monster_composite.advance(monster.playback, composite, elapsed)
    end
    if
        composite
        and composite.key ~= monster.composite_key
        and composite.key ~= monster.failed_composite_key
        and not monster.pending_job
    then
    ...
    for _, snapshot in ipairs(self.gameplay_world.missile_snapshots()) do
        if snapshot.level_id == self.world_level_id then
            ...
            if recipe and not missile.pending_job and not missile.ready and not missile.failed then
                ...
                missile.pending_job = render.preload({
                    { kind = "dcc", path = recipe.path, palette = recipe.palette, direction = recipe.direction },
                })
            end
            if missile.pending_job then
                ...
                if status.done then
                    if status.failed == 0 then
                        ...
                        missile.node:set_dcc_animation(
                            ready.path,
                            ready.palette,
                            ready.direction,
            ...
            local x, y = self.world:subtile_to_pixel(snapshot.x, snapshot.y)
            missile.node:set_position(
                x - self.world_canvas_width / 2 + snapshot.offset_x,
...
return {
    create = function(self)
        ...
        self.gameplay_world = require("d2legacy.gameplay.world")
        ...
        if render.assets_available() then
            ...
            -- different jobs.
            --
            -- engine.world/v1 produces renderer-INDEPENDENT semantic map facts:
        ...
        if render.assets_available() then
            self.world_hover_tip =
                tooltip.create(self.root, "", 0, 0, { origin_x = "left", origin_y = "top", alpha = 190 })
        end
    ...
    update = function(self, elapsed, focused, input_allowed, world_view)
        ...
        if self.hud then
            game_hud.set_item_cursor_visible(self.hud, input_allowed)
        end
        ...
        if self.hud then
            ...
            game_hud.update(self.hud, snapshot)
        ...
        if input.pressed("debug_origins") then
            ...
            if not self.origins_visible then
                destroy_cross(self.hero_origin)
                self.hero_origin = nil
            end
        end
        ...
        if self.origins_visible and not self.hero_origin and self.map then
            self.hero_origin = create_cross("world", self.map.root, 900002, 255, 64, 255)
        end
        if self.hero_origin then
            set_cross(self.hero_origin, true, hero_screen_x, hero_screen_y)
        end
        ...
        if self.map then
            ...
            local _, map_error, effective_target_x, effective_target_y =
                chunked_map.update(self.map, camera_pixel_x, camera_pixel_y, target_x, target_y, world_view)
            if map_error then
                error("updating world map: " .. tostring(map_error))
            end
        ...
        local in_world = pointer_y >= 0 and pointer_y < (screen.world_input_bottom or manifest.resolution.height)
        if world_view == "left" then
        ...
        if self.world_hover_tip then
            ...
            if hover then
                self.world_hover_tip:set_text(hover.label ~= "" and hover.label or hover.kind)
                self.world_hover_tip:set_position(pointer_x + 16, pointer_y + 18)
            end
        end
        ...
        if
            ...
            and (primary_pressed or (primary_down and not self.pending_interaction))
        then
            local target_world_x, target_world_y =
                self.world:screen_to_subtile(pointer_x, pointer_y, camera_x, camera_y, target_x, target_y)
