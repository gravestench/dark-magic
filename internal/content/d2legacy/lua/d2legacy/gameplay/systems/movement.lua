...
local function move_x(entity, units, position, velocity, bounds, collider, collision, elapsed)
    if velocity:get("x") == 0 then
        return
    end
...
local function move_y(entity, units, position, velocity, bounds, collider, collision, elapsed)
    if velocity:get("y") == 0 then
        return
    end
    local x, y = position:get("x"), position:get("y")
    local radius = collider:get("radius")
...
local function update_entity(entity, units, elapsed)
    ...
    move_x(entity, units, position, velocity, bounds, collider, collision, elapsed)
    move_y(entity, units, position, velocity, bounds, collider, collision, elapsed)
end
...
function M.register(collision)
    ...
    ecs.system({
        ...
        update = function(context, entities)
            ...
            for _, entity in ipairs(entities) do
                update_entity(entity, units, context.delta_seconds)
            end
        end,
    })
end
