...
local function difficulty_value(row, columns, difficulty)
    if difficulty == 2 then return integer(row, columns.hell) end
    if difficulty == 1 then return integer(row, columns.nightmare) end
...
local function scaled_stats(stats, level, difficulty)
    if truth(stats, "noRatio") or not level then return raw_stats(stats, difficulty) end

    local percentages = raw_stats(stats, difficulty)
    return {
        life_min = ratio(
            difficulty_value(level, columns("HP"), difficulty),
            percentages.life_min
        ),
        life_max = ratio(
            difficulty_value(level, columns("HP"), difficulty),
            percentages.life_max
        ),
        defense = ratio(
            difficulty_value(level, columns("AC"), difficulty),
            percentages.defense
        ),
        attack = ratio(
            difficulty_value(level, columns("TH"), difficulty),
            percentages.attack
        ),
        damage_min = ratio(
            difficulty_value(level, columns("DM"), difficulty),
            percentages.damage_min
        ),
        damage_max = ratio(
            difficulty_value(level, columns("DM"), difficulty),
            percentages.damage_max
        ),
        experience = ratio(
            difficulty_value(level, columns("XP"), difficulty),
...
local function treasure_class(stats, difficulty)
    if difficulty == 2 then return stats["TreasureClass1(H)"] or "" end
    if difficulty == 1 then return stats["TreasureClass1(N)"] or "" end
    return stats.TreasureClass1 or ""
end

local function aggro_radius(stats)
    local radius = integer(stats, "aidist", 35)
    if radius > 0 then return radius end
    return 35
end

local function graphics_id(stats)
    if stats.MonStatsEx and stats.MonStatsEx ~= "" then return stats.MonStatsEx end
    return stats.Id
end

local function runtime_definition(stats, graphics, values, difficulty)
    local size = math.max(
        integer(graphics, "SizeX", 2),
        integer(graphics, "SizeY", 2)
    ) / 2
    return {
        id = stats.Id,
...
function M.load(id, difficulty)
    local stats = assert(find(
        records.load("data/global/excel/monstats.txt"),
        "Id",
        id
    ), "missing MonStats row")
    assert(truth(stats, "enabled") and truth(stats, "isSpawn")
        and not truth(stats, "npc"), "monster is not an ordinary spawn")

    local graphics = assert(find(
        records.load("data/global/excel/monstats2.txt"),
        "Id",
        graphics_id(stats)
    ), "missing MonStats2 row")
    local level = find(
        records.load("data/global/excel/monlvl.txt"),
        "Level",
        stats.Level
    )
    return runtime_definition(
        stats,
        graphics,
        scaled_stats(stats, level, difficulty),
        difficulty
    )
end
