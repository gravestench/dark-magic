...
local components = require("d2.gameplay.components.init")
local systems = require("d2.gameplay.systems.init")
...
function M.bind(state)
    ...
    local entities = ecs.query({
        all = {
            "d2.player.identity", "d2.player.appearance", "d2.player.animation",
            "d2.world.position", "d2.world.velocity",
            "d2.world.player_control", "d2.world.bounds", "d2.world.collider",
        },
    })

    for _, entity in ipairs(entities) do
        local control = ecs.get(entity, "d2.world.player_control")
        ...
        if control:get("player") == state.player then
            ...
            state.camera = ecs.create({
                ...
                ["d2.world.position"] = ecs.get(entity, "d2.world.position"):snapshot(),
                ["d2.world.camera_follow"] = { target = entity },
            })
            return true
        end
    end
...
function M.position(entity)
    local position = assert(ecs.get(entity, "d2.world.position"))
...
function M.composite_snapshot(entity)
    local appearance = assert(ecs.get(entity, "d2.player.appearance"))
    local animation = assert(ecs.get(entity, "d2.player.animation"))
...
function M.monster_snapshots()
    ...
    local entities = ecs.query({ all = {
        "d2.monster.identity", "d2.monster.appearance", "d2.world.position",
        "d2.world.velocity", "d2.world.location",
    }})
    for _, entity in ipairs(entities) do
        local identity = ecs.get(entity, "d2.monster.identity"):snapshot()
        local appearance = ecs.get(entity, "d2.monster.appearance"):snapshot()
        local position = ecs.get(entity, "d2.world.position")
        local velocity = ecs.get(entity, "d2.world.velocity")
        local location = ecs.get(entity, "d2.world.location")
        local ai = ecs.get(entity, "d2.monster.ai")
...
function M.semantic_cues()
    ...
    for _, component in ipairs({"d2.monster.death_event", "d2.missile.event",
    ...
    for _, entity in ipairs(entities) do
        local kind, values
        local death = ecs.get(entity, "d2.monster.death_event")
        if death then kind, values = "monster_death", death:snapshot() end
        local missile = ecs.get(entity, "d2.missile.event")
...
function M.missile_snapshots()
    ...
    for _, component in ipairs({ "d2.missile.instance", "d2legacy.missile.projectile" }) do
        local ok, entities = pcall(ecs.query, { all = {
            component, "d2.world.position", "d2.world.location",
        } })
        if ok then
            for _, entity in ipairs(entities) do
...
function M.hud_snapshot(entity, saved)
    ...
    local progress = assert(ecs.get(entity, "d2.player.progress"))
    local vitals = assert(ecs.get(entity, "d2.player.vitals"))
    local movement_mode = assert(ecs.get(entity, "d2.player.movement_mode"))
    local skills = assert(ecs.get(entity, "d2.player.skill_assignment"))
    local belt = assert(ecs.get(entity, "d2.player.belt"))

    local belt_slots = {}
    for slot = 1, 16 do belt_slots[slot] = belt:get("slot_" .. slot) end

    local learned_skills = {}
    for _, learned in ipairs(ecs.query({ all = { "d2.player.learned_skill" } })) do
        local skill = ecs.get(learned, "d2.player.learned_skill")
