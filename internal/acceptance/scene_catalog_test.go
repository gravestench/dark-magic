...
// TestCaptureAllCatalogMatchesRegisteredScenes keeps automated capture coverage aligned with boot registration.
func TestCaptureAllCatalogMatchesRegisteredScenes(t *testing.T) {
	...
	catalogPattern := regexp.MustCompile(`(?m)^\s+([a-z0-9_]+)=\{(?:title|module)=`)
	for _, match := range catalogPattern.FindAllStringSubmatch(bootstrap, -1) {
		registered = append(registered, string(match[1]))
	}
	...
	if !slices.Equal(captured, registered) {
		t.Fatalf(
			"capture scenes do not match boot registrations\ncapture: %s\nregistered: %s",
...
// TestMessagesShellLeavesGameplayLive protects the overlay flags that keep the world visible and updating.
...
// TestGameWorldUsesChunkedAuthoritativeCameraAdapter prevents the retired full-map camera path from returning.
...
// TestMapgenLabRegenerationUsesDocumentedRenderNodeLifetime protects explicit node release during regeneration.
...
// TestDS1LabUsesNativeChunkDepthForLayerOrdering keeps semantic layer IDs separate from draw depth.
...
// readBootstrapLua concatenates all boot modules because scene registrations are intentionally split by domain.
