...
func TestCatalogBuildsClonedTypedSnapshotAndIndexes(t *testing.T) {
	...
	source := fstest.MapFS{
		...
		MonsterSequencesTable:     &fstest.MapFile{Data: []byte("sequence\tmode\tframe\tevent\nzombie_attack\tA1\t0\t1\n")},
		MonsterUniqueModsTable:    &fstest.MapFile{Data: []byte("uniquemod\tid\tenabled\nExtra Strong\t5\t1\n")},
		UniqueAppellationsTable:   &fstest.MapFile{Data: []byte("Name\nBeast\n")},
		UniquePrefixesTable:       &fstest.MapFile{Data: []byte("name\nBishibosh\n")},
		UniqueSuffixesTable:       &fstest.MapFile{Data: []byte("name\nthe Quick\n")},
