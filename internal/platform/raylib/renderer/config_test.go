...
// TestDefaultConfigDataRoundTripsCompleteConfiguration verifies that the embedded configuration boundary stays aligned
// with the typed defaults and continues to provide every required positive capacity.
func TestDefaultConfigDataRoundTripsCompleteConfiguration(t *testing.T) {
	...
	if got.Window.Width <= 0 || got.Window.Height <= 0 ||
		got.Resolution.Width <= 0 || got.Resolution.Height <= 0 ||
		got.Cache.BudgetMB <= 0 {
		t.Fatal("default configuration is incomplete")
	}
}

// TestWindowConfigFlagsUseDesktopBorderlessMode verifies that borderless startup fills the desktop without accidentally
// requesting an exclusive video mode.
