...
func gameViewport(windowWidth, windowHeight, gameWidth, gameHeight int, fit string) (Viewport, error) {
	...
	return Viewport{X: (float32(windowWidth) - width) / 2, Y: (float32(windowHeight) - height) / 2, Width: width, Height: height}, nil
}

// ScreenToGame maps native window pixels into logical game coordinates.
func (s *Service) ScreenToGame(x, y int) (gameX, gameY int, inside bool) {
	windowWidth, windowHeight := s.WindowSize()
	viewport, err := gameViewport(windowWidth, windowHeight, s.config.Resolution.Width, s.config.Resolution.Height, s.config.Resolution.Fit)
	if err != nil {
		return 0, 0, false
	}
...
func mapScreenToGame(viewport Viewport, gameWidth, gameHeight, x, y int) (gameX, gameY int, inside bool) {
	if float32(x) < viewport.X || float32(y) < viewport.Y || float32(x) >= viewport.X+viewport.Width || float32(y) >= viewport.Y+viewport.Height {
		return 0, 0, false
	}
