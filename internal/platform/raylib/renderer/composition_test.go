...
// TestCompositionBackendOwnsEveryManagedResourceKind verifies symmetric admission and destruction for every resource
// type so final draining cannot strand backend-owned state.
func TestCompositionBackendOwnsEveryManagedResourceKind(t *testing.T) {
	...
	texture := render.Resource{
		ID:      render.ResourceID{Slot: 1, Generation: 1},
		Kind:    render.ResourceTexture,
		Payload: image.NewRGBA(image.Rect(0, 0, 1, 1)),
	}

	resources := []render.Resource{
		texture,
		{
			ID: render.ResourceID{Slot: 2, Generation: 1}, Kind: render.ResourcePalette,
			Payload: color.Palette{color.Black},
		},
		{
			ID: render.ResourceID{Slot: 3, Generation: 1}, Kind: render.ResourceFont,
			Payload: render.FontData{Bytes: []byte("font"), Size: 12},
		},
		{
			ID: render.ResourceID{Slot: 4, Generation: 1}, Kind: render.ResourceAnimation,
			Payload: render.AnimationData{
				Frames:    []render.ResourceID{texture.ID},
				Durations: []time.Duration{time.Second},
			},
		},
		{
			ID: render.ResourceID{Slot: 5, Generation: 1}, Kind: render.ResourceRenderTarget,
			Payload: render.RenderTargetData{Width: 2, Height: 2},
		},
	}

	for _, resource := range resources {
		change := render.Change{Kind: "resource-create", Resource: resource, ResourceID: resource.ID}
...
// TestCompositionBackendMirrorsCheckedNodes verifies parentage, transforms, visibility, and destruction when neutral
// composer nodes cross into raylib-owned scene state.
func TestCompositionBackendMirrorsCheckedNodes(t *testing.T) {
	...
	parentChange := render.Change{
		Kind: "create",
		ID:   parent,
		Node: render.Node{ID: parent, Layer: render.LayerHUD, ScaleX: 1, Visible: true},
	}
	if err := backend.Apply(parentChange); err != nil {
		t.Fatal(err)
	}

	childChange := render.Change{
		Kind: "create",
		ID:   child,
		Node: render.Node{ID: child, Parent: parent, X: 12, Y: 34, ScaleX: 2, Visible: true},
	}
	if err := backend.Apply(childChange); err != nil {
		t.Fatal(err)
	}
...
// TestCompositionBackendMapsDiabloScreenBlend verifies the custom blend selection needed by authored Diablo assets.
func TestCompositionBackendMapsDiabloScreenBlend(t *testing.T) {
	...
	change := render.Change{
		Kind: "create",
		ID:   id,
		Node: render.Node{ID: id, Layer: render.LayerHUD, ScaleX: 1, Blend: "screen"},
	}
	if err := backend.Apply(change); err != nil {
		t.Fatal(err)
	}
...
// TestBackendUsesNonUniformScaleAndDirtyTransformPropagation verifies that child world position reflects both parent
// axes and that dirty state clears only after propagation.
func TestBackendUsesNonUniformScaleAndDirtyTransformPropagation(t *testing.T) {
	...
	parentChange := render.Change{
		Kind: "create",
		ID:   parent,
		Node: render.Node{ID: parent, ScaleX: 2, ScaleY: 3, Visible: true},
	}
	if err := backend.Apply(parentChange); err != nil {
		t.Fatal(err)
	}
	...
	childChange := render.Change{
		Kind: "create",
		ID:   child,
		Node: render.Node{ID: child, Parent: parent, X: 4, Y: 5, ScaleX: 1, ScaleY: 1, Visible: true},
	}
	if err := backend.Apply(childChange); err != nil {
		t.Fatal(err)
	}
...
// TestFinalDrainLeavesNoPendingCompositionCommands verifies shutdown drains queued mutations before native resources
// are closed, preventing commands from referring to destroyed backend nodes.
