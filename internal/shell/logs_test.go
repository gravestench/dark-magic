...
// TestLogBufferIsBoundedAndTimelineIsChronological protects tail eviction and
// stable timestamp merging across transcript and process logs.
func TestLogBufferIsBoundedAndTimelineIsChronological(t *testing.T) {
	...
	invalidTimeline := len(events) != 5 ||
		events[0].Kind != "motd" ||
		!strings.Contains(events[1].Text, "middle") ||
		!strings.Contains(events[2].Text, "later")
	if invalidTimeline {
