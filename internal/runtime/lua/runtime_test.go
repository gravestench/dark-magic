...
// TestRuntimeExecutionBudgetInterruptsLuaAndRecovers protects the runtime execution budget interrupts lua and
// recovers contract, including its observable ordering and failure behavior.
func TestRuntimeExecutionBudgetInterruptsLuaAndRecovers(t *testing.T) {
	...
	err := runtime.Execute(
		context.Background(),
		fstest.MapFS{"loop.lua": &fstest.MapFile{Data: []byte(`while true do end`)}},
		"loop.lua",
	)
	...
	if err := runtime.Execute(
		context.Background(),
		fstest.MapFS{"ok.lua": &fstest.MapFile{Data: []byte(`recovered = true`)}},
		"ok.lua",
	); err != nil {
		t.Fatal(err)
	}
...
// TestRuntimeSupportsASeparateBoundedLifecycleBudget protects the runtime supports aseparate bounded lifecycle
// budget contract, including its observable ordering and failure behavior.
...
// TestRuntimeReturnsSourceAwareTraceback protects the runtime returns source aware traceback contract, including
...
func TestRuntimeReturnsSourceAwareTraceback(t *testing.T) {
	...
	err := runtime.Execute(
		context.Background(),
		fstest.MapFS{
			"broken.lua": &fstest.MapFile{
				Data: []byte("local function inner() error('boom') end\ninner()"),
			},
		},
		"broken.lua",
	)
	if err == nil || !strings.Contains(err.Error(), "broken.lua") ||
		!strings.Contains(err.Error(), "stack traceback") {
		t.Fatalf("diagnostic = %v", err)
	}
}

// TestRuntimeExecutesVersionedModuleOnOneOwner protects the runtime executes versioned module on one owner
...
func TestRuntimeExecutesVersionedModuleOnOneOwner(t *testing.T) {
	...
	if err := runtime.RegisterModule(
		Module{Name: "example.test/v1", Loader: func(state *lua.LState) int {
	...
	source := fstest.MapFS{
		"boot.lua": &fstest.MapFile{
			Data: []byte(`local test = require("example.test/v1"); result = test.answer()`),
		},
	}
...
// TestRuntimeModuleDefaultsPreserveHostOverrides protects the runtime module defaults preserve host overrides
// contract, including its observable ordering and failure behavior.
func TestRuntimeModuleDefaultsPreserveHostOverrides(t *testing.T) {
	t.Parallel()
	...
	if err := runtime.RegisterModule(host); err != nil {
		t.Fatal(err)
	}
...
// TestRuntimeHostOverrideReplacesEarlierDefault protects the runtime host override replaces earlier default
// contract, including its observable ordering and failure behavior.
func TestRuntimeHostOverrideReplacesEarlierDefault(t *testing.T) {
	...
	if err := runtime.RegisterModule(
		Module{Name: "example.catalog/v1", Loader: func(*lua.LState) int { return 0 }},
	); err == nil {
		t.Fatal("strict registration accepted duplicate after override")
	}
}

// TestRuntimeSerializesConcurrentCalls protects the runtime serializes concurrent calls contract, including its
...
// TestRuntimeDoesNotExposeAmbientNondeterminism protects the runtime does not expose ambient nondeterminism
// contract, including its observable ordering and failure behavior.
...
// TestScopeClosesInReverseAndJoinsErrors protects the scope closes in reverse and joins errors contract, including
// its observable ordering and failure behavior.
func TestScopeClosesInReverseAndJoinsErrors(t *testing.T) {
	...
	for index, failure := range []error{first, nil, second} {
		...
		if err := scope.Add(
			func() error { calls = append(calls, index); return failure },
		); err != nil {
			t.Fatal(err)
		}
	}
...
// TestHandlesRejectStaleAndMistypedReferences protects the handles reject stale and mistyped references contract,
...
func TestHandlesRejectStaleAndMistypedReferences(t *testing.T) {
	...
	if _, err := handles.Get(
		Handle{Type: "sound", Slot: texture.Slot, Generation: texture.Generation},
	); err == nil {
