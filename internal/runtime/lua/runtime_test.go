...
func TestRuntimeExecutesVersionedModuleOnOneOwner(t *testing.T) {
	...
	runtime := New()
	if err := runtime.RegisterModule(Module{Name: "d2.test/v1", Loader: func(state *lua.LState) int {
	...
	defer runtime.Stop(context.Background())
	source := fstest.MapFS{"boot.lua": &fstest.MapFile{Data: []byte(`local test = require("d2.test/v1"); result = test.answer()`)}}
