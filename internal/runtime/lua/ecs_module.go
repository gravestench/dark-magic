...
func (capability *ECSCapability) Module() Module {
	return Module{
		...
		Help: documentedModule(
			"Define runtime ECS components and deterministic scoped systems.",
			map[string]CommandHelp{
				"component": commandHelp(
					"engine.ecs.component(definition)",
					"Register or migrate a named runtime component schema.",
				),
				"create": commandHelp(
					"engine.ecs.create([components])",
					"Create an entity and optionally attach named components.",
				),
				"get": commandHelp(
					"engine.ecs.get(entity, component)",
					"Return a checked component reference or nil.",
				),
				"query": commandHelp(
					"engine.ecs.query(filter)",
					"Return an ordered snapshot of entities matching all, any, and none component filters.",
				),
				"set": commandHelp(
					"engine.ecs.set(entity, component, values)",
					"Add or replace a component after schema validation.",
				),
				"remove": commandHelp(
					"engine.ecs.remove(entity, component)",
					"Remove a component immediately outside system iteration.",
				),
				"destroy": commandHelp(
					"engine.ecs.destroy(entity)",
					"Destroy an entity immediately outside system iteration.",
				),
				"system": commandHelp(
					"engine.ecs.system(definition)",
					"Register a scope-owned ordered simulation system.",
				),
			},
		),
...
// queryEntities returns an ordered snapshot and closes the temporary subscription before exposing entity handles.
// Queries are rejected during system iteration because systems must use their declared, engine-supplied query.
func (capability *ECSCapability) queryEntities(state *lua.LState) int {
	if capability.active != nil {
		state.RaiseError(
			"ecs.query cannot be called inside a system; use the system query entities",
		)
	}
...
// registerECSTypes installs entity, component, and command-buffer methods before any of those handles reach Lua.
...
// loadComponent loads component before publication so malformed content cannot become observable runtime
// state.
func (capability *ECSCapability) loadComponent(state *lua.LState) int {
	...
	if existing, found := akara.GetDynamicStore(capability.engine.World(), name); found {
		...
		if current.Version == version {
			...
			if !ok {
				state.RaiseError(
					"component %q version %d requires migrate(previous, entity)",
					name,
					version,
				)
			}

			err := existing.Migrate(
				schema,
				func(entity akara.Entity, previous map[string]any) (map[string]any, error) {
					if err := state.CallByParam(
						lua.P{Fn: migrate, NRet: 1, Protect: true},
						dynamicMapToLua(state, capability.engine, previous),
						capability.entityValue(state, entity),
					); err != nil {
						return nil, err
					}
...
// createEntity owns the create entity step at this boundary, keeping its side effects and failure point explicit to
// callers.
...
// getComponent retrieves component through the capability boundary so callers cannot bypass ownership or access
// checks.
...
// setComponent applies component through the capability boundary so validation completes before shared state
// changes.
func (capability *ECSCapability) setComponent(state *lua.LState) int {
	...
	if capability.active != nil {
		state.RaiseError(
			"component replacement inside a system requires commands:set; " +
				"mutate existing fields through the component reference",
		)
	}
...
// removeComponent removes component through the owning capability so stale handles cannot bypass lifecycle
// policy.
...
// destroyEntity owns the destroy entity step at this boundary, keeping its side effects and failure point explicit
// to callers.
func (capability *ECSCapability) destroyEntity(state *lua.LState) int {
	...
	state.Push(
		lua.LBool(capability.engine.World().DestroyEntity(capability.checkEntity(state, 1).entity)),
	)

	return 1
}

// registerSystem registers register system before use so Lua cannot observe a partially configured capability.
func (capability *ECSCapability) registerSystem(state *lua.LState) int {
	...
	definition := gameecs.Definition{
		ID:     id,
		Phase:  phase,
		After:  tableStringList(state, table.RawGetString("after")),
		Before: tableStringList(state, table.RawGetString("before")),
		All:    componentTypes(all),
		Any:    componentTypes(any),
		None:   componentTypes(none),
		Read:   componentTypes(read),
		Write:  componentTypes(write),
	}

	definition.Update = func(ctx gameecs.Context, entities []akara.Entity, commands *gameecs.StructuralCommands) error {
		return capability.runtime.runScoped(
			context.Background(),
			scope,
			func(state *lua.LState) error {
				...
				if err := state.CallByParam(
					lua.P{Fn: update, NRet: 0, Protect: true},
					contextTable,
					entityTable,
					commandsValue,
				); err != nil {
...
// stores owns the stores step at this boundary, keeping its side effects and failure point explicit to callers.
...
// componentTypes owns the component types step at this boundary, keeping its side effects and failure point
// explicit to callers.
...
// nameSet owns the name set step at this boundary, keeping its side effects and failure point explicit to callers.
...
// requireRead enforces require read before execution so undeclared capability access fails deterministically.
...
// requireWrite enforces require write before execution so undeclared capability access fails deterministically.
...
// setDynamic applies dynamic through the capability boundary so validation completes before shared state
// changes.
func (capability *ECSCapability) setDynamic(
	entity akara.Entity,
	name string,
	value lua.LValue,
) (*akara.DynamicComponent, error) {
	store, found := akara.GetDynamicStore(capability.engine.World(), name)
	if !found {
		return nil, fmt.Errorf("unknown component %q", name)
	}
...
// entityValue owns the entity value step at this boundary, keeping its side effects and failure point explicit to
// callers.
func (capability *ECSCapability) entityValue(
	state *lua.LState,
	entity akara.Entity,
) *lua.LUserData {
...
// componentValue owns the component value step at this boundary, keeping its side effects and failure point
// explicit to callers.
func (capability *ECSCapability) componentValue(
	state *lua.LState,
	name string,
	component *akara.DynamicComponent,
) *lua.LUserData {
...
// write owns the write step at this boundary, keeping its side effects and failure point explicit to callers.
...
// commandSet owns the command set step at this boundary, keeping its side effects and failure point explicit to
// callers.
...
// commandCreate owns the command create step at this boundary, keeping its side effects and failure point explicit
// to callers.
...
// commandRemove owns the command remove step at this boundary, keeping its side effects and failure point explicit
// to callers.
...
// commandDestroy owns the command destroy step at this boundary, keeping its side effects and failure point
// explicit to callers.
...
// checkECSEntity validates check ecsentity at the Lua boundary so invalid values fail before shared state can
// change.
...
// checkEntity validates check entity at the Lua boundary so invalid values fail before shared state can change.
...
// checkECSComponent validates check ecscomponent at the Lua boundary so invalid values fail before shared state can
// change.
...
// checkECSCommands validates check ecscommands at the Lua boundary so invalid values fail before shared state can
// change.
...
// requiredTableString enforces required table string before execution so undeclared capability access fails
// deterministically.
...
// tableInteger owns the table integer step at this boundary, keeping its side effects and failure point explicit to
// callers.
...
// tableStringList owns the table string list step at this boundary, keeping its side effects and failure point
// explicit to callers.
...
// schemaField owns the schema field step at this boundary, keeping its side effects and failure point explicit to
// callers.
...
// luaDynamicMapForSchema owns the lua dynamic map for schema step at this boundary, keeping its side effects and
...
// luaToDynamic owns the lua to dynamic step at this boundary, keeping its side effects and failure point explicit
// to callers.
func luaToDynamic(_ *lua.LState, kind akara.FieldKind, value lua.LValue) any {
	...
	case akara.FieldUint64:
		if typed, ok := value.(lua.LNumber); ok && typed >= 0 &&
			math.Trunc(float64(typed)) == float64(typed) {
			return uint64(typed)
		}
...
// dynamicToLua publishes dynamic to lua in one canonical Lua representation, keeping scripts independent of Go
// storage details.
...
// dynamicMapToLua publishes dynamic map to lua in one canonical Lua representation, keeping scripts independent of
// Go storage details.
