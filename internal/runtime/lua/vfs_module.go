...
func VFSModule(source *content.FS) Module {
	return Module{
		...
		Help: documentedModule(
			"Read assets from the layered virtual filesystem.",
			map[string]CommandHelp{
				...
				"source": commandHelp(
					"engine.vfs.source(path)",
					"Describe the layer and resolved path supplying an asset.",
				),
				"list": commandHelp(
					"engine.vfs.list(prefix[, suffix])",
					"List mounted asset paths below a prefix, optionally filtered by case-insensitive suffix.",
				),
			},
		),
		Loader: func(state *lua.LState) int {
			...
			module.RawSetString("name", lua.LString("engine.vfs/v1"))
			state.Push(module)
