...
// TestVersionedCapabilityConformance protects the versioned capability conformance contract, including its
...
func TestVersionedCapabilityConformance(t *testing.T) {
	...
	modules := []Module{
		...
		DataModule(contentFS),
		WorldModule(contentFS),
		InputModule(&input),
		AudioModule(runtime, &mixer, source),
		SettingsModule(preferences.NewTransient(), &mixer),
		VideoModule(runtime, video.Unavailable{}, source),
		RecordsModule(recordstore.New(source)),
		LocaleModule(localization.New(source, "English")),
		PlayerControlModule(
			&testPlayerController{},
		),
		CommandIntentModule(&gamesession.IntentController{}),
		SimulationModule(NewSimulation(scene.New(1, 10, 10))),
		RenderModule(runtime, &composer),
		scenes.Module(),
	}
	expected := map[string][]string{
		...
		"engine.input/v1": {"down", "pressed", "released", "cursor", "text"},
		...
		"engine.records/v1":        {"load", "reload", "loaded"},
		...
		"engine.simulation/v1": {
			"move_hero",
			"state",
		},
		"engine.render/v1": {"create", "diagnostics"},
