...
func TestContentRequireLoadsModuleFromVFS(t *testing.T) {
	...
	source := fstest.MapFS{
		"lua/d2legacy/screens/loading.lua": &fstest.MapFile{Data: []byte(`return { id = "loading" }`)},
		"boot.lua":                         &fstest.MapFile{Data: []byte(`screen_id = require("d2legacy.screens.loading").id`)},
	}
	runtime := New()
