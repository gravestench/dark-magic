...
// TestDCCGroundOriginPreservesAuthoredCanvasAnchor protects the dccground origin preserves authored canvas anchor
...
// TestNormalizedDC6FramesPreserveSharedAnchor protects the normalized dc6 frames preserve shared anchor contract,
...
// TestDCCDirectionForCOFUsesIndependentLegacyOrder protects the dccdirection for cofuses independent legacy order
// contract, including its observable ordering and failure behavior.
...
// TestCombinedDC6PagesJoinsFrameTiles protects the combined dc6 pages joins frame tiles contract, including its
...
// TestHorizontalDC6StripPreservesExplicitRightCap protects the horizontal dc6 strip preserves explicit right cap
// contract, including its observable ordering and failure behavior.
...
// TestAssetWeightReadsAssetContents protects the asset weight reads asset contents contract, including its
// observable ordering and failure behavior.
...
// TestRenderNodesBelongToLuaComponentScope protects the render nodes belong to lua component scope contract,
// including its observable ordering and failure behavior.
...
`)}}
	...
	if len(nodes) != 1 || nodes[0].X != 320 || nodes[0].Y != 240 || nodes[0].Z != 7 ||
		nodes[0].Layer != render.LayerTransition {
		t.Fatalf("nodes = %#v", nodes)
	}

	if nodes[0].Clip == nil ||
		*nodes[0].Clip != (render.Rect{X: 10, Y: 20, Width: 300, Height: 200}) {
...
// TestRenderNodePaletteQuantizationAcceptsTinyPalettes protects the render node palette quantization accepts tiny
// palettes contract, including its observable ordering and failure behavior.
...
`)},
	...
	if err := runtime.RegisterModule(
		RenderModuleWithAssets(runtime, &composer, assets),
	); err != nil {
		t.Fatal(err)
	}
...
// TestRenderCapabilityExposesCOFCompositionMetadata verifies that Lua receives COF dimensions, layer metadata,
// priority order, and frame events without decoding component graphics.
func TestRenderCapabilityExposesCOFCompositionMetadata(t *testing.T) {
	...
	input.CofLayers = []cof.CofLayer{
		{Type: 0, Selectable: true, WeaponClass: cof.WeaponClassFromString("hth")},
	}
	...
	script := `local r=require("engine.render/v1"); local c=assert(r.cof_info("unit.cof")); ` +
		`assert(c.directions==1 and c.frames==1 and c.layers[1].type=="HD" and ` +
		`c.priority[1][1][1]=="HD" and c.events[1]==1)`
...
// TestCOFCompositionUsesFramePriorityAndPlacement protects the cofcomposition uses frame priority and placement
// contract, including its observable ordering and failure behavior.
func TestCOFCompositionUsesFramePriorityAndPlacement(t *testing.T) {
	...
	asset.CofLayers = []cof.CofLayer{
		{Type: head, DrawEffect: none},
		{Type: torso, DrawEffect: none},
	}
...
// TestCOFCompositionDrawsOnlyShadowEnabledLayers protects the cofcomposition draws only shadow enabled layers
// contract, including its observable ordering and failure behavior.
...
// TestCompositeShadowUsesLegacyHalfHeightShear protects the composite shadow uses legacy half height shear
// contract, including its observable ordering and failure behavior.
...
// TestCompositeShadowProjectsPartsFromOneSharedBaseline protects the composite shadow projects parts from one
// shared baseline contract, including its observable ordering and failure behavior.
...
// TestCOFCompositionDrawsAllShadowsBehindAllVisibleLayers protects the cofcomposition draws all shadows behind all
// visible layers contract, including its observable ordering and failure behavior.
func TestCOFCompositionDrawsAllShadowsBehindAllVisibleLayers(t *testing.T) {
	...
	if got := color.RGBAModel.Convert(composed.At(point.X, point.Y)).(color.RGBA); got.R != 255 ||
		got.A != 255 {
		t.Fatalf("visible back layer was covered by a later component shadow: %#v", got)
	}
}

// TestCOFCompositionIgnoresDrawEffectOnOpaqueLayer protects the cofcomposition ignores draw effect on opaque layer
// contract, including its observable ordering and failure behavior.
func TestCOFCompositionIgnoresDrawEffectOnOpaqueLayer(t *testing.T) {
	...
	head := cof.CompositeType(0)
	asset.CofLayers = []cof.CofLayer{
		{Type: head, Transparent: false, DrawEffect: cof.DrawEffect(0)},
	}
	asset.Priority = [][][]cof.CompositeType{{{head}}}
	source := image.NewRGBA(image.Rect(0, 0, 1, 1))
	source.Set(0, 0, color.RGBA{R: 255, A: 255})
...
// TestCOFAnimationFramesShareCanvasButKeepDistinctPixels protects the cofanimation frames share canvas but keep
// distinct pixels contract, including its observable ordering and failure behavior.
...
// TestRenderNodeDecodesPaletteAwareDC6 protects the render node decodes palette aware dc6 contract, including its
// observable ordering and failure behavior.
...
`)}}
	...
	if animationNode.AnimationPaused || animationNode.AnimationSeekRevision != 2 ||
		animationNode.AnimationSeek != 250*time.Millisecond {
...
// TestRenderCapabilityLazyDecodesDT1LabTile protects the render capability lazy decodes dt1 lab tile contract,
// including its observable ordering and failure behavior.
func TestRenderCapabilityLazyDecodesDT1LabTile(t *testing.T) {
	...
	if prepared.total != 1 || prepared.tile.Type != 2 || prepared.tile.Style != 3 ||
		prepared.tile.Sequence != 4 {
...
// TestRenderCapabilityPreloadsAssetsAndReportsProgress protects the render capability preloads assets and reports
// progress contract, including its observable ordering and failure behavior.
func TestRenderCapabilityPreloadsAssetsAndReportsProgress(t *testing.T) {
	...
	capability := NewRenderCapability(New(), &render.Composer{}, assets)
	job := capability.preloads.Start(
		[]AssetPreloadRequest{
			{Kind: "dc6_animation", Path: "one.dc6", Palette: "pal.dat", Anchor: "offsets"},
		},
	)
	...
	if _, err := capability.cache.loadDC6Animation(
		assets,
		"one.dc6",
		"pal.dat",
		0,
...
// TestDC6DirectionLoadingDoesNotDecodeOtherDirections protects the dc6 direction loading does not decode other
// directions contract, including its observable ordering and failure behavior.
...
// TestRenderCapabilityRequiresComponentScope protects the render capability requires component scope contract,
// including its observable ordering and failure behavior.
func TestRenderCapabilityRequiresComponentScope(t *testing.T) {
	...
	err := runtime.Execute(
		context.Background(),
		fstest.MapFS{
			"bad.lua": &fstest.MapFile{
				Data: []byte(`local render = require("engine.render/v1"); render.create("world")`),
			},
		},
		"bad.lua",
	)
...
// TestChunkResourcesUseTheirResidencyCacheBudgets protects the chunk resources use their residency cache budgets
// contract, including its observable ordering and failure behavior.
...
// TestOwnedRenderNodeReleaseToleratesRecursiveParentDestruction protects the owned render node release tolerates
// recursive parent destruction contract, including its observable ordering and failure behavior.
...
// TestAnimationSharesIdenticalRGBAFrames protects the animation shares identical rgbaframes contract, including its
// observable ordering and failure behavior.
func TestAnimationSharesIdenticalRGBAFrames(t *testing.T) {
	...
	if err := node.setAnimation(
		[]image.Image{first, duplicate, different},
		time.Millisecond,
		"loop",
	); err != nil {
		t.Fatal(err)
