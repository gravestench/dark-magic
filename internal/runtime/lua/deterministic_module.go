...
func DeterministicModule() Module {
	return Module{
		...
		Help: documentedModule(
			"Derive stable values from a seed and purpose without mutable random state.",
			map[string]CommandHelp{
				"integer": commandHelp(
					"engine.deterministic.integer(seed, purpose, limit [, ordinal])",
					"Return a stable zero-based integer below limit.",
				),
			},
		),
...
// deterministicInteger owns the deterministic integer step at this boundary, keeping its side effects and failure
...
// exactLuaUint owns the exact lua uint step at this boundary, keeping its side effects and failure point explicit
// to callers.
