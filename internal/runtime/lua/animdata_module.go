...
func AnimDataModule(source animDataReader) Module {
	...
	return Module{
		Name: "engine.animdata/v1",
		Help: documentedModule(
			"Read session-pinned fixed-tick animation records.",
			map[string]CommandHelp{
				"record": commandHelp(
					"engine.animdata.record(key)",
					"Resolve one typed AnimData record and its deterministic tick schedule.",
				),
			},
		),
				...
				"record": func(state *lua.LState) int {
					...
					if frames <= 0 || speed <= 0 {
						...
						state.Push(
							lua.LString(
								fmt.Sprintf(
									"animdata: record %q has invalid frames or speed",
									record.Name(),
								),
							),
						)
					...
					result.RawSetString(
						"complete_delay",
						lua.LNumber(tickForCursor(frames*animDataFrameScale, speed)),
					)
					...
					for frame := 0; frame < frames; frame++ {
						...
						entry.RawSetString(
							"delay",
							lua.LNumber(tickForCursor(frame*animDataFrameScale, speed)),
						)
						events.Append(entry)
					}
...
// tickForCursor owns the tick for cursor step at this boundary, keeping its side effects and failure point explicit
// to callers.
...
// animDataEventName owns the anim data event name step at this boundary, keeping its side effects and failure point
// explicit to callers.
