...
func RealmModule(controller RealmController) Module {
	return Module{Name: "engine.realm/v1", Help: documentedModule("Operate explicit Realm account, character, and lobby flows.", map[string]CommandHelp{
		"connect":          commandHelp("engine.realm.connect(endpoint)", "Prepare a TLS realm endpoint without opening gameplay transport."),
		"set_gateway":      commandHelp("engine.realm.set_gateway(endpoint)", "Validate and persist the selected realm gateway."),
		"login":            commandHelp("engine.realm.login(name, password)", "Log into the prepared Realm with explicit account credentials."),
		"logout":           commandHelp("engine.realm.logout()", "Log out and remove the selected character from live Realm presence."),
		"signup":           commandHelp("engine.realm.signup(name, email, password)", "Create an unverified Realm account and send its verification email."),
		"recover_password": commandHelp("engine.realm.recover_password(email)", "Send a browser password-reset link without revealing whether the email exists."),
		"create_character": commandHelp("engine.realm.create_character(name, class, expansion, hardcore)", "Create and select a realm-owned character."),
		"delete_character": commandHelp("engine.realm.delete_character(id)", "Delete an idle account-owned character."),
		"select_character": commandHelp("engine.realm.select_character(id)", "Select an account-owned character."),
		"join_channel":     commandHelp("engine.realm.join_channel(name)", "Enter a public realm channel with the selected character."),
		"send_message":     commandHelp("engine.realm.send_message(text)", "Send public channel chat or a slash command."),
		"refresh":          commandHelp("engine.realm.refresh()", "Refresh channel members, chat, and public games."),
		"select_game":      commandHelp("engine.realm.select_game(name_or_id)", "Load public details for a selected realm game."),
		"create_game":      commandHelp("engine.realm.create_game(options)", "Create a uniquely named realm game."),
		"join_game":        commandHelp("engine.realm.join_game(name_or_id, password)", "Resolve a named realm game for admission."),
		"cancel":           commandHelp("engine.realm.cancel()", "Cancel the current realm request."),
		...
		module := state.SetFuncs(state.NewTable(), map[string]lua.LGFunction{
			"connect": func(state *lua.LState) int {
				return accepted(func() error { return controller.Connect(state.CheckString(1)) })
			},
			"set_gateway": func(state *lua.LState) int {
				return accepted(func() error { return controller.SetGateway(state.CheckString(1)) })
			},
			...
			"signup": func(state *lua.LState) int {
				name, email, password := state.CheckString(1), state.CheckString(2), state.CheckString(3)
				return accepted(func() error { return controller.Signup(name, email, password) })
			},
			...
			"create_character": func(state *lua.LState) int {
				...
				return accepted(func() error { return controller.CreateCharacter(name, class, expansion, hardcore) })
			},
			...
			"join_game": func(state *lua.LState) int {
				...
				return accepted(func() error { return controller.JoinGame(reference, password) })
			},
