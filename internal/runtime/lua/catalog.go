...
func DiscoverOwnedDefinitions(ctx context.Context, runtime *Runtime, source fs.FS, owner string) ([]Definition, error) {
	if owner == "" {
	...
	for _, definition := range definitions {
		if !strings.HasPrefix(definition.ID, prefix) {
			return nil, fmt.Errorf("modruntime: package %q does not own component %q", owner, definition.ID)
		}
	}
	return definitions, nil
}
...
func ValidateDefinitionDependencies(definitions []Definition, owner string, dependencies []string) error {
	...
	for _, definition := range definitions {
		for _, dependency := range definition.DependsOn {
			...
			if !owned {
				return fmt.Errorf("modruntime: component %q depends on undeclared package component %q", definition.ID, dependency)
			}
		}
	}
	return nil
...
func ValidateDefinitionDomains(definitions []Definition, clientEntrypoints, authorityEntrypoints []string) error {
	...
	validate := func(root string, forbidden map[string]bool, domain string) error {
		...
		walk = func(id string) error {
			if forbidden[id] {
				return fmt.Errorf("modruntime: %s entrypoint %q depends on opposite-domain entrypoint %q", domain, root, id)
			}
			if visited[id] {
				return nil
			}
