...
// newAssetPreloader constructs asset preloader with its dependencies in one place, keeping ownership and
// cleanup explicit.
func newAssetPreloader(
	assets fs.FS,
	cache *renderAssetCache,
	composer *render.Composer,
) *assetPreloader {
	...
	return &assetPreloader{
		...
		cache:    cache,
		composer: composer,
		jobs:     make(map[uint64]*assetPreloadJob),
		work:     make(chan struct{}, workers),
	}
}

// warm owns the warm step at this boundary, keeping its side effects and failure point explicit to callers.
...
// warmKeyed owns the warm keyed step at this boundary, keeping its side effects and failure point explicit to
// callers.
		...
		go func() {
			for request := range queue {
				...
				if err != nil {
					...
					job.errors = append(
						job.errors,
						fmt.Sprintf("%s %q: %v", request.Kind, request.Path, err),
					)
				}
				p.mu.Unlock()
			}
...
// Pending owns the pending step at this boundary, keeping its side effects and failure point explicit to callers.
...
// Forget owns the forget step at this boundary, keeping its side effects and failure point explicit to callers.
...
func (p *assetPreloader) load(request AssetPreloadRequest) error {
	...
	case "dc6":
		...
		return err
	case "dc6_frame":
		frame, err := p.cache.loadDC6Frame(
			p.assets,
			request.Path,
			request.Palette,
			request.Direction,
		...
		return err
	case "dc6_combined":
		pages, err := p.cache.loadDC6Combined(
			p.assets,
			request.Path,
			request.Palette,
			request.Direction,
		...
		return err
	case "dc6_animation":
		animation, err := p.cache.loadDC6Animation(
			p.assets,
			request.Path,
			request.Palette,
			request.Direction,
		...
		return err
	case "dc6_composite":
		first, err := p.cache.loadDC6Direction(
			p.assets,
			request.Path,
			request.Palette,
			request.Direction,
		...
		second, err := p.cache.loadDC6Direction(
			p.assets,
			request.Secondary,
			request.Palette,
			request.Direction,
		...
		firstAnimation, err := p.cache.loadDC6Animation(
			p.assets,
			request.Path,
			request.Palette,
			request.Direction,
		...
		secondAnimation, err := p.cache.loadDC6Animation(
			p.assets,
			request.Secondary,
			request.Palette,
			request.Direction,
		...
		return err
	case "dcc":
		_, err := p.cache.loadDCCDirection(
			p.assets,
			request.Path,
			request.Palette,
			request.Direction,
	...
	case "cof_animation":
		...
		animation, err := node.cachedCOFAnimation(
			request.Path,
			request.Palette,
			request.Direction,
			request.Components,
	...
	case "world_tile":
		...
		pixels, err := p.cache.loadWorldTileGraphic(
			p.assets,
			request.World,
			request.Palette,
			set.Draws[request.ChunkIndex].Graphic,
		)
		if err == nil {
			p.warm(pixels)
		}
...
// luaPreloadRequests owns the lua preload requests step at this boundary, keeping its side effects and failure
...
func luaPreloadRequests(state *lua.LState, index int) ([]AssetPreloadRequest, error) {
	...
	for item := 1; item <= table.Len(); item++ {
		...
		request := AssetPreloadRequest{
			...
			Secondary: stringField("overlay"),
			Palette:   stringField("palette"),
			Table: stringField(
				"table",
			),
			Sheet:     stringField("sheet"),
			Transform: stringField("transform"),
		}
		...
		} else if isWorldPreloadKind(request.Kind) {
			if request.World == nil {
