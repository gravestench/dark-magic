...
// loadImage loads image before publication so malformed content cannot become observable runtime state.
...
func (r *RenderCapability) Diagnostics() RenderDiagnostics {
	...
	encoded := r.cache.encoded.Diagnostics()
	decoded := r.cache.decoded.Diagnostics()
	composed := r.cache.composed.Diagnostics()
	world := r.cache.world.Diagnostics()

	return RenderDiagnostics{
		Decoded:         combinedCacheStats(encoded, decoded, composed, world),
		Encoded:         encoded,
		Composed:        composed,
		World:           world,
		Retained:        r.composer.Diagnostics(),
		DecodeCalls:     r.cache.decodeCalls,
		DecodeTime:      r.cache.decodeTime,
		PreloadsPending: r.preloads.Pending(),
		Stages:          stages,
	}
}

// combinedCacheStats owns the combined cache stats step at this boundary, keeping its side effects and failure
...
// imageWeight estimates image weight consistently so cache eviction compares resources using the same accounting
// unit.
...
// imagesWeight estimates images weight consistently so cache eviction compares resources using the same accounting
// unit.
...
// blendRGBA performs blend rgba in one rendering boundary so compositing order and pixel ownership remain explicit.
...
//
//nolint:unused // Retain the direct-index compositor for callers restored by renderer build variants.
func drawCompositeComponent(
	...
	if opacity == 255 {
		draw.Draw(
			output,
			component.image.Bounds().Add(destination),
			component.image,
			component.image.Bounds().Min,
			draw.Over,
		)
...
// shadowCanvasBounds owns the shadow canvas bounds step at this boundary, keeping its side effects and failure
// point explicit to callers.
...
// drawCompositeShadow performs composite shadow in one rendering boundary so compositing order and pixel
// ownership remain explicit.
func drawCompositeShadow(
	output *image.RGBA,
	mask *image.RGBA,
	bounds, canvas image.Rectangle,
	opacity uint8,
) {
...
// dccGroundOrigin owns the dcc ground origin step at this boundary, keeping its side effects and failure point
// explicit to callers.
...
// composeCOFFrame owns the compose cofframe step at this boundary, keeping its side effects and failure point
// explicit to callers.
...
// tier owns the tier step at this boundary, keeping its side effects and failure point explicit to callers.
func (c *renderAssetCache) tier(key string) (*cachepkg.Cache, string) {
	...
	case strings.HasPrefix(key, "dcc-file\x00"),
		strings.HasPrefix(key, "dc6-file\x00"),
		strings.HasPrefix(key, "dt1-file\x00"),
		strings.HasPrefix(key, "cof\x00"),
		strings.HasPrefix(key, "animdata\x00"):
		return c.encoded, "encoded"
	case strings.HasPrefix(key, "cof-animation\x00"),
		strings.HasPrefix(key, "dc6-animation\x00"),
		strings.HasPrefix(key, "dc6-combined\x00"),
		strings.HasPrefix(key, "ds1\x00"),
		strings.HasPrefix(key, "ds1-chunks\x00"):
...
// assetWeight estimates asset weight consistently so cache eviction compares resources using the same accounting
// unit.
...
// dccDirectionWeight estimates dcc direction weight consistently so cache eviction compares resources using the
...
// load loads before publication so malformed content cannot become observable runtime state.
func (c *renderAssetCache) load(
	assets fs.FS,
	key string,
	decode func() (any, int, error),
) (any, error) {
	c.mu.Lock()
	c.refresh(assets)
...
// currentGeneration owns the current generation step at this boundary, keeping its side effects and failure point
// explicit to callers.
...
// loadFont loads font before publication so malformed content cannot become observable runtime state.
func (c *renderAssetCache) loadFont(
	assets fs.FS,
	table, sheet, palette, transform string,
) (*assetdecode.BitmapFont, error) {
...
// loadCOF loads cof before publication so malformed content cannot become observable runtime state.
...
// loadAnimationData loads animation data before publication so malformed content cannot become observable
// runtime state.
func (c *renderAssetCache) loadAnimationData(
	assets fs.FS,
	name string,
) (*cof.AnimationData, error) {
...
// loadDCCFile loads dccfile before publication so malformed content cannot become observable runtime state.
func (c *renderAssetCache) loadDCCFile(
	assets fs.FS,
	name, palette string,
) (preparedDCCFile, error) {
	...
	value, err := c.load(assets, "dcc-file\x00"+key, func() (any, int, error) {
		...
		return preparedDCCFile{
				file:    opened,
				palette: colors,
			}, max(
				len(encoded)+len(colors)*4,
				1,
			), nil
	})
	if err != nil {
...
// loadDCCDirection loads dccdirection before publication so malformed content cannot become observable runtime
// state.
func (c *renderAssetCache) loadDCCDirection(
	assets fs.FS,
	name, palette string,
	direction int,
) (preparedDCCDirection, error) {
...
// loadDC6File loads dc6 file before publication so malformed content cannot become observable runtime state.
func (c *renderAssetCache) loadDC6File(
	assets fs.FS,
	name, palette string,
) (preparedDC6File, error) {
	key := name + "\x00" + palette
...
// loadDC6Direction loads dc6 direction before publication so malformed content cannot become observable
// runtime state.
func (c *renderAssetCache) loadDC6Direction(
	assets fs.FS,
	name, palette string,
	direction int,
) (preparedDC6Direction, error) {
	...
	value, err := c.load(assets, key, func() (any, int, error) {
		...
		if direction < 0 || direction >= opened.file.Directions() {
			return nil, 0, fmt.Errorf(
				"dc6: direction %d out of range [0,%d)",
				direction,
				opened.file.Directions(),
			)
		}

		decoded := &dc6.DC6{
			Directions: []*dc6.Direction{
				{Frames: make([]*dc6.Frame, opened.file.FramesPerDirection())},
			},
		}
...
// loadDC6Frame loads dc6 frame before publication so malformed content cannot become observable runtime state.
func (c *renderAssetCache) loadDC6Frame(
	assets fs.FS,
	name, palette string,
	direction, frameIndex int,
) (preparedDC6Frame, error) {
...
// loadDC6Animation loads dc6 animation before publication so malformed content cannot become observable
// runtime state.
func (c *renderAssetCache) loadDC6Animation(
	assets fs.FS,
	name, palette string,
	direction int,
	anchorMode string,
	sharedBounds ...image.Rectangle,
) (preparedDC6Animation, error) {
	boundsKey := ""
	...
	key := fmt.Sprintf(
		"dc6-animation\x00%s\x00%s\x00%d\x00%s\x00%s",
		name,
		palette,
		direction,
		anchorMode,
...
// loadDT1File loads dt1 file before publication so malformed content cannot become observable runtime state.
func (c *renderAssetCache) loadDT1File(
	assets fs.FS,
	name, palette string,
) (preparedDT1File, error) {
	key := name + "\x00" + palette
...
// loadDT1Tile loads dt1 tile before publication so malformed content cannot become observable runtime state.
func (c *renderAssetCache) loadDT1Tile(
	assets fs.FS,
	name, palette string,
	index int,
	view string,
) (preparedDT1Tile, error) {
	...
	value, err := c.load(assets, key, func() (any, int, error) {
		...
		return preparedDT1Tile{
				image: pixels,
				tile:  tile,
				total: opened.file.NumTiles(),
			}, imageWeight(
				pixels,
			), nil
	})
...
// loadDS1 loads ds1 before publication so malformed content cannot become observable runtime state.
func (c *renderAssetCache) loadDS1(
	assets fs.FS,
	name string,
	tiles []string,
	palette string,
...
// loadDS1Chunks loads ds1 chunks before publication so malformed content cannot become observable runtime
// state.
func (c *renderAssetCache) loadDS1Chunks(
	assets fs.FS,
	name string,
	tiles []string,
	palette string,
	...
	key := fmt.Sprintf(
		"ds1-chunks\x00%s\x00%s\x00%s\x00%d",
		name,
		strings.Join(tiles, "\x00"),
		palette,
		chunkSize,
	)
...
// loadWorldChunks loads world chunks before publication so malformed content cannot become observable runtime
// state.
func (c *renderAssetCache) loadWorldChunks(
	assets fs.FS,
	world *gameworld.Map,
	palette string,
	chunkSize int,
...
// loadWorldTiles loads world tiles before publication so malformed content cannot become observable runtime
// state.
func (c *renderAssetCache) loadWorldTiles(
	assets fs.FS,
	world *gameworld.Map,
	palette string,
) (*maprender.TileSet, error) {
	if world == nil {
		return nil, errors.New("world map is required")
	}
...
// loadWorldTileGraphic loads world tile graphic before publication so malformed content cannot become
// observable runtime state.
func (c *renderAssetCache) loadWorldTileGraphic(
	assets fs.FS,
	world *gameworld.Map,
	palette string,
	graphicIndex int,
...
// state.
func (c *renderAssetCache) loadWorldChunk(
	assets fs.FS,
	world *gameworld.Map,
	palette string,
	chunkSize, index int,
) (maprender.Chunk, error) {
...
// pushChunkSet publishes chunk set in one canonical Lua representation, keeping scripts independent of Go
// storage details.
...
// pushTileSet publishes tile set in one canonical Lua representation, keeping scripts independent of Go
// storage details.
...
// runtime state.
func (c *renderAssetCache) loadDS1Collision(
	assets fs.FS,
	name string,
	tiles []string,
) (image.Image, error) {
...
// setPaletteQuantization applies palette quantization through the capability boundary so validation completes
// before shared state changes.
func (n *ownedRenderNode) setPaletteQuantization(palette color.Palette) error {
	...
	previous := n.palette
	if err := n.composer.Update(
		n.id,
		func(current *render.Node) { current.Palette = resource },
	); err != nil {
...
// clearPaletteQuantization owns the clear palette quantization step at this boundary, keeping its side effects and
// failure point explicit to callers.
func (n *ownedRenderNode) clearPaletteQuantization() error {
	previous := n.palette
	if err := n.composer.Update(
		n.id,
		func(current *render.Node) { current.Palette = render.ResourceID{} },
	); err != nil {
		return err
	}
...
// setImage applies image through the capability boundary so validation completes before shared state changes.
func (n *ownedRenderNode) setImage(decoded image.Image) error {
	...
	if err := n.composer.Update(
		n.id,
		func(current *render.Node) { current.Resource = resource },
	); err != nil {
		_ = n.composer.DestroyResource(resource)
		return err
	}
...
// setAnimation applies animation through the capability boundary so validation completes before shared state
// changes.
func (n *ownedRenderNode) setAnimation(
	frames []image.Image,
	duration time.Duration,
	loop string,
	initialSeek ...time.Duration,
) error {
	return n.setAnimationKeyed(frames, nil, duration, loop, initialSeek...)
}

// setAnimationKeyed applies animation keyed through the capability boundary so validation completes before
// shared state changes.
func (n *ownedRenderNode) setAnimationKeyed(
	frames []image.Image,
	textureKeys []string,
	duration time.Duration,
	loop string,
	...
	animation, err := n.composer.CreateResource(
		render.ResourceAnimation,
		render.AnimationData{Frames: textures, Durations: durations, Loop: loop},
	)
	if err != nil {
		cleanup()
		return err
...
// rgbaFrameKey owns the rgba frame key step at this boundary, keeping its side effects and failure point explicit
// to callers.
func rgbaFrameKey(frame image.Image) (rgbaFrameDigest, bool) {
	...
	return rgbaFrameDigest{
		width:  bounds.Dx(),
		height: bounds.Dy(),
		pixels: sha256.Sum256(rgba.Pix[start : start+size]),
	}, true
}

// requireAnimation enforces require animation before execution so undeclared capability access fails
// deterministically.
...
// cofFrames owns the cof frames step at this boundary, keeping its side effects and failure point explicit to
// callers.
func (n *ownedRenderNode) cofFrames(
	cofName, palette string,
	direction int,
	paths map[string]string,
) ([]image.Image, *cof.COF, image.Rectangle, error) {
	...
	if direction < 0 || direction >= asset.NumberOfDirections {
		return nil, nil, image.Rectangle{}, fmt.Errorf(
			"COF direction %d is out of range",
			direction,
		)
	}
	...
	if animationBounds.Empty() {
		return nil, nil, image.Rectangle{}, errors.New(
			"COF composition has no component animation bounds",
		)
	}
	...
	for frameIndex := range frames {
		...
		for componentType, component := range decoded {
			...
			components[componentType] = compositeFrame{
		...
		frames[frameIndex], err = composeCOFFrame(
			asset,
			direction,
			frameIndex,
			components,
...
// compositeCacheKey owns the composite cache key step at this boundary, keeping its side effects and failure point
// explicit to callers.
func compositeCacheKey(cofName, palette string, direction int, paths map[string]string) string {
	...
	return fmt.Sprintf(
		"cof-animation\x00%s\x00%s\x00%d\x00%s",
		cofName,
		palette,
		direction,
...
func (n *ownedRenderNode) cachedCOFAnimation(
	cofName, palette string,
	direction int,
	paths map[string]string,
) (preparedCOFAnimation, error) {
	...
	value, err := n.cache.load(n.assets, key, func() (any, int, error) {
		...
		return preparedCOFAnimation{
				frames: frames,
				asset:  asset,
				origin: image.Pt(-canvas.Min.X, -canvas.Min.Y),
			}, imagesWeight(
				frames,
			), err
	})
...
// luaComponentPaths owns the lua component paths step at this boundary, keeping its side effects and failure point
// explicit to callers.
...
// dc6AnimationBounds owns the dc6 animation bounds step at this boundary, keeping its side effects and failure
// point explicit to callers.
func dc6AnimationBounds(asset *dc6.DC6, direction int) image.Rectangle {
	...
	for index, frame := range asset.Directions[direction].Frames {
		...
		frameBounds := image.Rect(
			int(frame.OffsetX),
			top,
			int(frame.OffsetX+int32(frame.Width)),
			top+int(frame.Height),
		)
		if index == 0 {
			bounds = frameBounds
		} else {
...
// dc6FixedAnimationBounds owns the dc6 fixed animation bounds step at this boundary, keeping its side effects and
// failure point explicit to callers.
...
// normalizedDC6Frames owns the normalized dc6 frames step at this boundary, keeping its side effects and failure
// point explicit to callers.
func normalizedDC6Frames(
	asset *dc6.DC6,
	direction int,
	anchorMode string,
	sharedBounds ...image.Rectangle,
	...
	for index, frame := range frames {
		...
		if anchorMode == "first-frame" && len(sharedBounds) > 0 {
			position = image.Pt(
				int(frames[0].OffsetX)-bounds.Min.X,
				dc6FrameTop(frames[0])-bounds.Min.Y,
			)
...
// horizontalDC6Strip owns the horizontal dc6 strip step at this boundary, keeping its side effects and failure
// point explicit to callers.
...
func (r *RenderCapability) Module() Module {
	...
	return Module{
		Name: "engine.render/v1",
		Help: documentedModule(
			"Create and inspect retained presentation nodes.",
			map[string]CommandHelp{
				"diagnostics": commandHelp(
					"engine.render.diagnostics()",
					"Return decoded-asset cache and retained-renderer diagnostics.",
				),
				"preload": commandHelp(
					"engine.render.preload(requests)",
					"Decode assets asynchronously and return a preload job identifier.",
				),
				"preload_status": commandHelp(
					"engine.render.preload_status(job)",
					"Return progress and errors for a preload job.",
				),
				"preload_release": commandHelp(
					"engine.render.preload_release(job)",
					"Release bookkeeping for a completed preload job.",
				),
				"ds1_dependencies": commandHelp(
					"engine.render.ds1_dependencies(map)",
					"Resolve the mounted DT1 libraries declared by a DS1 stamp.",
				),
				"ds1_chunks": commandHelp(
					"engine.render.ds1_chunks(map, tiles, palette [, chunk_size])",
					"Return sparse DS1 chunk geometry after CPU composition.",
				),
				"world_chunks": commandHelp(
					"engine.render.world_chunks(world, palette [, chunk_size])",
					"Return sparse chunk geometry for an assembled authoritative world map.",
				),
				"world_tiles": commandHelp(
				...
				"cof_info": commandHelp(
					"engine.render.cof_info(path)",
					"Inspect COF layer and animation metadata.",
				),
				"animdata_info": commandHelp(
					"engine.render.animdata_info(key)",
					"Read typed rate and frame-event metadata from an AnimData record source.",
				),
				"create": commandHelp(
					"engine.render.create()",
					"Create a scoped retained presentation node.",
				),
			},
			map[string]TypeHelp{
				renderNodeType: {
					Summary: "A scoped retained presentation node.",
					Methods: map[string]CommandHelp{
						"set_position": commandHelp(
						...
						"set_rotation": commandHelp(
							"node:set_rotation(degrees)",
							"Set the node rotation.",
						),
						"set_tint": commandHelp(
							"node:set_tint(red, green, blue)",
							"Multiply node RGB while preserving full opacity.",
						),
						"set_blend": commandHelp(
							"node:set_blend(mode)",
							"Set the node blend mode.",
						),
						"set_palette_quantization": commandHelp(
							"node:set_palette_quantization(path)",
							"Quantize the node through a display palette.",
						),
						"clear_palette_quantization": commandHelp(
							"node:clear_palette_quantization()",
							"Disable node palette quantization.",
						),
						"set_visible": commandHelp(
							"node:set_visible(visible)",
							"Show or hide the node.",
						),
						"set_origin": commandHelp(
							"node:set_origin(x, y)",
							"Set the normalized texture pivot used at the node position.",
						),
						"set_clip": commandHelp(
							"node:set_clip(x, y, width, height)",
							"Set the node clip rectangle.",
						),
						"clear_clip": commandHelp(
							"node:clear_clip()",
							"Remove the node clip rectangle.",
						),
						"set_image": commandHelp(
							"node:set_image(path)",
							"Render a decoded image asset.",
						),
						"set_ds1": commandHelp(
							"node:set_ds1(map, tiles, palette)",
							"Render a DS1 map using mounted DT1 tiles and a palette.",
						),
						"set_ds1_chunk": commandHelp(
							"node:set_ds1_chunk(map, tiles, palette, chunk_index [, chunk_size])",
							"Render one sparse DS1 map chunk and return its map-space geometry.",
						),
						"set_world_chunk": commandHelp(
							"node:set_world_chunk(world, palette, chunk_index [, chunk_size])",
							"Render one sparse chunk from an assembled authoritative world map.",
						),
						...
						"set_ds1_collision": commandHelp(
							"node:set_ds1_collision(map, tiles)",
							"Render a diagnostic DT1 subtile collision overlay for a DS1 map.",
						),
						"set_dt1": commandHelp(
							"node:set_dt1(path, palette, tile_index[, view])",
							"Render one lazy-decoded DT1 tile and return its dimensions and metadata.",
						),
						"set_dc6": commandHelp(
						...
						"set_dc6_combined": commandHelp(
							...
							"Reconstruct and render one tiled DC6 page.",
						),
						"set_dc6_strip": commandHelp(
							"node:set_dc6_strip(path [, options])",
							"Join every frame in one DC6 direction horizontally.",
						),
						"set_dc6_animation": commandHelp(
							"node:set_dc6_animation(path [, options])",
							"Render a DC6 animation.",
						),
						"set_dcc": commandHelp(
							"node:set_dcc(path [, options])",
							"Render a DCC asset.",
						),
						"set_dcc_animation": commandHelp(
							"node:set_dcc_animation(path [, options])",
							"Render a DCC animation.",
						),
						"set_cof": commandHelp(
							"node:set_cof(path [, options])",
							"Render a COF composite.",
						),
						"set_cof_animation": commandHelp(
							"frames, events, width, height, origin_x, origin_y = "+
								"node:set_cof_animation(path, palette, direction, components "+
								"[, loop, rate, seek_seconds])",
							"Render a cached COF composite and return its shared canvas geometry.",
						),
						"set_text": commandHelp(
							"node:set_text(text [, options])",
							"Render bitmap-font text.",
						),
						"animation_pause": commandHelp(
							"node:animation_pause()",
							"Pause the node animation.",
						),
						"animation_resume": commandHelp(
							"node:animation_resume()",
							"Resume the node animation.",
						),
						"animation_seek": commandHelp(
							"node:animation_seek(frame)",
							"Seek the node animation.",
						),
						"fill_rect": commandHelp(
							"node:fill_rect(width, height, color)",
							"Render a filled rectangle.",
						),
						"destroy": commandHelp(
							"node:destroy()",
							"Destroy and release the node.",
						),
						"exists": commandHelp(
							"node:exists()",
							"Report whether this retained node handle is still live.",
						),
					},
				},
			},
		...
		Loader: func(state *lua.LState) int {
			...
			module := state.SetFuncs(state.NewTable(), map[string]lua.LGFunction{
				...
				"world_chunks": func(state *lua.LState) int {
					...
					chunks, err := cache.loadWorldChunks(
						assets,
						world,
						state.CheckString(2),
						state.OptInt(3, maprender.DefaultChunkSize),
					)
					if err != nil {
				...
				"ds1_chunks": func(state *lua.LState) int {
					...
					chunks, err := cache.loadDS1Chunks(
						assets,
						mapName,
						tiles,
						state.CheckString(3),
						state.OptInt(4, maprender.DefaultChunkSize),
					)
					if err != nil {
				...
				"diagnostics": func(state *lua.LState) int {
					...
					result.RawSetString(
						"pending_warm_bytes",
						lua.LNumber(retained.WarmPendingBytes),
					)
					result.RawSetString("decode_calls", lua.LNumber(decodeCalls))
					result.RawSetString(
						"decode_time_ms",
						lua.LNumber(float64(decodeTime)/float64(time.Millisecond)),
					)
					...
					for name, stage := range stages {
						...
						values.RawSetString(
							"time_ms",
							lua.LNumber(float64(stage.Time)/float64(time.Millisecond)),
						)
				...
				"dc6_animation_bounds": func(state *lua.LState) int {
					...
					prepared, err := cache.loadDC6Direction(
						assets,
						state.CheckString(1),
						state.OptString(2, ""),
						direction,
...
func registerRenderNodeType(state *lua.LState) {
	...
	state.SetField(meta, "__index", state.SetFuncs(state.NewTable(), map[string]lua.LGFunction{
		...
		"set_position": func(state *lua.LState) int {
			...
			if err := node.composer.Update(
				node.id,
				func(current *render.Node) { current.X, current.Y = x, y },
			); err != nil {
		...
		"set_scale": func(state *lua.LState) int {
			...
			if err := node.composer.Update(
				node.id,
				func(current *render.Node) { current.ScaleX, current.ScaleY = x, y },
			); err != nil {
				state.RaiseError("updating render node: %v", err)
			}
		...
		"set_z": func(state *lua.LState) int {
			...
			if err := node.composer.Update(
				node.id,
				func(current *render.Node) { current.Z = z },
			); err != nil {
				state.RaiseError("updating render node: %v", err)
			}
		...
		"set_rotation": func(state *lua.LState) int {
			...
			if err := node.composer.Update(
				node.id,
				func(current *render.Node) { current.Rotation = rotation },
			); err != nil {
				state.RaiseError("updating render node: %v", err)
			}
		...
		"set_blend": func(state *lua.LState) int {
			...
			default:
				state.ArgError(
					2,
					"blend must be alpha, additive, multiply, add-colors, subtract-colors, or screen",
				)
			...
			if err := node.composer.Update(
				node.id,
				func(current *render.Node) { current.Blend = blend },
			); err != nil {
				state.RaiseError("updating render node: %v", err)
			}
		...
		"set_visible": func(state *lua.LState) int {
			...
			if err := node.composer.Update(
				node.id,
				func(current *render.Node) { current.Visible = visible },
			); err != nil {
				state.RaiseError("updating render node: %v", err)
			}
		...
		"set_origin": func(state *lua.LState) int {
			...
			if err := node.composer.Update(
				node.id,
				func(current *render.Node) { current.OriginX, current.OriginY = x, y },
			); err != nil {
		...
		"clear_clip": func(state *lua.LState) int {
			...
			if err := node.composer.Update(
				node.id,
				func(current *render.Node) { current.Clip = nil },
			); err != nil {
		...
		"set_ds1_chunk": func(state *lua.LState) int {
			...
			chunks, err := node.cache.loadDS1Chunks(
				node.assets,
				mapName,
				tiles,
				state.CheckString(4),
		...
		"set_world_chunk": func(state *lua.LState) int {
			...
			if chunkIndex < 0 || chunkIndex >= len(chunks.Chunks) {
				state.RaiseError(
					"world chunk %d out of range [0,%d)",
					chunkIndex,
					len(chunks.Chunks),
				)
			...
			chunk, err := node.cache.loadWorldChunk(
				node.assets,
				world,
				palette,
				chunkSize,
		...
		"set_world_tile": func(state *lua.LState) int {
			...
			if drawIndex < 0 || drawIndex >= len(set.Draws) {
				state.RaiseError(
					"world tile draw %d out of range [0,%d)",
					drawIndex,
					len(set.Draws),
				)
			...
			if draw.Graphic < 0 || draw.Graphic >= len(set.Graphics) {
				state.RaiseError(
					"world tile graphic %d out of range [0,%d)",
					draw.Graphic,
					len(set.Graphics),
				)
			...
			pixels, err := node.cache.loadWorldTileGraphic(
				node.assets,
				world,
				palette,
				draw.Graphic,
		...
		"set_dt1": func(state *lua.LState) int {
			...
			prepared, err := node.cache.loadDT1Tile(
				node.assets,
				state.CheckString(2),
				state.OptString(3, ""),
				state.OptInt(4, 0),
				state.OptString(5, "composite"),
			)
			if err != nil {
		...
		"set_dc6": func(state *lua.LState) int {
			...
			prepared, err := node.cache.loadDC6Frame(
				node.assets,
				fileName,
				paletteName,
				direction,
		...
		"set_dc6_strip": func(state *lua.LState) int {
			...
			prepared, err := node.cache.loadDC6Direction(
				node.assets,
				fileName,
				paletteName,
				direction,
		...
		"set_dc6_animation": func(state *lua.LState) int {
			...
			prepared, err := node.cache.loadDC6Animation(
				node.assets,
				fileName,
				paletteName,
				direction,
			...
			if err := node.setAnimation(
				prepared.frames,
				time.Duration(float64(time.Second)/framesPerSecond),
				loop,
			); err != nil {
		...
		"set_cof": func(state *lua.LState) int {
			...
			frames, asset, canvas, err := node.cofFrames(
				cofName,
				palette,
				direction,
				luaComponentPaths(state, 6),
			)
			if err != nil {
		...
		"set_text": func(state *lua.LState) int {
			...
			rendered, err := font.Render(
				text,
				color.RGBA{R: uint8(red), G: uint8(green), B: uint8(blue), A: uint8(alpha)},
				maxWidth,
				align,
			)
			if err != nil {
		...
		"animation_pause": func(state *lua.LState) int {
			...
			if err := node.composer.Update(
				node.id,
				func(current *render.Node) { current.AnimationPaused = true },
			); err != nil {
				state.RaiseError("pausing animation: %v", err)
			}
		...
		"animation_resume": func(state *lua.LState) int {
			...
			if err := node.composer.Update(
				node.id,
				func(current *render.Node) { current.AnimationPaused = false },
			); err != nil {
				state.RaiseError("resuming animation: %v", err)
			}
...
// checkRenderNode validates check render node at the Lua boundary so invalid values fail before shared state can
// change.
...
// parseLayer parses parse layer at the package boundary so malformed input fails before state publication.
