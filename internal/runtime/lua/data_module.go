...
func readDataJSON(source fs.FS, name string) (any, error) {
	if strings.ToLower(path.Ext(name)) != ".json" {
		return nil, fmt.Errorf("dm.data/v1: only JSON documents are supported")
	}
	data, err := fs.ReadFile(source, name)
	if err != nil {
		return nil, fmt.Errorf("dm.data/v1: read %q: %w", name, err)
	}
	var decoded any
	decoder := json.NewDecoder(strings.NewReader(string(data)))
	decoder.UseNumber()
	if err := decoder.Decode(&decoded); err != nil {
		return nil, fmt.Errorf("dm.data/v1: decode %q: %w", name, err)
	}
	if err := decoder.Decode(new(any)); err != io.EOF {
		return nil, fmt.Errorf("dm.data/v1: decode %q: trailing JSON value", name)
	}
	return decoded, nil
}
...
func validateManifest(decoded any, expectedSchema string) error {
	...
	if !ok {
		return fmt.Errorf("dm.data/v1: manifest root must be an object")
	}
	schema, ok := document["schema"].(string)
	if !ok || schema != expectedSchema {
		return fmt.Errorf("dm.data/v1: manifest schema is %q, want %q", schema, expectedSchema)
	}
	version, ok := document["version"].(json.Number)
	parsedVersion, err := version.Int64()
	if !ok || err != nil || parsedVersion < 1 {
		return fmt.Errorf("dm.data/v1: manifest version must be a positive integer")
	}
	for _, field := range []string{"game_version", "language", "confidence"} {
		value, ok := document[field].(string)
		if !ok || strings.TrimSpace(value) == "" {
			return fmt.Errorf("dm.data/v1: manifest field %q must be a non-empty string", field)
		}
	}
...
func dataToLua(state *lua.LState, input any) (lua.LValue, error) {
	...
	case json.Number:
		...
		if err != nil {
			return nil, fmt.Errorf("dm.data/v1: invalid number %q: %w", value, err)
		}
	...
	case map[string]any:
		...
		return table, nil
	default:
		return nil, fmt.Errorf("dm.data/v1: unsupported decoded value %T", input)
	}
}
