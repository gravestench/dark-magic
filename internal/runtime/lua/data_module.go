...
func DataModule(source fs.FS, transforms ...map[string]ManifestTransform) Module {
	...
	return Module{
		...
		Help: documentedModule(
			"Load structured data and Lua manifests from mounted content.",
			map[string]CommandHelp{
				"load": commandHelp(
					"engine.data.load(path)",
					"Decode a structured data asset into Lua values.",
				),
				"load_manifest": commandHelp(
					"engine.data.load_manifest(path)",
					"Load and validate a Lua-facing content manifest.",
				),
			},
		),
...
// readDataJSON parses read data json at the package boundary so malformed input fails before state publication.
...
// validateManifest validates validate manifest at the Lua boundary so invalid values fail before shared state can
// change.
func validateManifest(decoded any, expectedSchema string) error {
	...
	if !ok || !positiveJSONNumber(resolution["width"]) ||
		!positiveJSONNumber(resolution["height"]) {
		...
		for index, value := range profiles {
			...
			for _, field := range []string{"id", "game_version", "language"} {
				...
				if !ok || strings.TrimSpace(text) == "" {
					return fmt.Errorf(
						"engine.data/v1: supported_profiles[%d].%s must be a non-empty string",
						index,
						field,
					)
				}
			...
			if !ok || !positiveJSONNumber(profileResolution["width"]) ||
				!positiveJSONNumber(profileResolution["height"]) {
				return fmt.Errorf(
					"engine.data/v1: supported_profiles[%d] resolution requires positive width and height",
					index,
				)
			}
			...
			if scope, exists := profile["screens"]; exists {
				...
				if !ok || len(screens) == 0 {
					return fmt.Errorf(
						"engine.data/v1: supported_profiles[%d].screens must be a non-empty array",
						index,
					)
				}
				...
				for _, value := range screens {
					if name, ok := value.(string); !ok || strings.TrimSpace(name) == "" {
						return fmt.Errorf(
							"engine.data/v1: supported_profiles[%d].screens contains an invalid ID",
							index,
						)
					}
				}
			...
			if overrides, exists := profile["overrides"]; exists {
				if _, ok := overrides.(map[string]any); !ok {
					return fmt.Errorf(
						"engine.data/v1: supported_profiles[%d].overrides must be an object",
						index,
					)
				}
			}
...
// positiveJSONNumber owns the positive jsonnumber step at this boundary, keeping its side effects and failure point
// explicit to callers.
...
// pushLuaError publishes lua error in one canonical Lua representation, keeping scripts independent of Go
// storage details.
...
// dataToLua publishes data to lua in one canonical Lua representation, keeping scripts independent of Go storage
// details.
