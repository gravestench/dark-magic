...
func TestShimWorldGameplayUsesLuaDefinedECSSystems(t *testing.T) {
	...
	if err := runtime.Run(context.Background(), func(state *lua.LState) error {
		if state.GetGlobal("hero_x") != lua.LNumber(100) || state.GetGlobal("hero_y") != lua.LNumber(40) || state.GetGlobal("camera_x") != lua.LNumber(100) || state.GetGlobal("camera_y") != lua.LNumber(40) {
