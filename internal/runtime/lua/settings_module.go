...
func SettingsModule(
	...
	mixer *audio.Mixer,
	renderTargets ...RenderSettingsTarget,
) Module {
	...
	return Module{
		...
		Help: documentedModule(
			"Inspect, preview, and persist client game preferences.",
			map[string]CommandHelp{
				"get": commandHelp("engine.settings.get(name)", "Read one active preference."),
				"set": commandHelp(
					"engine.settings.set(name, value)",
					"Validate and apply one preference immediately.",
				),
				"save": commandHelp(
					"engine.settings.save()",
					"Persist active preferences for future launches.",
				),
				"status": commandHelp(
					"engine.settings.status()",
					"Return the persistence path and dirty state.",
				),
			},
		),
