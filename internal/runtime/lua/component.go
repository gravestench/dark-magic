...
func (c *scriptComponent) Start(ctx context.Context) error {
	...
	return c.definition.runtime.runScoped(ctx, c.scope, func(state *lua.LState) error {
		if err := state.CallByParam(
			lua.P{Fn: c.definition.start, NRet: 0, Protect: true},
			c.definition.table,
		); err != nil {
...
func (c *scriptComponent) Stop(ctx context.Context) error {
	...
	if c.definition.stop != nil {
		lifecycleErr = c.definition.runtime.runScoped(ctx, c.scope, func(state *lua.LState) error {
			if err := state.CallByParam(
				lua.P{Fn: c.definition.stop, NRet: 0, Protect: true},
				c.definition.table,
			); err != nil {
...
// ExportState returns the export state observation under the service lock so callers cannot see a partially sampled
// input frame.
func (c *scriptComponent) ExportState(ctx context.Context) (any, error) {
	...
	err := c.definition.runtime.runScoped(ctx, c.scope, func(state *lua.LState) error {
		if err := state.CallByParam(
			lua.P{Fn: c.definition.export, NRet: 1, Protect: true},
			c.definition.table,
		); err != nil {
...
// ImportState returns the import state observation under the service lock so callers cannot see a partially sampled
// input frame.
func (c *scriptComponent) ImportState(ctx context.Context, value any) error {
	...
	return c.definition.runtime.runScoped(ctx, c.scope, func(state *lua.LState) error {
		...
		if err := state.CallByParam(
			lua.P{Fn: c.definition.import_, NRet: 0, Protect: true},
			c.definition.table,
			converted,
		); err != nil {
...
// luaToGo owns the lua to go step at this boundary, keeping its side effects and failure point explicit to callers.
func luaToGo(value lua.LValue, depth int) (any, error) {
	...
	case *lua.LTable:
		...
		value.ForEach(func(key, entry lua.LValue) {
			...
			if number, ok := key.(lua.LNumber); ok && int(number) >= 1 &&
...
// goToLua publishes go to lua in one canonical Lua representation, keeping scripts independent of Go storage
// details.
