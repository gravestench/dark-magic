...
// runScoped executes scoped through the runtime boundary so ownership, error wrapping, and cleanup remain
// consistent.
...
// runScopedWithBudget executes scoped with budget through the runtime boundary so ownership, error wrapping,
// and cleanup remain consistent.
func (r *Runtime) runScopedWithBudget(
	ctx context.Context,
	scope *Scope,
...
// requireActiveScope enforces require active scope before execution so undeclared capability access fails
// deterministically.
...
// runWithBudget executes with budget through the runtime boundary so ownership, error wrapping, and cleanup
// remain consistent.
func (r *Runtime) runWithBudget(
	ctx context.Context,
	budget time.Duration,
	fn func(*lua.LState) error,
) error {
...
// execute executes execute through the runtime boundary so ownership, error wrapping, and cleanup remain
// consistent.
...
// runLoop executes loop through the runtime boundary so ownership, error wrapping, and cleanup remain
// consistent.
func runLoop(
	requests <-chan request,
	done chan<- struct{},
