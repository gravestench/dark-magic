...
// TestDataModuleLoadsJSONAsLuaValues verifies JSON conversion, manifest schema checking, and rejection of non-JSON
// paths through the Lua capability.
func TestDataModuleLoadsJSONAsLuaValues(t *testing.T) {
	runtime := New()

	source := fstest.MapFS{
		"data.json": &fstest.MapFile{
			Data: []byte(`{"version":1,"enabled":true,"names":["one","two"],"nothing":null}`),
		},
		"manifest.json": &fstest.MapFile{
			Data: []byte(
				`{"schema":"test/v1","version":1,"game_version":"test","language":"neutral",` +
					`"confidence":"verified","resolution":{"width":800,"height":600}}`,
			),
		},
	}
...
// TestManifestRejectsInvalidSupportedProfile keeps invalid nested profile resolutions from passing validation.
func TestManifestRejectsInvalidSupportedProfile(t *testing.T) {
	decoded, err := readDataJSON(fstest.MapFS{
		"manifest.json": &fstest.MapFile{Data: []byte(`{
            ...
            "supported_profiles":[{"id":"broken","game_version":"test",` +
			`"language":"English","resolution":{"width":0,"height":600}}]
        }`)},
	}, "manifest.json")
	if err != nil {
		t.Fatal(err)
...
// TestDataModuleAppliesSelectedPresentationProfile verifies that schema-specific transforms select a profile and
// apply its nested overrides before the manifest reaches Lua.
func TestDataModuleAppliesSelectedPresentationProfile(t *testing.T) {
	...
	source := fstest.MapFS{"manifest.json": &fstest.MapFile{Data: []byte(`{
        "schema":"example.presentation/v1","version":1,"game_version":"test",` +
		`"language":"neutral","confidence":"verified",
