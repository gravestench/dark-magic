...
func TestCallTransportsOnlySerializableValues(t *testing.T) {
	...
	if err := runtime.RegisterInstaller(ContentRequire(fstest.MapFS{
		"lua/example.lua": {Data: []byte(`return {choose=function(values) return {id=values[2].id,count=#values} end}`)},
	}, "lua")); err != nil {
		t.Fatal(err)
	}
	...
	value, err := Call(t.Context(), runtime, "example", "choose", []any{map[string]any{"id": 1}, map[string]any{"id": 2}})
	if err != nil {
		t.Fatal(err)
	}
