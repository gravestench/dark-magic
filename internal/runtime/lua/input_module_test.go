...
func TestInputModuleReadsLogicalFrameSnapshot(t *testing.T) {
	...
	input.Publish(inputstate.Frame{Actions: map[string]inputstate.ActionState{"confirm": {Pressed: true}}, Text: "hé", CursorX: 12, CursorY: 34, Owner: inputstate.FocusOwner{Domain: inputstate.FocusScene, ID: "title"}})
	runtime := New()
...
`)}}, "test.lua"); err != nil {
	...
	if err := runtime.Run(context.Background(), func(state *lua.LState) error {
		if state.GetGlobal("confirmed") != lua.LTrue || state.GetGlobal("cursor_x").String() != "12" || state.GetGlobal("cursor_y").String() != "34" || state.GetGlobal("entered").String() != "hé" || state.GetGlobal("owner_domain").String() != "scene" || state.GetGlobal("owner_id").String() != "title" {
