...
func TestAnimDataModuleExposesTypedDeterministicTimeline(t *testing.T) {
	t.Parallel()
	source := fstest.MapFS{"test.lua": {Data: []byte(`local a=require("engine.animdata/v1"); timing=assert(a.record("ama1hth"))`)}}
	runtime := New()
	if err := runtime.RegisterModule(AnimDataModule(animDataFixtureReader(fixtureAnimData("AMA1HTH", 8, 128, map[int]byte{3: 1, 5: 3})))); err != nil {
		t.Fatal(err)
	}
	...
	if err := runtime.Run(context.Background(), func(state *lua.LState) error {
		...
		if timing.RawGetString("name").String() != "AMA1HTH" || timing.RawGetString("complete_delay") != lua.LNumber(16) ||
