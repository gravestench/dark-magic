...
// TestRenderResourcePoolSharesAndReferenceCountsTextures protects the render resource pool shares and reference
// counts textures contract, including its observable ordering and failure behavior.
func TestRenderResourcePoolSharesAndReferenceCountsTextures(t *testing.T) {
	...
	if got := composer.Diagnostics(); got.ActiveResources != 1 ||
		got.RetainedTextureBytes != 16*8*4 {
