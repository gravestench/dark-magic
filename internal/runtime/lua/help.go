...
// cloneModuleHelp defines the clone module help Lua boundary in one place so scripts receive a stable command and
// error contract.
func cloneModuleHelp(help ModuleHelp) ModuleHelp {
	clone := ModuleHelp{
		Summary:  help.Summary,
		Commands: make(map[string]CommandHelp, len(help.Commands)),
		Types:    make(map[string]TypeHelp, len(help.Types)),
	}
...
// documentedModule defines the documented module Lua boundary in one place so scripts receive a stable command and
// error contract.
func documentedModule(
	summary string,
	commands map[string]CommandHelp,
	types ...map[string]TypeHelp,
) ModuleHelp {
...
// commandHelp owns the command help step at this boundary, keeping its side effects and failure point explicit to
// callers.
