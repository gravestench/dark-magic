...
ecs.system{id="observe",phase="movement",query={all={"spawn"}},read={"spawn"},update=function(ctx,entities,commands) observed=#entities end}
`)
	})
	if err != nil {
		t.Fatal(err)
	...
	err = runtime.Run(context.Background(), func(state *lua.LState) error { return state.DoString(`assert(observed==1)`) })
	if err != nil {
		t.Fatal(err)
	}
}
...
`)
	...
	err = runtime.Run(context.Background(), func(state *lua.LState) error { return state.DoString(`assert(monster_count==1)`) })
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestLuaRejectsEntityFromAnotherWorld(t *testing.T) {
	...
	defer first.engine.Close()
	defer second.engine.Close()
