...
func SessionModule(session *gamesession.Session) Module {
	return Module{
		Name: "engine.session/v1",
		Help: documentedModule(
			"Inspect the authoritative game session and export its deterministic replay.",
			map[string]CommandHelp{
				"audit": commandHelp(
					"engine.session.audit()",
					"Return accepted administrator and system commands as a JSON audit record.",
				),
				"status": commandHelp(
					"engine.session.status()",
					"Return the current tick and recorded/pending command and checkpoint counts.",
				),
				"replay": commandHelp(
					"engine.session.replay()",
					"Return the current versioned replay as a JSON string.",
				),
			},
		),
