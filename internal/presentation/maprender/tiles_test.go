...
// TestTileSetVisibleReturnsOnlyIntersectingDraws verifies spatial buckets still apply exact rectangle culling.
...
// TestWallPlacementUsesBlockMinimumInsteadOfHeaderHeight protects legacy wall alignment against a tempting shortcut.
func TestWallPlacementUsesBlockMinimumInsteadOfHeaderHeight(t *testing.T) {
	...
	oldGuess := -absolute(int(reference.Height)) + world.TilePixelHeight
	if oldGuess == referenceYAdjust(reference, world.LayerUpperWall) {
		t.Fatal("wall placement silently regressed to DT1 header-height guess")
	}
}

// TestTileSetVisibleKeepsSharedGraphicIdentity ensures repeated placements never duplicate their decoded source image.
...
// TestTileSetVisibleDeduplicatesDrawSpanningBuckets protects one-draw output when a placement spans multiple cells.
...
// BenchmarkTileSetVisibleSpatialIndex tracks the camera-culling hot path and its reusable scratch allocation behavior.
