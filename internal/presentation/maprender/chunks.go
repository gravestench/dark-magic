...
type chunkIndex map[world.TileLayer]map[int]map[[2]int]*indexedChunk
...
func Index(source fs.FS, mapData *world.Map, palettePath string, chunkSize int) (*Set, error) {
	...
	chunksByLayer := make(chunkIndex)
	// Layer-first indexing preserves the renderer's architectural order before depth and cell sorting refine it.
	for layer := world.LayerFloor; layer <= world.LayerRoof; layer++ {
		...
		for _, placement := range mapData.Tiles {
			...
			indexChunkPlacement(mapData, set, depths, placement, decoded)
		}
	}
		...
		for _, depth := range depths {
			...
			for _, key := range keys {
				...
				set.Chunks = append(set.Chunks, Chunk{
					Column: key[0],
					Row:    key[1],
					X:      indexed.bounds.Min.X,
					Y:      indexed.bounds.Min.Y,
					Width:  indexed.bounds.Dx(),
					Height: indexed.bounds.Dy(),
					Layer:  layer,
					Depth:  depth,
				})
...
// indexChunkPlacement adds one decoded tile to every intersected cell while preserving its full draw destination.
func indexChunkPlacement(
	mapData *world.Map,
	set *Set,
	depths map[int]map[[2]int]*indexedChunk,
	placement world.TilePlacement,
	decoded map[tileSourceKey]*dt1.Tile,
) {
	...
	width, height := int(tile.Width), absolute(int(tile.Height))
	if width <= 0 || height <= 0 {
		return
	}
	...
	destination := image.Rect(0, 0, width, height).Add(image.Pt(
		originX-world.TilePixelWidth/2,
		originY+tileYAdjust(tile, placement.Layer),
	))
	...
	firstColumn, lastColumn := clipped.Min.X/set.ChunkSize, (clipped.Max.X-1)/set.ChunkSize
	firstRow, lastRow := clipped.Min.Y/set.ChunkSize, (clipped.Max.Y-1)/set.ChunkSize

	for row := firstRow; row <= lastRow; row++ {
		for column := firstColumn; column <= lastColumn; column++ {
			cell := image.Rect(
				column*set.ChunkSize,
				row*set.ChunkSize,
				min((column+1)*set.ChunkSize, set.Width),
				min((row+1)*set.ChunkSize, set.Height),
			)
...
func drawIntoTightChunks(
	chunks map[[2]int]*draftChunk,
	chunkSize int,
	width int,
	height int,
	destination image.Rectangle,
...
// loadPalette supports both ordinary palettes and PL2 base palettes while keeping index zero transparent for sprites.
...
// decodeSelectedTiles opens each DT1 once and decodes only referenced records, bounding startup work to map usage.
func decodeSelectedTiles(
	source fs.FS,
	placements []world.TilePlacement,
	palette color.Palette,
) (map[tileSourceKey]*dt1.Tile, error) {
	needed := make(map[string]map[int]struct{})

	for _, placement := range placements {
		if placement.Reference.Path == "" {
			// Empty paths represent semantic-only hidden cells and never own presentation pixels.
			continue
		}
...
// openDT1 retains random-access decoding when available and falls back to buffering generic filesystem handles.
...
// drawIntoChunks clips one source image across fixed cells without allocating cells it never intersects.
func drawIntoChunks(
	chunks map[[2]int]*image.RGBA,
	chunkSize int,
	width int,
	height int,
	destination image.Rectangle,
	...
	for row := firstRow; row <= lastRow; row++ {
		for column := firstColumn; column <= lastColumn; column++ {
			...
			pixels := chunks[key]
			if pixels == nil {
				pixels = image.NewRGBA(image.Rect(
					0,
					0,
					min(chunkSize, width-column*chunkSize),
					min(chunkSize, height-row*chunkSize),
				))
				chunks[key] = pixels
			}
...
// tileYAdjust maps DT1 header geometry into the layer-specific vertical anchor expected by legacy maps.
