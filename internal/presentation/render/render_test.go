...
// TestAllManagedResourceKindsUseCheckedHandles verifies every semantic resource participates in generation checking.
...
// TestCreateTextureUsesSemanticIdentityWithoutPixelHashing protects the asset-pipeline shortcut from implicit scans.
...
// TestManagedResourcePayloadValidation ensures malformed CPU input is rejected before backend ownership begins.
...
// TestPaletteResourcesRemainAliveWhileAttached protects nodes from observing a destroyed auxiliary palette resource.
...
// TestTextureUpdatesRemainCheckedAndOrdered verifies stale handles fail and create precedes the first texture update.
func TestTextureUpdatesRemainCheckedAndOrdered(t *testing.T) {
	...
	got := []string{backend.changes[0].Kind, backend.changes[1].Kind}

	want := []string{"resource-create", "resource-update"}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("changes = %v", got)
	}

	stale := ResourceID{Slot: texture.Slot, Generation: texture.Generation + 1}
	if err := composer.UpdateTexture(stale, replacement); err == nil {
...
// TestNoOpNodeUpdatesAreNotQueued protects the per-frame unchanged-state hot path from unnecessary backend work.
...
// TestStreamingTextureUpdatesKeepOnlyNewestPendingFrame verifies adjacent video frames coalesce before owner upload.
...
// TestDiagnosticsTrackTextureResidencyAndUploadVolume checks counters distinguish retained bytes from upload traffic.
func TestDiagnosticsTrackTextureResidencyAndUploadVolume(t *testing.T) {
	...
	if diagnostics.RetainedTextureBytes != 16 ||
		diagnostics.TextureUploads != 2 ||
		diagnostics.TextureUploadBytes != 64 ||
		diagnostics.ResourceCreates != 1 {
...
// BenchmarkComposerNoOpNodeUpdates tracks the cost of submitting retained state that already matches the composer.
...
// BenchmarkComposerStreamingTextureCoalescing tracks the producer-side cost of replacing pending video frames.
...
// TestAnimationLoopModeValidation ensures invalid playback policy never reaches a backend animation clock.
...
// TestManagedResourcesAreCheckedAndDrainInOwnershipOrder protects create, attach, detach, and destroy sequencing.
...
// Apply records the attempted command before returning its configured failure, matching an owner-thread boundary.
...
// TestComposerQueuesRetainedChangesAndRejectsStaleNodes verifies generation changes invalidate destroyed handles.
func TestComposerQueuesRetainedChangesAndRejectsStaleNodes(t *testing.T) {
	...
	got := []string{backend.changes[0].Kind, backend.changes[1].Kind, backend.changes[2].Kind}

	want := []string{"create", "create", "update"}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("changes = %v", got)
	}
...
// TestComposerOrdersLayersThenZThenCreation protects deterministic snapshot order across architectural layers.
...
// TestComposerExistsRejectsDestroyedGeneration ensures idempotent teardown cannot mistake a reused slot for its prior
// node.
...
// TestComposerRetainsFailedDrainForRetry verifies only successfully applied commands disappear after backend failure.
...
// TestComposerAcceptsConcurrentSubmission exercises slot allocation and retained updates under producer contention.
...
// TestEmptyDrainHotPathDoesNotAllocate protects the normal no-change renderer frame from heap traffic.
...
// TestWarmTextureQueueDeduplicatesAndRespectsBudget verifies identity reuse, progress, and byte-budget deferral.
func TestWarmTextureQueueDeduplicatesAndRespectsBudget(t *testing.T) {
	...
	firstKey := composer.WarmTexture(first)
	if firstKey != composer.WarmTexture(first) {
...
// CanWarmTexture rejects speculative work so the test can verify that refusal retains rather than discards it.
func (*refusingWarmBackend) CanWarmTexture(string, uint64) bool { return false }

// TestWarmTextureQueuePreservesWorkRejectedByResidencyPolicy protects deferred work from admission-policy loss.
func TestWarmTextureQueuePreservesWorkRejectedByResidencyPolicy(t *testing.T) {
	...
	if len(backend.changes) != 0 || composer.Diagnostics().WarmPending != 1 {
		t.Fatalf(
			"rejected warm work was applied or discarded: changes=%d diagnostics=%#v",
			len(backend.changes),
...
// TestStructuralRevisionIgnoresOrdinaryNodeUpdates ensures scene rebuild signals exclude ordinary visual mutations.
