...
// NewAnimationPlayer copies frame durations so caller mutations cannot change playback timing after construction.
...
// Frame reports the current zero-based frame without advancing the owner-driven clock.
func (p *AnimationPlayer) Frame() int { return p.frame }

// SetPaused freezes future advances while preserving the partial duration already accumulated for the current frame.
func (p *AnimationPlayer) SetPaused(paused bool) { p.paused = paused }

// Seek rebuilds playback from the start so loop direction and partial-frame time match ordinary advancement exactly.
...
// Advance consumes positive owner-supplied time and reports whether the visible frame changed during that interval.
...
// advance performs mode-specific transitions after callers establish valid, non-paused input.
func (p *AnimationPlayer) advance(delta time.Duration) (int, bool) {
	previous := p.frame
...
// cycleDuration counts ping-pong endpoints once because reversing direction does not replay either endpoint in place.
