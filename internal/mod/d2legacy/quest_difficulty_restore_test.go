...
// TestDecodedQuestRewardAndDifficultyRestoreIdentically verifies decoded quest
// rewards and progression rules remain deterministic after checkpoint restore.
func TestDecodedQuestRewardAndDifficultyRestoreIdentically(t *testing.T) {
	...
	enter := []byte(`{"character_id":"hero","player":"alice","name":"Hero","class":"Amazon",` +
		`"level":1,"experience":0,"dexterity":20,"vitality":20,"defense":0,"health":50,"max_health":50,` +
		`"mana":20,"max_mana":20,"stamina":84,"max_stamina":84,"expansion":true,"hardcore":false,` +
		`"cof":"","palette":"units",` +
	...
	commands := []simulation.Command{
		{
			...
			Sequence: 1, Kind: "system.player.enter", Payload: enter,
		},
		{
			Tick: 2, Player: "alice", Authority: simulation.AuthorityPlayer,
			Sequence: 1, Kind: "interaction.open", Payload: []byte(`{"target":"npc:akara"}`),
		},
		{
			Tick: 3, Player: "system", Authority: simulation.AuthoritySystem,
			Sequence: 2, Kind: "system.quest.complete",
			Payload: []byte(`{"player":"alice","quest_id":1}`),
		},
	}
	...
	continuation := []simulation.Command{
		{
			Tick: 4, Player: "alice", Authority: simulation.AuthorityPlayer,
			Sequence: 2, Kind: "quest.claim_reward", Payload: []byte(`{}`),
		},
		{
			Tick: 5, Player: "system", Authority: simulation.AuthoritySystem,
			Sequence: 3, Kind: "system.difficulty.advance",
			Payload: []byte(`{"player":"alice","difficulty":1,"completed_act":5}`),
		},
	}
...
// assertQuestRewardAndDifficulty checks the coupled reward and progression
// outcome so a partial application cannot satisfy the restore scenario.
