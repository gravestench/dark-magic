...
// TestD2LegacyBuildsSkillCommandsThroughGenericIntentMailbox protects the
// boundary where game-specific intent becomes an engine-neutral command.
func TestD2LegacyBuildsSkillCommandsThroughGenericIntentMailbox(t *testing.T) {
	...
	if err := runtime.Execute(
		context.Background(),
		content.D2Legacy(),
		"lua/d2legacy/tests/integration/player_intents.lua",
	); err != nil {
		t.Fatal(err)
	}
