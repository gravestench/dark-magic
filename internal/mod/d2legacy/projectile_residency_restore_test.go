...
func TestProjectileResidencyRestoresAcrossInactiveRoom(t *testing.T) {
	...
	if got := componentSnapshot(t, engine, projectile, "d2legacy.world.position"); !reflect.DeepEqual(got, positionBefore) {
		t.Fatalf("inactive projectile moved: got %#v, want %#v", got, positionBefore)
	}
	if got := componentSnapshot(t, engine, projectile, "d2legacy.missile.projectile"); !reflect.DeepEqual(got, projectileBefore) {
		t.Fatalf("inactive projectile lifetime advanced: got %#v, want %#v", got, projectileBefore)
	}
	setProjectileMotion(t, engine, projectile, 0, 0, 1000)
	if err := session.Step(); err != nil {
		t.Fatal(err)
	...
	if restoredCheckpoint.Checksum != originalCheckpoint.Checksum {
		t.Fatalf("inactive projectile restore checksum = %s, want %s", restoredCheckpoint.Checksum, originalCheckpoint.Checksum)
	}
	assertResidentActivation(t, engine, projectile, true, false)
	assertResidentActivation(t, restoredEngine, projectile, true, false)
	if got, want := projectileResidencySnapshot(t, restoredEngine, projectile), projectileResidencySnapshot(t, engine, projectile); !reflect.DeepEqual(got, want) {
		t.Fatalf("restored reactivated projectile = %#v, want %#v", got, want)
	}
}

func waitForProjectile(t *testing.T, session *gamesession.Session, engine *gameecs.Engine, limit int) akara.Entity {
	t.Helper()
...
func setProjectileMotion(t *testing.T, engine *gameecs.Engine, projectile akara.Entity, velocityX, velocityY float64, ticks int64) {
	t.Helper()
...
func projectileResidencySnapshot(t *testing.T, engine *gameecs.Engine, projectile akara.Entity) map[string]map[string]any {
	t.Helper()
