...
// TestCatalogLoadsRecoveredD2LegacyTables verifies all recovered families join
// into one immutable generation with independent caller-owned storage.
func TestCatalogLoadsRecoveredD2LegacyTables(t *testing.T) {
	quests := "id\tname\tact\torder\tvisible\ticon\tquestdone\tqstr\tqsts1\tqsts1a\tqsts1b\n" +
		"0\tPrologue\t0\t\t\t\t\tq0\ts0\t\t\n" +
		"1\tFirst Quest\t0\t1\t1\ta1q1\t0\tq1\ts1\ts1a\ts1b\n"
	source := fstest.MapFS{
		QuestsPath:   &fstest.MapFile{Data: []byte(quests)},
	...
	if quest.PrerequisiteID == nil || *quest.PrerequisiteID != 0 ||
		len(quest.Stages) != 1 || len(quest.Stages[0].Alternates) != 2 {
...
// TestParsersRejectBrokenRelationships ensures invalid cross-row and normalized
// identities fail before indexes can silently overwrite evidence.
...
// TestReferenceValidationReportsSoundAndStringJoins verifies optional mounted
// references produce diagnostics without invalidating recovered rows.
...
// TestRepositoryD2LegacyDataParses locks the checked-in recovered corpus counts,
// catching accidental truncation or parser drift.
func TestRepositoryD2LegacyDataParses(t *testing.T) {
	...
	if len(snapshot.Quests) != 31 || len(snapshot.Speech) != 185 ||
		len(snapshot.DS1Types) != 1091 || len(snapshot.MapObjects) != 750 {
		t.Fatalf(
			"counts = %d quests, %d speech, %d DS1 types, %d map objects",
			len(snapshot.Quests),
			len(snapshot.Speech),
			len(snapshot.DS1Types),
			len(snapshot.MapObjects),
		)
	}
}

// repositoryD2Legacy returns the package's checked-in content root without
// mutating the repository or depending on the process working directory.
