...
// TestProjectWorldViewFiltersOrdersAndBoundsPublicState verifies the public
// allowlist, spatial filter, deterministic ordering, and collection bound.
func TestProjectWorldViewFiltersOrdersAndBoundsPublicState(t *testing.T) {
	engine := gameecs.New()

	t.Cleanup(func() { _ = engine.Close() })
	identity := registerProjectionStore(t, engine, "d2legacy.player.identity", []akara.Field{
	...
	position := registerProjectionStore(t, engine, "d2legacy.world.position", []akara.Field{
		{Name: "x", Kind: akara.FieldFloat64},
		{Name: "y", Kind: akara.FieldFloat64},
	})
	location := registerProjectionStore(t, engine, "d2legacy.world.location", []akara.Field{
		{Name: "act", Kind: akara.FieldInt64},
		{Name: "level_id", Kind: akara.FieldInt64},
	})
	velocity := registerProjectionStore(t, engine, "d2legacy.world.velocity", []akara.Field{
		{Name: "x", Kind: akara.FieldFloat64},
		{Name: "y", Kind: akara.FieldFloat64},
	})
	selectable := registerProjectionStore(t, engine, "d2legacy.world.selectable", []akara.Field{
		{Name: "id", Kind: akara.FieldString},
		{Name: "kind", Kind: akara.FieldString},
		{Name: "label", Kind: akara.FieldString},
		{Name: "owner", Kind: akara.FieldString},
		{Name: "radius", Kind: akara.FieldFloat64},
		{Name: "priority", Kind: akara.FieldInt64},
	})
	inactive := registerProjectionStore(t, engine, "d2legacy.world.inactive", nil)
	monster := registerProjectionStore(t, engine, "d2legacy.monster.stats", []akara.Field{
		{Name: "health", Kind: akara.FieldInt64},
		{Name: "max_health", Kind: akara.FieldInt64},
		{Name: "hidden_damage", Kind: akara.FieldInt64},
	})
	monsterIdentity := registerProjectionStore(t, engine, "d2legacy.monster.identity", []akara.Field{
		{Name: "spawn_id", Kind: akara.FieldString},
		{Name: "definition_id", Kind: akara.FieldString},
	})
	...
	monsterDeath := registerProjectionStore(t, engine, "d2legacy.monster.death", []akara.Field{
		{Name: "tick", Kind: akara.FieldInt64},
		{Name: "drops", Kind: akara.FieldString},
	})
	monsterAI := registerProjectionStore(t, engine, "d2legacy.monster.ai", []akara.Field{
		{Name: "target_id", Kind: akara.FieldString},
		{Name: "state", Kind: akara.FieldString},
	})
	...
	_, _ = identity.Set(player, map[string]any{
		"character_id": "character", "player": "alice", "name": "Alice", "class": "Amazon",
	})
	...
	// setPublic materializes the shared public monster fixture contract.
	setPublic := func(entity akara.Entity, id string, x, y float64) {
		...
		_, _ = selectable.Set(entity, map[string]any{
			"id": id, "kind": "monster", "label": "Fallen", "owner": "",
			"radius": 0.75, "priority": int64(2),
		})
		_, _ = monster.Set(entity, map[string]any{
			"health": int64(8), "max_health": int64(10), "hidden_damage": int64(9999),
		})
	...
	if len(view.Entities) != 2 || view.Entities[0].ID != "monster:a" ||
		view.Entities[0].Health == nil || *view.Entities[0].Health != 8 {
		t.Fatalf("world view = %#v", view)
	}

	living, dead := view.Entities[0], view.Entities[1]
	if living.Token != "FA" || living.Mode != "A1" ||
		living.DeathSound != "fallen_death" || living.OverlayHeight != 3 ||
		dead.ID != "monster:corpse" || dead.Kind != "corpse" ||
		dead.Label != "Fallen" || dead.Mode != "DT" || dead.SpawnID != "corpse" {
	...
	if strings.Contains(string(payload), "9999") ||
		strings.Contains(string(payload), "7777") ||
		strings.Contains(string(payload), "server-only-drop") ||
		strings.Contains(string(payload), "alice-secret") || strings.Contains(string(payload), "attack") ||
		strings.Contains(string(payload), "server-secret-source") ||
		strings.Contains(string(payload), "second-secret-source") ||
		strings.Contains(string(payload), "source_id") ||
...
// TestMonsterPresentationModePublishesOutcomeWithoutAIPolicy documents the
// animation precedence that hides AI decisions from clients.
...
// TestProjectWorldViewRejectsDuplicatePublicIDs ensures client map identities
// remain unique even when ECS entities differ.
func TestProjectWorldViewRejectsDuplicatePublicIDs(t *testing.T) {
	...
	position := registerPositionStore(t, engine)
	location := registerLocationStore(t, engine)
	selectable := registerSelectableStore(t, engine)
	player := engine.World().MustCreateEntity()
	...
	for range 2 {
		...
		_, _ = selectable.Set(entity, map[string]any{
			"id": "duplicate", "kind": "object", "label": "", "owner": "",
			"radius": 1.0, "priority": int64(1),
		})
	}
...
// TestProjectWorldViewPublishesPlayerMovementAnimationInputs verifies remote
// presentation receives motion inputs without receiving movement authority.
func TestProjectWorldViewPublishesPlayerMovementAnimationInputs(t *testing.T) {
	...
	location := registerLocationStore(t, engine)
	selectable := registerSelectableStore(t, engine)
	appearance := registerProjectionStore(t, engine, "d2legacy.player.appearance", []akara.Field{
		{Name: "token", Kind: akara.FieldString},
		{Name: "mode", Kind: akara.FieldString},
	})
	animation := registerProjectionStore(t, engine, "d2legacy.player.animation", []akara.Field{
		{Name: "mode", Kind: akara.FieldString},
		{Name: "start_tick", Kind: akara.FieldInt64},
	})
	facing := registerProjectionStore(t, engine, "d2legacy.world.facing", []akara.Field{
		{Name: "direction", Kind: akara.FieldInt64},
	})
	movement := registerProjectionStore(t, engine, "d2legacy.player.movement_stats", []akara.Field{
		{Name: "velocitypercent", Kind: akara.FieldInt64},
		{Name: "item_fastermovevelocity", Kind: akara.FieldInt64},
	})
	...
	_, _ = selectable.Set(peer, map[string]any{
		"id": "player:peer", "kind": "player", "label": "Peer", "owner": "peer",
		"radius": .75, "priority": int64(10),
	})
	...
	_, _ = movement.Set(peer, map[string]any{
		"velocitypercent": int64(-50), "item_fastermovevelocity": int64(100),
	})
	...
	got := view.Entities[0]
	if got.Class != "Amazon" || got.Token != "AM" || got.Mode != "WL" ||
		got.Direction != 3 || got.AnimationStartTick != 7 ||
		got.VelocityPercent != -50 || got.ItemFasterMoveVelocity != 100 {
		t.Fatalf("player projection = %#v", got)
	}
}

// TestProjectWorldViewExcludesEntitiesInAnotherLevel proves Euclidean proximity
// cannot leak entities across authoritative act or level boundaries.
...
// registerProjectionStore creates a test ECS component and fails immediately if
// fixture schema registration is invalid, keeping later assertions meaningful.
func registerProjectionStore(
	t *testing.T,
...
// registerPositionStore defines the shared spatial fixture schema so tests do
// not hide their scenario under repeated field boilerplate.
func registerPositionStore(t *testing.T, engine *gameecs.Engine) *akara.DynamicStore {
	t.Helper()

	return registerProjectionStore(t, engine, "d2legacy.world.position", []akara.Field{
		{Name: "x", Kind: akara.FieldFloat64},
		{Name: "y", Kind: akara.FieldFloat64},
	})
}

// registerLocationStore defines the authoritative act and level boundary used
// by every world-projection scenario.
func registerLocationStore(t *testing.T, engine *gameecs.Engine) *akara.DynamicStore {
	t.Helper()

	return registerProjectionStore(t, engine, "d2legacy.world.location", []akara.Field{
		{Name: "act", Kind: akara.FieldInt64},
		{Name: "level_id", Kind: akara.FieldInt64},
	})
}

// registerSelectableStore defines the public entity allowlist schema shared by
// world scenarios, keeping contract fields visible in one fixture helper.
func registerSelectableStore(t *testing.T, engine *gameecs.Engine) *akara.DynamicStore {
	t.Helper()

	return registerProjectionStore(t, engine, "d2legacy.world.selectable", []akara.Field{
		{Name: "id", Kind: akara.FieldString},
		{Name: "kind", Kind: akara.FieldString},
		{Name: "label", Kind: akara.FieldString},
		{Name: "owner", Kind: akara.FieldString},
