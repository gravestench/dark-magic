...
func projectWorldMissiles(snapshot gameecs.Snapshot, origin HUDPosition, act, levelID int64) ([]WorldMissile, bool) {
	...
	for _, source := range []struct {
		...
		for _, instance := range component.Instances {
			...
			entry := WorldMissile{
				ID: fmt.Sprintf("missile:%d", instance.Entity), Kind: source.kind,
				MissileID: stringField(fields, "missile_id"), Position: HUDPosition{X: x, Y: y},
				Velocity: HUDPosition{X: floatField(fields, "velocity_x"), Y: floatField(fields, "velocity_y")},
				Act:      entryAct, LevelID: entryLevel, DCC: dcc, Palette: stringField(fields, "palette"),
				LogicalDirection: logicalDirection, Directions: intField(fields, "directions"),
				FramesPerSecond: intField(fields, "frames_per_second"), Loop: boolField(fields, "loop"),
				TransparencyMode: intField(fields, "transparency_mode"),
				OffsetX:          floatField(fields, "offset_x"), OffsetY: floatField(fields, "offset_y"), OffsetZ: floatField(fields, "offset_z"),
