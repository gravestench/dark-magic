...
func TestValidateWorldViewRejectsUntrustedMissileShape(t *testing.T) {
	...
	for name, mutate := range map[string]func(*WorldMissile){
		...
		"projectile pins frame": func(value *WorldMissile) { value.Kind, value.LogicalDirection = "projectile", 0 },
...
func missileCheckpoint() simulation.Checkpoint {
	...
	boolValue := func(value bool) gameecs.ValueSnapshot { return gameecs.ValueSnapshot{Bool: &value} }
	...
	missileFields := []string{"missile_id", "dcc", "palette", "velocity_x", "velocity_y", "directions", "frames_per_second", "loop", "transparency_mode", "offset_x", "offset_y", "offset_z"}
	...
	return simulation.Checkpoint{Tick: 20, Snapshot: &gameecs.Snapshot{
		Version: gameecs.SnapshotVersion, Tick: 20, Entities: []uint64{player, fireball, farEffect, invisible},
		Components: []gameecs.ComponentSnapshot{
			...
			component("d2legacy.world.position", []string{"x", "y"},
				...
				gameecs.InstanceSnapshot{Entity: fireball, Values: []gameecs.ValueSnapshot{floatValue(12), floatValue(10)}},
				gameecs.InstanceSnapshot{Entity: farEffect, Values: []gameecs.ValueSnapshot{floatValue(200), floatValue(10)}},
				gameecs.InstanceSnapshot{Entity: invisible, Values: []gameecs.ValueSnapshot{floatValue(13), floatValue(10)}}),
			component("d2legacy.world.location", []string{"act", "level_id"},
				...
				gameecs.InstanceSnapshot{Entity: fireball, Values: []gameecs.ValueSnapshot{intValue(1), intValue(2)}},
				gameecs.InstanceSnapshot{Entity: farEffect, Values: []gameecs.ValueSnapshot{intValue(1), intValue(2)}},
				gameecs.InstanceSnapshot{Entity: invisible, Values: []gameecs.ValueSnapshot{intValue(1), intValue(2)}}),
			component("d2legacy.missile.projectile", missileFields,
				gameecs.InstanceSnapshot{Entity: fireball, Values: missileValues("fireball", "data/global/missiles/Fireball.dcc", 1)},
				gameecs.InstanceSnapshot{Entity: farEffect, Values: missileValues("far", "data/global/missiles/Far.dcc", 0)},
