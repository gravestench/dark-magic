...
// TestProjectHUDSelectsAuthenticatedPlayerAndAllowlistedFields verifies that
// owner selection and the explicit HUD allowlist share one checkpoint.
func TestProjectHUDSelectsAuthenticatedPlayerAndAllowlistedFields(t *testing.T) {
	snapshot := gameecs.Snapshot{
		...
		Components: []gameecs.ComponentSnapshot{
			...
			intsComponent(
				...
				[]string{"health", "max_health", "mana", "max_mana", "stamina", "max_stamina", "stamina_raw", "max_stamina_raw"},
				1, 25, 30, 12, 15, 42, 84, 42*256+7, 84*256,
			),
			intsComponent(
			...
			intsComponent(
				...
				[]string{"level", "experience", "unspent_skill_points"},
				1, 7, 1234, 2,
			),
	...
	if view.Player.CharacterID != "character:a" || view.Player.Name != "Alice" ||
		view.Vitals.Health != 25 || view.Vitals.StaminaRaw != 42*256+7 ||
		view.Movement.ItemFasterMoveVelocity != 30 || view.Movement.RunDrain != 20 ||
		view.Position.Y != 20.25 {
		t.Fatalf("HUD = %#v", view)
	}
...
// TestProjectHUDRejectsUnknownPlayer ensures authentication failure cannot fall
// through to another player's first component row.
...
// TestProjectPrivateViewSelectsOnlyAuthenticatedOwner proves inventory and
// interaction projections follow the authenticated owner relationship.
func TestProjectPrivateViewSelectsOnlyAuthenticatedOwner(t *testing.T) {
	aliceItems := entityAndStringsComponent(
		"d2legacy.item.identity", 11, 10, map[string]string{"id": "alice-item", "code": "ssd"},
	)
	bobItems := entityAndStringsComponent(
		"d2legacy.item.identity", 21, 20, map[string]string{"id": "bob-secret", "code": "rin"},
	)
	...
	if len(view.Items.Items) != 1 || view.Items.Items[0].ID != "alice-item" ||
		strings.Contains(string(encoded), "bob-secret") {
		t.Fatalf("private projection = %s", encoded)
	}
}

// entityAndStringsComponent builds an owner-linked string component while
// preserving field order expected by positional snapshots.
func entityAndStringsComponent(
	name string,
	entity, owner uint64,
	...
	component := gameecs.ComponentSnapshot{
		Name: name, Version: 1,
		Fields: []gameecs.FieldSnapshot{{Name: "owner", Kind: akara.FieldEntity}},
	}
	...
	for _, key := range keys {
		value := strings[key]
		component.Fields = append(
			component.Fields,
			gameecs.FieldSnapshot{Name: key, Kind: akara.FieldString},
		)
...
// stringsComponent creates deterministic string rows for projection fixtures;
// callers control row order to test first-match behavior.
func stringsComponent(name string, names []string, rows ...[]any) gameecs.ComponentSnapshot {
	...
	for _, name := range names {
		component.Fields = append(
			component.Fields,
			gameecs.FieldSnapshot{Name: name, Kind: akara.FieldString},
		)
	}
...
// intsComponent encodes ordered integer fields exactly as an ECS checkpoint
// would, keeping fixture construction separate from assertions.
func intsComponent(
	name string,
	names []string,
	entity uint64,
	...
	for index, name := range names {
		component.Fields = append(
			component.Fields,
			gameecs.FieldSnapshot{Name: name, Kind: akara.FieldInt64},
		)
		value := values[index]
...
// floatsComponent stores floating-point bit patterns in checkpoint form so
// projection tests exercise the real decoder.
func floatsComponent(
	name string,
	names []string,
	entity uint64,
	values ...float64,
	...
	for index, name := range names {
		component.Fields = append(
			component.Fields,
			gameecs.FieldSnapshot{Name: name, Kind: akara.FieldFloat64},
		)
