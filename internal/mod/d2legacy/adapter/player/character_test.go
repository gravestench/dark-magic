...
// TestProjectCharacterMergesCanonicalFieldsIntoDurableBaseline verifies that
// checkpoint-owned fields change without erasing save-only profile state.
func TestProjectCharacterMergesCanonicalFieldsIntoDurableBaseline(t *testing.T) {
	snapshot := gameecs.Snapshot{
		...
		Components: []gameecs.ComponentSnapshot{
			...
			intsComponent(
				...
				1, 25, 30, 12, 15, 42, 84, 42*256, 84*256,
			),
			intsComponent(
	...
	baseline := d2save.Character{
		ID: "character", Name: "Old", Class: "Amazon", Level: 1,
		Expansion: true, Hardcore: true,
		Appearance: &d2save.Appearance{
			COF: "hero.cof", Components: map[string]string{"HD": "head.dcc"},
		},
	...
	if result.Name != "Canonical Hero" || result.Level != 7 || result.Stats.Experience != 1234 ||
		result.Stats.Health != 25 || result.Stats.Stamina != 42 ||
		result.Stats.MaxStamina != 84 || result.Stats.Defense != 21 {
		t.Fatalf("projected character = %#v", result)
	}

	if !result.Expansion || !result.Hardcore || result.Stats.Strength != 30 ||
		result.Stats.Vitality != 25 || result.Appearance.COF != "hero.cof" {
...
// TestProjectCharacterRejectsMismatchedCharacter ensures one player's
// checkpoint can never be persisted over another durable character.
func TestProjectCharacterRejectsMismatchedCharacter(t *testing.T) {
	...
	_, err := ProjectCharacter(
		"player",
		d2save.Character{ID: "expected"},
		simulation.Checkpoint{Snapshot: &snapshot},
	)
	if err == nil {
