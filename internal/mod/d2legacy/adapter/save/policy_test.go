...
func TestNewCharacterOwnsCanonicalCreationDefaults(t *testing.T) {
	character, err := NewCharacter(CharacterRequest{ID: "id", Name: "  Vale'era  ", Class: "ASSASSIN", Expansion: true, Hardcore: true})
	if err != nil {
		t.Fatal(err)
	}
	if character.Name != "Vale'era" || character.Class != "Assassin" || character.Level != 1 || !character.Expansion || !character.Hardcore {
