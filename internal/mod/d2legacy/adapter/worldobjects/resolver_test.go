...
// TestResolverUsesActLocalOrderingForBothObjectKinds protects static keys and positional dynamic IDs per act.
func TestResolverUsesActLocalOrderingForBothObjectKinds(t *testing.T) {
	rows := fstest.MapFS{
		"data/global/excel/monpreset.txt": &fstest.MapFile{
			Data: []byte("Act\tPlace\n1\tfallen\n2\tskeleton\n1\tzombie\n"),
		},
	}
	recoveredData := recovered.Snapshot{MapObjects: []recovered.MapObject{{
		Act:         1,
		ID:          3,
		ObjectID:    108,
		Description: "Malus",
	...
	resolver, err := New(recoveredData, recordstore.New(rows))
	if err != nil {
		t.Fatal(err)
	}
