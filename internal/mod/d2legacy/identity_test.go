...
// TestIdentityIncludesCanonicalGameplayConfiguration proves configuration map
// ordering does not affect identity while meaningful values still do.
...
// TestIdentityPinsExplicitGameDataGeneration ensures authoritative table
// generations remain independently visible from presentation asset identity.
...
// TestAuthoritativeCapabilityIdentityRejectsCompositionDrift ensures peers
// cannot start with either missing or undeclared deterministic capabilities.
...
// TestIdentitySeparatesPackageAndAuthoritativeLuaHashes verifies the package
// lock and authoritative source digest represent distinct compatibility facts.
...
// TestIdentityCanonicalizesCheckoutLineEndings protects network compatibility
// when equivalent source is checked out with platform-specific line endings.
...
// checkoutWithCRLF copies a source tree while changing only text transport
// line endings, isolating canonicalization from any semantic content change.
...
// TestIdentityForPackagesRejectsBuiltinMetadataDrift proves callers cannot pair
// built-in source with a lock describing different package bytes or metadata.
