...
// TestPartyAuthorityCommandsRestoreAndDepartureCleanup verifies membership,
// checkpoint continuity, and owner departure cleanup as one lifecycle contract.
func TestPartyAuthorityCommandsRestoreAndDepartureCleanup(t *testing.T) {
	...
	additional, err := modruntime.Call(
		ctx,
		authority.Runtime,
		"d2legacy.policy.party",
		"additional_living_members_in_same_level",
		"alice",
	)
	if err != nil {
		t.Fatal(err)
...
// startPartyFixture creates fresh and restored runs through the same authority
// startup path, preventing fixture differences from masking restore defects.
func startPartyFixture(
	t *testing.T,
...
// submitPartyCommand preserves explicit tick, actor, and sequence metadata so
// tests expose ordering as part of the party command contract.
func submitPartyCommand(
	t *testing.T,
...
// stepPartySession advances exactly one tick and reports the current tick on
// failure, keeping multi-command lifecycle failures localizable.
...
// readPartyState converts Lua-owned state into a typed fixture, centralizing the
// module call and decode boundary used by parity assertions.
