...
func TestAnalyzeNormalizesCompleteOwned114dMatrix(t *testing.T) {
	...
	if fireBolt.LifetimeFrames != 30 || fireBolt.ContactFromEffect == nil || *fireBolt.ContactFromEffect != 20 || len(fireBolt.Sounds) != 2 {
		t.Fatalf("normalized fire-bolt hit = %#v", fireBolt)
	}
	for _, sound := range fireBolt.Sounds {
		if sound.Role == "travel" && (!sound.RecordLoop || sound.Intervals[0].FirstFromEffect != 0 || sound.Intervals[0].LastFromRemoval != 0) {
			t.Fatalf("normalized travel sound = %#v", sound)
		}
		if sound.Role == "hit" && (sound.RecordLoop || sound.Intervals[0].FirstFromContact == nil || *sound.Intervals[0].FirstFromContact != 0) {
