...
// all succeed, then becomes an immutable cache blob. Installation preserves a
// different profile-selected version already recorded for the same package ID.
func (store *Store) InstallVerified(ctx context.Context, source io.Reader, expected Descriptor) (Manifest, error) {
	...
	limitedSource := io.LimitReader(
		...
		maxBundledPackageBytes+1,
	)
	written, copyErr := io.Copy(temporary, limitedSource)
	...
	if manifest.ID != expected.ID ||
		...
		manifest.Redistributable != expected.Redistributable {
		return Manifest{}, fmt.Errorf(
			"modcache: downloaded manifest differs from authenticated descriptor for %q",
			expected.ID,
		)
	}

	err = store.withMutationLock(func() error {
		return store.promoteVerifiedPackage(temporaryPath, expected)
	})
	if err != nil {
		return Manifest{}, err
	}
...
// promoteVerifiedPackage moves one authenticated download into immutable
// storage and updates the selected-descriptor catalog while the caller holds
// the mutation lock. Existing exact blobs are reverified rather than overwritten,
// and a different profile-selected version remains selected.
func (store *Store) promoteVerifiedPackage(temporaryPath string, expected Descriptor) error {
...
// quarantinePath identifies the directory whose files are never considered
// installed until an atomic promotion succeeds.
...
// Read checks cancellation before each underlying read so a canceled transfer
// stops before requesting more bytes. It cannot interrupt a source Read already
// in progress, so callers remain responsible for an interruptible source.
