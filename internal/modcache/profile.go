...
// LoadOrCreateProfile strictly reads an existing user selection or creates one
// exclusively from copied defaults. Losing the creation race triggers a read so
// the winning process's explicit choice is never overwritten.
...
// SaveProfile validates and replaces a user-owned extension selection through a
// flushed same-directory temporary file. Product composition uses this for
// explicit schema-preserving migrations, never for temporary command-line
// overrides.
...
// writeJSONExclusive creates and flushes a private JSON file only when no profile
// exists, allowing independent processes to race without replacing user state.
...
// ValidateProfile rejects malformed or duplicate selections before dependency
// resolution, preserving the user's declared activation order.
