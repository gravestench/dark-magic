...
// TestPresenterLetterboxesAndReusesOneTexture verifies fit geometry, stable
// retained ownership across uploads, resize behavior, and terminal cleanup.
func TestPresenterLetterboxesAndReusesOneTexture(t *testing.T) {
	...
	t.Cleanup(func() {
		// Close is idempotent and keeps early assertion failures from obscuring
		// retained-resource ownership in later diagnostics.
		if err := presenter.Close(); err != nil {
			t.Errorf("close presenter: %v", err)
		}
	})
