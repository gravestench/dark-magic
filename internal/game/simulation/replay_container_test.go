...
// replayContainerFixture supplies one complete envelope shared by persistence boundary tests.
...
// TestReplayContainerRoundTripsVersionedEvidence protects the current envelope and integrity contract.
...
// TestReplayContainerRejectsOversizedAndOverpopulatedInput ensures bounds apply before untrusted evidence is used.
...
// TestReplayContainerRequiresExplicitVersionMigration prevents accidental interpretation of unknown schemas.
func TestReplayContainerRequiresExplicitVersionMigration(t *testing.T) {
	...
	_, err = DecodeReplayContainer(bytes.NewReader(encoded), ReplayContainerLimits{})
	if !errors.Is(err, ErrReplayMigration) {
		t.Fatalf("version error = %v, want migration requirement", err)
	}
}

// TestReplayContainerAppliesExplicitSequentialMigration proves each migration yields a current valid envelope.
...
// TestReplayContainerRejectsMigrationThatSkipsVersions keeps every intermediate schema transition observable.
func TestReplayContainerRejectsMigrationThatSkipsVersions(t *testing.T) {
	...
	_, err = DecodeReplayContainerWithMigrations(bytes.NewReader(encoded), ReplayContainerLimits{}, migrations)
	if !errors.Is(err, ErrReplayMigration) || !strings.Contains(err.Error(), "must produce version 1") {
		t.Fatalf("skipped migration error = %v", err)
	}
}

// TestReplayContainerRejectsTamperedEvidence ensures persisted evidence is authenticated before decoding succeeds.
func TestReplayContainerRejectsTamperedEvidence(t *testing.T) {
	...
	_, err := DecodeReplayContainer(bytes.NewReader(tampered), ReplayContainerLimits{})
	if !errors.Is(err, ErrReplayContainer) || !strings.Contains(err.Error(), "integrity mismatch") {
		t.Fatalf("tampered evidence error = %v", err)
	}
}

// TestReplayContainerRejectsUnknownOrTrailingInput keeps the JSON envelope unambiguous and forward-safe.
func TestReplayContainerRejectsUnknownOrTrailingInput(t *testing.T) {
	...
	_, err = DecodeReplayContainer(bytes.NewReader(unknown), ReplayContainerLimits{})
	if !errors.Is(err, ErrReplayContainer) {
		t.Fatalf("unknown field error = %v", err)
	}

	withTrailingData := bytes.NewReader(append(encoded, []byte(`{}`)...))

	_, err = DecodeReplayContainer(withTrailingData, ReplayContainerLimits{})
	if !errors.Is(err, ErrReplayContainer) || !strings.Contains(err.Error(), "trailing") {
