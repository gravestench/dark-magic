...
// Error keeps the checkpoint identity visible while adding the first useful state difference.
func (err *DesyncError) Error() string {
	...
	return fmt.Sprintf(
		"simulation: replay desync at tick %d: %s (expected %s, got %s)",
		err.Tick,
		err.Detail,
		err.Expected,
		err.Actual,
...
func VerifyReplay(replay Replay, prepare ReplayPrepare, apply ReplayApply, participants ...StateParticipant) error {
	...
	for _, checkpoint := range replay.Checkpoints {
		...
		for currentTick < checkpoint.Tick {
			...
			// Commands for a tick must run before its fixed update. Reversing this
			// order would produce a different checkpoint from the recorded session.
		...
		// ECS and participant snapshots are taken at the same completed-tick
		// boundary so the checksum never combines two authoritative moments.
...
// writeFramed prevents distinct component boundaries from producing an identical checksum stream.
...
// indexParticipants validates stable unique identities before any participant is restored.
...
// snapshotParticipants copies and sorts participant state for deterministic replay evidence.
...
// restoreParticipants requires an exact participant topology before applying saved bytes.
...
// cloneParticipantStates prevents diagnostic sorting from mutating replay-owned slices.
...
// firstParticipantDifference reports the first stable-ID difference for actionable desync errors.
...
// equalBytes compares opaque participant state without converting arbitrary bytes to text.
