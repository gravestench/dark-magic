...
// TestReplayRestoresAppliesAndDiagnosesFirstDivergence protects deterministic execution and useful drift evidence.
func TestReplayRestoresAppliesAndDiagnosesFirstDivergence(t *testing.T) {
	engine := gameecs.New()
	defer func() { _ = engine.Close() }()

	schema := akara.Schema{
		Name: "test.counter", Version: 1,
	...
	replay := Replay{
		Version: ReplayVersion, StepNanos: int64(40 * time.Millisecond),
		Initial: initial, Commands: commands, Checkpoints: checkpoints,
	}
	...
	err = VerifyReplay(replay, nil, apply)

	wantDifference := `component "test.counter" entity 1 field "value" differs`
	if !errors.As(err, &desync) || !strings.Contains(desync.Detail, wantDifference) {
