...
// List simulates retail MPQ listfiles that omit tables required by authoritative
// runtime logic, proving Pin supplements discovery rather than trusting the list.
...
// TestPinHashesEffectiveTablesAndFreezesTheirBytes protects the generation
// boundary: authoritative bytes are immutable and presentation bytes are excluded.
...
// TestPinIncludesWinningSourceProvenance ensures identical bytes from different
// winning layers cannot collapse into the same authoritative generation.
...
// TestPinnedStoreRetainsCaseInsensitiveMPQLookups preserves MPQ-style path
// semantics across loading, provenance lookup, and cache invalidation.
...
// TestPinDiscoversRequiredTableMissingFromMPQListfile ensures mandatory tables
// remain pinned even when archive enumeration cannot reveal them.
