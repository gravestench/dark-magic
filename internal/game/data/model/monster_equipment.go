...
type MonsterEquipment struct {
	// Defines the monster that should be equipped. Points to the matching "Id" value in the monstats.txt file.
	Monster string `csv:"monster"`
	// Defines if the monster equipment is added on initialization during the monster's creation.
	OnInit int `csv:"oninit"`
	Level  int `csv:"level"` // Defines the level requirement for the monster to gain this equipment.

	// Item to be equipped on the monster. Uses ID pointer from Weapons.txt, Armor.txt, or Misc.txt.
