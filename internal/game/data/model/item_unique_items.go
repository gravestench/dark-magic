...
type ItemUnique struct {
	...
	// Defines which game version to create this item (<100 = Classic mode | 100 = Expansion mode).
	Version int `csv:"version"`
	// If equals 1, then this item can be rolled as a choice when randomly dropping a unique. If equals 0, then this item
	// cannot be dropped randomly, but can still be dropped explicitly from a treasure class.
	Enabled bool `csv:"enabled"`
	// The first ladder season the unique item can be dropped or created on (inclusive). If blank or 0, then it is
	// available in non-ladder.
	FirstLadderSeason int `csv:"firstLadderSeason"`
	// The last ladder season the unique item is ladder-only (inclusive). Must be used in conjunction with
	// firstLadderSeason.
	LastLadderSeason int `csv:"lastLadderSeason"`
	// Modifies the chances that this Unique item will spawn compared to other Unique items. Each "rarity" value gets
	// summed together to give a total denominator used for the random roll for the item.
	Rarity int `csv:"rarity"`
	// Requires the "quest" field from the misc.txt file to be enabled. If equals 1, then this item can be created and will
	// automatically be identified. If equals 0, then ignore this.
	NoLimit bool `csv:"nolimit"`
	...
	// Defines the baseline item code to use for this Unique item (must match the "code" field value from weapons.txt,
	// armor.txt, or misc.txt).
	Code string `csv:"code"`
	// If equals 1, then players can only carry one of these items in their inventory. If equals 0, then ignore this.
	Carry1 bool `csv:"carry1"`
	// Multiplicative modifier for the Unique item's buy, sell, and repair costs.
	CostMult int `csv:"cost mult"`
	// Flat integer modification to the Unique item's buy, sell, and repair costs. Added after the "cost mult" has modified
	// the costs.
	CostAdd int `csv:"cost add"`
	// Controls the color change of the item when equipped on a character or dropped on the ground. If empty, then the item
	// will have the default item color.
	ChrTransform string `csv:"chrtransform"`
	// Controls the color change of the item in the inventory UI. If empty, then the item will have the default item color.
	InvTransform string `csv:"invtransform"`
	// An override for the "invfile" field from the weapon.txt, armor.txt, or misc.txt files.
	InvFile string `csv:"invfile"`
	// An override for the "flippyfile" field from the weapon.txt, armor.txt, or misc.txt files.
	FlippyFile string `csv:"flippyfile"`
	// An override for the "dropsound" field from the weapon.txt, armor.txt, or misc.txt files.
	DropSound string `csv:"dropsound"`
	// An override for the "dropsfxframe" field from the weapon.txt, armor.txt, or misc.txt files.
	DropSFXFrame int `csv:"dropsfxframe"`
	// An override for the "usesound" field from the weapon.txt, armor.txt, or misc.txt files.
	UseSound string `csv:"usesound"`
	Prop1    string `csv:"prop1"` // Controls the first item property for the Unique item.
	// The stat's "parameter" value associated with the first property (prop1).
	...
	// The stat's "parameter" value associated with the second property (prop2).
	...
	// The stat's "parameter" value associated with the third property (prop3).
	...
	// The stat's "parameter" value associated with the fourth property (prop4).
	...
	// The stat's "parameter" value associated with the fifth property (prop5).
	...
	// The stat's "parameter" value associated with the sixth property (prop6).
	...
	// The stat's "parameter" value associated with the seventh property (prop7).
	...
	// The stat's "parameter" value associated with the eighth property (prop8).
	...
	// The stat's "parameter" value associated with the ninth property (prop9).
	...
	// The stat's "parameter" value associated with the tenth property (prop10).
	Par10  string `csv:"par10"`
	...
	// The stat's "parameter" value associated with the eleventh property (prop11).
	Par11 string `csv:"par11"`
	// The stat's "min" value to assign to the eleventh property (prop11).
	Min11 int `csv:"min11"`
	// The stat's "max" value to assign to the eleventh property (prop11).
	Max11  int    `csv:"max11"`
	Prop12 string `csv:"prop12"` // Controls the twelfth item property for the Unique item.
	// The stat's "parameter" value associated with the twelfth property (prop12).
	Par12 string `csv:"par12"`
	// The stat's "min" value to assign to the twelfth property (prop12).
	Min12 int `csv:"min12"`
	// The stat's "max" value to assign to the twelfth property (prop12).
	Max12 int `csv:"max12"`
	// The amount of weight added to the Diablo Clone progress when this item is sold.
