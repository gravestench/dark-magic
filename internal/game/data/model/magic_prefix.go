...
// MagicPrefix represents the data structure for controlling what item affixes (groups of item modifiers) are applied as
// the prefix for an item. These item affixes will appear at the start of an item’s name. This file is loaded together
// with other similar files in the following order: magicsuffix.txt, magicprefix.txt, automagic.txt. These combined
// files form the Item Mods structure.
type MagicPrefix struct {
	...
	// The minimum character level required for a specific class in order to equip an item that has this item affix. This
	// relies on the class specified in the “class” field. If equals null, then the class will default to using the
	// “levelreq” field.
	...
	// the item more often). This value gets summed together with other “frequency” values from all possible item
	// affixes that can spawn on the item, and then is used as a denominator value for the randomizer. Whichever item affix
	// is randomly selected will be the one to appear on the item. The formula is calculated as the following: [Item Affix
	// Selected] = [“frequency”] / [Total Frequency]. If the item has a magic level (from the “magic lvl” field in
	// weapons.txt/armor.txt/misc.txt) then the magic level value is multiplied with this value. If equals 0, then this
	// item affix will never appear on an item.
	...
	Mod1Param int `csv:"mod1param"`
	// The “min” value to assign to the listed property (mod). Usage depends on the property function (See the
	// “func” field on Properties.txt)
	Mod1Min int `csv:"mod1min"`
	// The “max” value to assign to the listed property (mod). Usage depends on the property function (See the
	// “func” field on Properties.txt)
	Mod1Max int `csv:"mod1max"`
	// Controls the item properties for the item affix (Uses the “code” field from Properties.txt)
	Mod2Code string `csv:"mod2code"`
	// The “parameter” value associated with the listed property (mod). Usage depends on the property function (See the
	// “func” field on Properties.txt)
	Mod2Param int `csv:"mod2param"`
	// The “min” value to assign to the listed property (mod). Usage depends on the property function (See the
	// “func” field on Properties.txt)
	Mod2Min int `csv:"mod2min"`
	// The “max” value to assign to the listed property (mod). Usage depends on the property function (See the
	// “func” field on Properties.txt)
	Mod2Max int `csv:"mod2max"`
	// Controls the item properties for the item affix (Uses the “code” field from Properties.txt)
	Mod3Code string `csv:"mod3code"`
	// The “parameter” value associated with the listed property (mod). Usage depends on the property function (See the
	// “func” field on Properties.txt)
	Mod3Param int `csv:"mod3param"`
	// The “min” value to assign to the listed property (mod). Usage depends on the property function (See the
	// “func” field on Properties.txt)
	Mod3Min int `csv:"mod3min"`
	// The “max” value to assign to the listed property (mod). Usage depends on the property function (See the
	// “func” field on Properties.txt)
	Mod3Max int `csv:"mod3max"`
	...
	// Controls what Item Types are excluded to spawn with this item affix. Uses the “code” field from ItemTypes.txt
	ExcludeType5 string `csv:"etype5"`
	// Controls what Item Types are excluded to spawn with this item affix. Uses the “code” field from ItemTypes.txt
	ExcludeType6 string `csv:"etype6"`
	// Controls what Item Types are excluded to spawn with this item affix. Uses the “code” field from ItemTypes.txt
	ExcludeType7 string `csv:"etype7"`
