...
type ItemArmor struct {
	...
	// Rarity determines the chance that the item will randomly spawn (1/#). The higher the value, the rarer the item will
	// be. This field depends on the "spawnable" field being enabled, the "quest" field being disabled, and the item level
	// being less than or equal to the area level. This value is also affected by the relative Act number that the item is
	// dropping in, where the higher the Act number, the more common the item will drop.
	Rarity int `csv:"rarity"`

	// Spawnable is a boolean field. If equals 1, then this item can be randomly spawned. If equals 0, then this item will
	// never randomly spawn.
	...
	// NoDurability is a boolean field. If equals 1, then the item will not have durability. If equals 0, then the item
	// will have durability.
	NoDurability int `csv:"nodurability"`

	// Level controls the base item level. This is used for determining when the item is allowed to drop, such as making
	// sure that the item level is not greater than the monster's level or the area level.
	Level int `csv:"level"`

	// LevelReq controls the player level requirement for being able to use the item.
	LevelReq int `csv:"levelreq"`

	// Cost defines the base gold cost of the item when being sold by an NPC. This can be affected by item modifiers and
	...
	// MagicLvl defines the magic level of the item, which can affect how magical item modifiers that can appear on the
	// item.
	MagicLvl int `csv:"magic lvl"`

	// AutoPrefix is a boolean field. If equals 1, then the item automatically picks an item affix name from a designated
	// "group" value from the automagic.txt file, instead of using random prefixes. This is only used when the item is of
	// Magical quality.
	AutoPrefix int `csv:"auto prefix"`

	// AlternateGfx uses a unique 3 letter/number code similar to the defined "Code" fields to determine what in-game
	// graphics to display on the player character when the item is equipped.
	...
	// Component determines the layer of player animation when the item is equipped. This uses a code referenced from the
	// Composit.txt file.
	...
	// HasInv is a boolean field. If equals 1, then the item will have its own inventory allowing for the capability to
	// socket gems, runes, or jewels. If equals 0, then the item will not have its own inventory.
	...
	// RArm, LArm, Torso, and Legs fields are used to specify which inventory slot the item should appear on the player
	// model.
	RArm  int `csv:"rArm"`
	LArm  int `csv:"lArm"`
	Torso int `csv:"Torso"`
	...
	// Useable is a boolean field. If equals 1, then the item can be used by right-clicking it. If equals 0, then the item
	// cannot be used.
	Useable int `csv:"useable"`

	// Throwable is a boolean field. If equals 1, then the item can be thrown. If equals 0, then the item cannot be thrown.
	Throwable int `csv:"throwable"`

	// Stackable is a boolean field. If equals 1, then the item can be stacked in the inventory. If equals 0, then the item
	// cannot be stacked.
	...
	// Unique is a boolean field. If equals 1, then the item is unique and can only be found once in the game. If equals 0,
	// then the item is not unique and can be found multiple times.
	Unique int `csv:"unique"`

	// Transparent is a boolean field. If equals 1, then the item has transparency. If equals 0, then the item does not
	// have transparency.
	...
	// Quivered is a boolean field. If equals 1, then the item can be equipped in the quiver slot. If equals 0, then the
	// item cannot be equipped in the quiver slot.
	Quivered int `csv:"quivered"`

	// LightRadius defines the light radius of the item when equipped.
	LightRadius int `csv:"lightradius"`

	// Belt is a boolean field. If equals 1, then the item can be equipped in the belt slot. If equals 0, then the item
	// cannot be equipped in the belt slot.
	Belt int `csv:"belt"`

	// Quest is a boolean field. If equals 1, then the item is a quest item. If equals 0, then the item is not a quest
	// item.
	Quest int `csv:"quest"`
	...
	// DurWarning is a boolean field. If equals 1, then the game will warn the player when the item's durability is low. If
	// equals 0, then there will be no warning for low durability.
	DurWarning int `csv:"durwarning"`

	// QntWarning is a boolean field. If equals 1, then the game will warn the player when the item's quantity is low. If
	// equals 0, then there will be no warning for low quantity.
	...
	// The following fields contain minimum and maximum values for magical modifiers provided by different NPCs in the
	// game.
	...
	// Transform is a boolean field. If equals 1, then the item can transform or "morph" the player character when
	// equipped. If equals 0, then the item does not have transformation capabilities.
	Transform int `csv:"Transform"`

	// InvTrans is a boolean field. If equals 1, then the item can be moved in the inventory while it is still in the
	// process of being identified. If equals 0, then the item cannot be moved until the identification process is
	// completed.
	InvTrans int `csv:"InvTrans"`

	// SkipName is a boolean field. If equals 1, then the item name will not be displayed when the item is dropped. If
	// equals 0, then the item name will be displayed when the item is dropped.
	SkipName int `csv:"SkipName"`

	// NightmareUpgrade is a boolean field. If equals 1, then the item can be upgraded to an exceptional version in
	// Nightmare difficulty. If equals 0, then the item cannot be upgraded in Nightmare difficulty.
	...
	// Nameable is a boolean field. If equals 1, then the item can be named. If equals 0, then the item cannot be named.
