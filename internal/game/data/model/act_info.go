...
type ActInfoData struct {
	...
	// ClassLevelRangeStart uses an area level ("Name field" from levels.txt) with its MonLvl values as a global Act
	// minimum monster level. For example, this is used to determine chest levels in an Act.
	ClassLevelRangeStart string `csv:"classlevelrangestart"`

	// ClassLevelRangeEnd uses an area level ("Name field" from levels.txt) with its MonLvl values as a global Act maximum
	// monster level. For example, this is used to determine chest levels in an Act.
	ClassLevelRangeEnd string `csv:"classlevelrangeend"`

	// WanderingNPCStart uses an index to determine which wandering monster class to use when populating areas. See
	// wanderingmmon.txt for a list of possible monsters to spawn.
	...
	// CommonActCOF specifies which ".D2" file to use for the common Act COF file. This is used to establish the seed when
	// initializing the Act.
	CommonActCOF string `csv:"commonactcof"`

	// Waypoint1 to Waypoint9 uses an area level ("Name field" from levels.txt) as the designated waypoint selection in the
	// Waypoint UI.
	...
	// WanderingMonsterPopulateChance is the percent chance (from 0 to 100) to spawn a wandering monster. See
	// wanderingmmon.txt for a list of possible monsters to spawn.
	...
	// WanderingPopulateRandomChance is a secondary percent chance (from 0 to #) to determine whether to attempt populating
	// with monsters. Only fails if the random chance selects 0.
