...
type TreasureClass struct {
	...
	// Number of item drop chances rolled or total guaranteed quantity of item drops.
	Picks int `lua:"Picks" csv:"Picks"`
	// Modifies the item ratio drop for a Unique Quality item.
	Unique float64 `lua:"Unique" csv:"Unique"`
	// Modifies the item ratio drop for a Set Quality item.
	Set float64 `lua:"Set" csv:"Set"`
	// Modifies the item ratio drop for a Rare Quality item.
	Rare float64 `lua:"Rare" csv:"Rare"`
	// Modifies the item ratio drop for a Magic Quality item.
	Magic float64 `lua:"Magic" csv:"Magic"`
	// Probability of no item dropping by the Treasure Class.
	NoDrop float64 `lua:"NoDrop" csv:"NoDrop"`
...
type TreasureClassEx struct {
	...
	// Last ladder season for which the treasure class is ladder-only (inclusive).
	LastLadderSeason int `csv:"lastLadderSeason"`
	// If true, this treasure class will roll normally when being forced to always drop.
	NoAlwaysDrop bool `csv:"noAlwaysDrop"`
}
