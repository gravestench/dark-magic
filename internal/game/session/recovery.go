...
// RecoveryCheckpointVersion identifies the serialized checksum schema; changing it invalidates persisted recovery data.
...
// recoveryChecksumPayload excludes its own checksum so the digest has one canonical, non-recursive representation.
...
func NewRecoveryCheckpoint(state simulation.Checkpoint, pending, accepted []simulation.Command,
	...
	recovery := RecoveryCheckpoint{
		Version:   RecoveryCheckpointVersion,
		State:     state,
		Pending:   append([]simulation.Command(nil), pending...),
		Accepted:  append([]simulation.Command(nil), accepted...),
		Sequences: make(map[string]SequenceCheckpoint, len(sequences)),
	}
	for player, sequence := range sequences {
		recovery.Sequences[player] = SequenceCheckpoint{
			Ack:    sequence.Ack,
			Beyond: append([]uint64(nil), sequence.Beyond...),
		}
	}
...
func (session *Session) RestoreRecoveryCheckpoint(recovery RecoveryCheckpoint) error {
	...
	session.history = []rollbackFrame{{
		tick:         initial.Tick,
		ecs:          initial,
		participants: cloneParticipantStates(recovery.State.Participants),
	}}
	...
	for player, sequence := range recovery.Sequences {
		progress := sequenceProgress{
			ack:    sequence.Ack,
			beyond: make(map[uint64]struct{}, len(sequence.Beyond)),
		}
		for _, value := range sequence.Beyond {
...
// recoveryChecksum hashes only canonical recovery evidence, allowing corruption checks before runtime restoration.
...
// cloneRecoveryCheckpoint uses the wire representation to deep-copy every nested command and state buffer.
...
// canonicalizeRecoveryCommands makes checksums independent of pending-map and accepted-map iteration order.
...
// validRecoveryCommand enforces the minimum identity needed for duplicate suppression and deterministic replay.
func validRecoveryCommand(command simulation.Command) bool {
	return command.Tick > 0 &&
		command.Sequence > 0 &&
		strings.TrimSpace(command.Player) != "" &&
		strings.TrimSpace(command.Kind) != ""
}

// recoveryCommandKey creates an unambiguous player/sequence map key; NUL cannot collide with decimal sequence text.
func recoveryCommandKey(command simulation.Command) string {
	return fmt.Sprintf("%s\x00%d", command.Player, command.Sequence)
}

// recoverySequenceContains checks contiguous and sparse acknowledgements without expanding them into a large set.
