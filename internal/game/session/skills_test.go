...
func TestSkillSourceAppliesAuthoritativeAssignments(t *testing.T) {
	...
	assignments, err := akara.RegisterSchema(engine.World(), akara.Schema{Name: "d2legacy.player.skill_assignment", Fields: []akara.Field{{Name: "left", Kind: akara.FieldInt64}, {Name: "right", Kind: akara.FieldInt64}}})
	if err != nil {
		t.Fatal(err)
	}
	intents, err := akara.RegisterSchema(engine.World(), akara.Schema{Name: "d2legacy.player.skill_intent", Fields: []akara.Field{{Name: "side", Kind: akara.FieldString}, {Name: "skill_id", Kind: akara.FieldInt64}, {Name: "target_x", Kind: akara.FieldFloat64}, {Name: "target_y", Kind: akara.FieldFloat64}, {Name: "target_id", Kind: akara.FieldString}}})
	if err != nil {
		t.Fatal(err)
	}
	...
	learned, err := akara.RegisterSchema(engine.World(), akara.Schema{Name: "d2legacy.player.learned_skill", Fields: []akara.Field{
