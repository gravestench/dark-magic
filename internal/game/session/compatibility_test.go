...
// compatibilityIdentity builds a valid runtime recipe whose package digest is controlled by the test scenario.
func compatibilityIdentity(packageHash string) simulation.RuntimeIdentity {
	...
	return simulation.RuntimeIdentity{Recipe: simulation.RuntimeRecipe{
		...
		AuthoritativeHash:  packageHash,
		ConfigurationHash:  "config",
		CapabilityVersions: map[string]string{"engine.ecs": "v1"},
	}}
}

// identityState serializes identity through the production participant boundary used by checkpoints and replays.
...
// TestHeadlessRealmAndGameServerPinSessionCompatibility covers every admission and persistence compatibility surface.
...
// TestChangedPackageAppliesOnlyToNewAllocationWithoutExplicitMigration requires explicit persisted-state migration.
...
// TestOrderedExtensionRecipeIsPinnedAcrossEveryCompatibilitySurface proves package order participates in identity.
func TestOrderedExtensionRecipeIsPinnedAcrossEveryCompatibilitySurface(t *testing.T) {
	...
	mismatch.Recipe.Packages.Extensions[0], mismatch.Recipe.Packages.Extensions[1] =
		mismatch.Recipe.Packages.Extensions[1], mismatch.Recipe.Packages.Extensions[0]
...
// runtimePackageDigest creates production-shaped package evidence from readable fixture labels.
