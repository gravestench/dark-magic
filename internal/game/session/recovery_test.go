...
// TestRecoveryCheckpointPreservesPendingAndDuplicateSuppression verifies crash recovery keeps admission semantics.
func TestRecoveryCheckpointPreservesPendingAndDuplicateSuppression(t *testing.T) {
	build := func(identity simulation.RuntimeIdentity) *Session {
		...
		if err := result.Register("move", CommandHandler{
	...
	defer closeTestResource(t, source)

	applied := simulation.Command{
		Tick: 1, Player: "player", Sequence: 1, Kind: "move", Payload: json.RawMessage(`{"x":1}`),
	}
	pending := simulation.Command{
		Tick: 3, Player: "player", Sequence: 2, Kind: "move", Payload: json.RawMessage(`{"x":2}`),
	}
	...
	wrongRecoveryShape := recovery.State.Tick != 1 ||
		len(recovery.Pending) != 1 ||
		len(recovery.Accepted) != 1 ||
		recovery.Sequences["player"].Ack != 1
	if wrongRecoveryShape {
		t.Fatalf("recovery checkpoint = %#v", recovery)
	}

	restored := build(identity)
	defer closeTestResource(t, restored)
	...
	accepted, found := restored.AcceptedNetworkCommand("player", 1)
	if !found || string(accepted.Payload) != string(applied.Payload) {
...
// TestRecoveryCheckpointRejectsMismatchedComposedRuntime prevents state restoration into different rules or packages.
func TestRecoveryCheckpointRejectsMismatchedComposedRuntime(t *testing.T) {
	...
	defer closeTestResource(t, target)
