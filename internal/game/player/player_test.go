...
func TestEntryCommandMaterializesAuthoritativePlayerAtomically(t *testing.T) {
	...
	identity, found := akara.GetDynamicStore(engine.World(), "d2legacy.player.identity")
	...
	position, _ := akara.GetDynamicStore(engine.World(), "d2legacy.world.position")
	transform, _ := position.Get(entity)
	if x, _ := transform.Get("x"); x != float64(5) {
		t.Fatalf("x = %v", x)
	}
	modes, found := akara.GetDynamicStore(engine.World(), "d2legacy.player.movement_mode")
	if !found {
	...
	appearanceStore, found := akara.GetDynamicStore(engine.World(), "d2legacy.player.appearance")
	if !found {
	...
	animationStore, found := akara.GetDynamicStore(engine.World(), "d2legacy.player.animation")
	if !found {
	...
	meleeStore, found := akara.GetDynamicStore(engine.World(), "d2legacy.combat.melee_profile")
	if !found {
	...
	beltStore, found := akara.GetDynamicStore(engine.World(), "d2legacy.player.belt")
	if !found {
	...
	selectableStore, found := akara.GetDynamicStore(engine.World(), "d2legacy.world.selectable")
	if !found {
	...
	colliderStore, found := akara.GetDynamicStore(engine.World(), "d2legacy.world.collider")
	if !found {
	...
	locationStore, found := akara.GetDynamicStore(engine.World(), "d2legacy.world.location")
	if !found {
	...
	assignments, found := akara.GetDynamicStore(engine.World(), "d2legacy.player.skill_assignment")
	if !found {
	...
	learned, found := akara.GetDynamicStore(engine.World(), "d2legacy.player.learned_skill")
...
func TestEntrySourceRecordsServerSelectedTown(t *testing.T) {
	...
	store, found := akara.GetDynamicStore(engine.World(), "d2legacy.world.location")
	if !found {
