...
// openTestMap creates a fully traversable subtile fixture and fails through Must-style test assumptions at call sites.
func openTestMap(width, height int) *Map {
	return &Map{WidthSubtiles: width, HeightSubtiles: height, flags: make([]Flags, width*height)}
}

// TestFindPathRoutesAroundWall verifies A* avoids blockers while every returned cell remains traversable.
...
// TestFindPathDoesNotCutBlockedCorner protects the paired-cardinal check for diagonal movement.
...
// TestFindPathStopsAroundOccupiedTarget verifies StopRadius can accept a reachable interaction cell near a blocker.
...
// TestFindPathRespectsEntityRadius ensures footprint collision prevents wide entities from using point-sized gaps.
...
// TestWalkableStepChecksDestinationAndDiagonalCorners pins the allocation-free local validator used by velocity motion.
