...
// TestSelectorUsesPriorityDistanceAndStableID pins the complete deterministic ordering for overlapping footprints.
...
// TestMapLineClearUsesLOSCollision verifies intervening LOS flags block traces while occupied endpoints remain allowed.
...
// TestMapBarrierClearUsesJumpCollisionIndependentlyFromLOS protects the distinct visual and melee barrier contracts.
...
// TestMapObjectsBecomeStableSelectables ensures authored object order contributes to unique, reproducible pointer IDs.
func TestMapObjectsBecomeStableSelectables(t *testing.T) {
	world := &Map{Objects: []Object{
		{Type: ObjectTypeStatic, ID: 7, X: 10, Y: 20, Resolved: true},
		{Type: ObjectTypeDynamic, ID: 2, X: 4, Y: 5, Resolved: true},
	}}
