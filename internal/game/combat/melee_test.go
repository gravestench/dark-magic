...
func TestPlayerAttackSkillUsesSharedMeleeTransaction(t *testing.T) {
	...
	controls, _ := akara.RegisterSchema(engine.World(), akara.Schema{Name: "d2legacy.world.player_control", Version: 1, Fields: []akara.Field{{Name: "player", Kind: akara.FieldString}}})
...
func meleeTestStores(t *testing.T, engine *gameecs.Engine) meleeStores {
	...
	playerVitals, _ := akara.GetDynamicStore(engine.World(), "d2legacy.player.vitals")
