...
func TestProcessAllocatorStartsControlsAndGracefullyReleasesWorker(t *testing.T) {
	...
	allocation, err := allocator.Allocate(t.Context(), realm.ProcessGameSpec{GameID: "game-1", AllocationID: "allocation-1"})
	if err != nil {
		t.Fatal(err)
	}
	if allocation.GameID != "game-1" || allocation.Endpoint.Address == "" || allocation.Worker == nil || allocation.Tickets == nil {
	...
	ticket, err := allocation.Tickets.Issue(t.Context(), realm.AdmissionPrincipal{AccountID: "account", CharacterID: "character",
		PlayerID: "player", CharacterRevision: 1, RuntimeIdentityHash: description.IdentityHash}, 10*time.Second)
	...
	if assignment.Endpoint != allocation.Endpoint || assignment.Ticket != "helper-ticket" || assignment.CharacterRevision != 1 {
	...
	if _, err := allocator.Allocate(t.Context(), realm.ProcessGameSpec{GameID: "game-1", AllocationID: "allocation-2"}); err == nil {
	...
	restored, err := allocator.Restore(t.Context(), realm.ProcessGameSpec{GameID: "game-restored", AllocationID: "allocation-restored"}, handoff)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := allocator.Allocate(t.Context(), realm.ProcessGameSpec{GameID: "after-close", AllocationID: "allocation-after-close"}); err == nil {
...
func TestProcessAllocatorCleansWorkerThatExitsBeforeReadiness(t *testing.T) {
	...
	if _, err := allocator.Allocate(t.Context(), realm.ProcessGameSpec{GameID: "failed", AllocationID: "allocation-failed"}); err == nil {
...
func TestProcessAllocatorRejectsWorkerWithDifferentAssetSet(t *testing.T) {
	...
	if _, err := allocator.Allocate(t.Context(), realm.ProcessGameSpec{GameID: "wrong-assets", AllocationID: "allocation"}); err == nil {
...
func TestProcessAllocatorHelper(t *testing.T) {
	...
	handler, err := realm.NewWorkerHTTPHandler(worker, processHelperTickets{}, string(token), func() { drain <- struct{}{} })
	if err != nil {
		t.Fatal(err)
	}
	...
	go func() {
		serverDone <- server.Serve(tls.NewListener(listener, &tls.Config{Certificates: []tls.Certificate{certificate}, MinVersion: tls.VersionTLS13}))
	}()
...
func (*processHelperWorker) ProjectCharacter(_ context.Context, _ string, baseline d2save.Character) (d2save.Character, error) {
	return baseline, nil
}
