...
// TestRenderAccountMailIsBoundedToKnownTemplates verifies render account mail is bounded to known templates. The
// scenario keeps the mail contract visible to maintainers.
...
// TestRunMailWorkerCompletesSuccessfulDelivery verifies run mail worker completes successful delivery. The scenario
// keeps the mail contract visible to maintainers.
...
// TestRunMailWorkerRetriesFailedDelivery verifies run mail worker retries failed delivery. The scenario keeps the mail
...
// TestDevelopmentMailRequiresExplicitModeAndLoopback verifies development mail requires explicit mode and loopback.
// The scenario keeps the mail contract visible to maintainers.
...
// TestDevelopmentMailCanLogOrAutoVerify verifies development mail can log or auto verify. The scenario keeps the mail
// contract visible to maintainers.
...
// ClaimMail supplies the test double's claim mail behavior, keeping the scenario deterministic and independent of
// external services.
...
// CompleteMail supplies the test double's complete mail behavior, keeping the scenario deterministic and independent
...
// RetryMail supplies the test double's retry mail behavior, keeping the scenario deterministic and independent of
// external services.
...
// Send supplies the test double's send behavior, keeping the scenario deterministic and independent of external
// services.
...
// Signup supplies the test double's signup behavior, keeping the scenario deterministic and independent of external
// services.
func (*testAccountLifecycle) Signup(context.Context, SignupRequest) (Account, error) {
	return Account{}, nil
}

// VerifyEmail supplies the test double's verify email behavior, keeping the scenario deterministic and independent of
// external services.
...
// BeginPasswordRecovery supplies the test double's begin password recovery behavior, keeping the scenario
...
// CompletePasswordRecovery supplies the test double's complete password recovery behavior, keeping the scenario
// deterministic and independent of external services.
