...
func (channels *Channels) EventsAfter(ctx context.Context, principal AuthenticatedPrincipal, sequence uint64, limit int) ([]ChatEvent, error) {
	if err := contextErr(ctx); err != nil {
		return nil, err
	}
...
func channelView(channel *channelState) ChannelView {
	view := ChannelView{Version: ChannelViewVersion, Revision: channel.revision, ID: channel.id, Name: channel.name, LastEvent: channel.sequence,
...
func validatePresence(presence CharacterPresence) error {
	if strings.TrimSpace(presence.CharacterID) == "" || strings.TrimSpace(presence.Name) == "" || strings.TrimSpace(presence.Class) == "" || presence.Level < 1 {
		return ErrChannelInput
	}
	return nil
}
