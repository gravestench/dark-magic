...
func TestRealmHTTPClientCompletesLobbyFlow(t *testing.T) {
	...
	character, err := client.CreateCharacter(t.Context(), CreateCharacterRequest{Name: "RealmHero", Class: "Amazon", Expansion: true})
	if err != nil {
		t.Fatal(err)
	}
	...
	retired, err := client.CreateCharacter(t.Context(), CreateCharacterRequest{Name: "Retired", Class: "Druid", Expansion: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRealmHTTPRejectsUnknownInput(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodPost, "/v1/accounts", strings.NewReader(`{"name":"Alice","password":"long enough password","admin":true}`))
...
func TestRealmOperatorDrainRequiresIndependentCredential(t *testing.T) {
	...
	if response.Code != http.StatusUnauthorized || allocator.releases != 0 {
		t.Fatalf("player-token drain status=%d releases=%d body=%s", response.Code, allocator.releases, response.Body.String())
	}
	request = httptest.NewRequest(http.MethodPost, "/v1/operator/games/drain", strings.NewReader(payload))
