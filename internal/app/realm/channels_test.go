...
// TestChannelsProjectCharacterPresenceAndPublicMessages verifies channels project character presence and public
// messages. The scenario keeps the channels contract visible to maintainers.
func TestChannelsProjectCharacterPresenceAndPublicMessages(t *testing.T) {
	...
	if view.Version != ChannelViewVersion || view.ID != "diablo ii en-1" || view.Name != "Diablo II En-1" ||
		len(view.Members) != 1 ||
		view.LastEvent != 1 {
	...
	if event.Kind != ChatEventMessage || event.Text != "hello realm" || event.Sender == nil ||
		event.Sender.Character.Name != "Borin" ||
		event.Sequence != 3 {
...
// TestChannelsMoveSessionsAndBoundHistory verifies channels move sessions and bound history. The scenario keeps the
// channels contract visible to maintainers.
func TestChannelsMoveSessionsAndBoundHistory(t *testing.T) {
	...
	if len(first.members) != 0 || len(first.events) != 2 || first.events[0].Text != "two" ||
		first.events[1].Kind != ChatEventLeft {
...
// TestChannelsDefendAgainstDuplicateCharacterPresence verifies channels defend against duplicate character presence.
// The scenario keeps the channels contract visible to maintainers.
...
// TestChannelsPruneOnlyPresenceThatStoppedRenewing verifies channels prune only presence that stopped renewing. The
// scenario keeps the channels contract visible to maintainers.
