...
// Admit emits the canonical durable membership representation so persisted and transported values retain one stable
// shape.
...
// Cancel executes cancel through the PostgreSQL membership store so SQL encoding and database error translation remain
// centralized.
...
// Depart emits the canonical durable membership representation so persisted and transported values retain one stable
// shape.
func (store *PostgresMemberships) Depart(
	ctx context.Context,
...
// MarkWorkerRemoved emits the canonical durable membership representation so persisted and transported values retain
// one stable shape.
func (store *PostgresMemberships) MarkWorkerRemoved(
	ctx context.Context,
...
// ByAccount executes by account through the PostgreSQL membership store so SQL encoding and database error translation
// remain centralized.
...
// ByCharacter executes by character through the PostgreSQL membership store so SQL encoding and database error
// translation remain centralized.
func (store *PostgresMemberships) ByCharacter(
	ctx context.Context,
...
// ByPlayer executes by player through the PostgreSQL membership store so SQL encoding and database error translation
// remain centralized.
...
// ActivePlayerIDs executes active player ids through the PostgreSQL membership store so SQL encoding and database
...
// DrainPlayerIDs executes drain player ids through the PostgreSQL membership store so SQL encoding and database error
// translation remain centralized.
...
// playerIDs executes player ids through the PostgreSQL membership store so SQL encoding and database error translation
// remain centralized.
func (store *PostgresMemberships) playerIDs(
	ctx context.Context,
	gameID string,
	includeDeparted bool,
) ([]string, error) {
...
func (store *PostgresMemberships) ResumeGame(
	ctx context.Context,
	gameID string,
	...
	err := pgx.BeginFunc(ctx, store.pool, func(tx pgx.Tx) error {
		locked, err := lockResumableMemberships(ctx, tx, gameID)
		if err != nil {
			return err
		}

		for _, candidate := range locked {
			record, err := rotateResumedMembershipLease(ctx, tx, candidate, gameID, lifetime)
			if err != nil {
				return err
			}
...
type resumableMembership struct {
	record      MembershipRecord
	characterID string
}

const (
	resumableMembershipSelect = "SELECT m.game_id, m.player_id, m.account_id, m.baseline, m.character_id\n" +
		"\t\t\tFROM realm_memberships m\n" +
		"\t\t\tWHERE m.game_id = $1 AND m.state = $2\n" +
		"\t\t\tORDER BY m.player_id\n" +
		"\t\t\tFOR UPDATE OF m"
	resumableLeaseSelect = "SELECT revision, game_id FROM realm_character_leases\n" +
		"\t\t\t\tWHERE character_id = $1 FOR UPDATE"
	resumableLeaseUpdate = "UPDATE realm_character_leases\n" +
		"\t\t\t\tSET token_digest = $1, expires_at = $2\n" +
		"\t\t\t\tWHERE character_id = $3 AND revision = $4 AND game_id = $5"
)

// lockResumableMemberships locks active membership rows in player order and verifies each stored baseline still names
// the indexed character. Closing the rows before lease queries avoids overlapping PostgreSQL result streams.
func lockResumableMemberships(
	ctx context.Context,
	tx pgx.Tx,
	gameID string,
) ([]resumableMembership, error) {
	rows, err := tx.Query(ctx, resumableMembershipSelect, gameID, MembershipActive)
	if err != nil {
		return nil, err
	}
	defer rows.Close()

	locked := make([]resumableMembership, 0)
	...
	for rows.Next() {
		...
		if err := json.Unmarshal(baseline, &record.Baseline); err != nil ||
			record.Baseline.Character.ID != characterID {
			return nil, ErrMembership
		}

		locked = append(locked, resumableMembership{record: record, characterID: characterID})
	}
...
// rotateResumedMembershipLease verifies the locked lease generation before replacing its raw token. Only the digest
// is persisted, preserving the rule that restart credentials exist solely in process memory.
func rotateResumedMembershipLease(
	ctx context.Context,
	tx pgx.Tx,
	candidate resumableMembership,
	gameID string,
	lifetime time.Duration,
) (MembershipRecord, error) {
	var (
	...
	err := tx.QueryRow(ctx, resumableLeaseSelect, candidate.characterID).Scan(&revision, &leaseGameID)
	if errors.Is(err, pgx.ErrNoRows) {
		return MembershipRecord{}, ErrLease
	}
	...
	command, err := tx.Exec(ctx, resumableLeaseUpdate,
	...
	candidate.record.Lease = lease
	candidate.record.State = MembershipActive

	return cloneMembershipRecord(candidate.record), nil
}

// get executes get through the PostgreSQL membership store so SQL encoding and database error translation remain
// centralized.
func (store *PostgresMemberships) get(ctx context.Context, predicate string, values ...any) (MembershipRecord, error) {
	...
	record, err := scanPostgresMembership(
		store.pool.QueryRow(ctx, postgresMembershipSelect+` WHERE `+predicate, values...),
	)
	if errors.Is(err, pgx.ErrNoRows) {
		return MembershipRecord{}, ErrMembership
	}
...
// AbandonGame executes abandon game through the PostgreSQL membership store so SQL encoding and database error
// translation remain centralized.
...
// scanPostgresMembership decodes the durable membership representation at one boundary so malformed data fails before
// it becomes shared state.
func scanPostgresMembership(row postgresRowScanner) (MembershipRecord, error) {
	var (
		record            MembershipRecord
		baseline, receipt []byte
	)
	if err := row.Scan(
		&record.GameID,
		&record.PlayerID,
