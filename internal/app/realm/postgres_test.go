...
// TestPostgresRepositoriesPreserveAccountCharacterAndLeaseContracts verifies postgres repositories preserve account
// character and lease contracts. The scenario keeps the durable postgres contract visible to maintainers.
func TestPostgresRepositoriesPreserveAccountCharacterAndLeaseContracts(t *testing.T) {
	...
	if _, err := store.Accounts.Authenticate(
		t.Context(),
		accountName,
		"wrong password",
	); !errors.Is(
		err,
		ErrAccountCredentials,
	) {
	...
	if err := store.Pool.QueryRow(t.Context(), `SELECT token_digest FROM realm_sessions WHERE id = $1`, session.ID).
	...
	created, err := control.CreateCharacter(
		t.Context(),
		session.Token,
		CreateCharacterRequest{Name: "Heroine", Class: "Amazon"},
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := store.Accounts.SelectCharacter(
		t.Context(),
		muleSession.Token,
	...
	if _, _, err := store.Characters.Acquire(
		t.Context(),
		account.ID,
		created.Character.ID,
		"game-2",
...
// TestPostgresGameDirectoryPersistsAndSerializesCapacity verifies postgres game directory persists and serializes
// capacity. The scenario keeps the durable postgres contract visible to maintainers.
func TestPostgresGameDirectoryPersistsAndSerializesCapacity(t *testing.T) {
	...
	if repeated, err := store.Allocations.Request(
		t.Context(),
		created.Entry.GameID,
		allocationID,
	); err != nil ||
		repeated.AllocationID != allocationID {
		store.Close()
		t.Fatalf("repeated PostgreSQL allocation request = %#v, %v", repeated, err)
	}

	if _, err := store.Allocations.Request(
		t.Context(),
		created.Entry.GameID,
		allocationID+"-other",
	); !errors.Is(
	...
	if _, err := store.Allocations.Ready(
		t.Context(),
		created.Entry.GameID,
		endpoint,
	...
	if repeated, err := store.Allocations.Ready(
		t.Context(),
		created.Entry.GameID,
		endpoint,
		orchestrationIdentity(),
	); err != nil ||
		repeated.State != AllocationReady {
		store.Close()
	...
	if err := restarted.Pool.QueryRow(
		t.Context(),
		`SELECT state, COALESCE(last_error, '') FROM realm_allocations WHERE game_id = $1`,
		created.Entry.GameID,
	).
		Scan(&allocationState, &allocationError); err != nil ||
		allocationState != string(AllocationFailed) ||
		allocationError == "" {
...
// TestPostgresGameDirectoryPersistsDrainingAdmissionBarrier verifies postgres game directory persists draining
// admission barrier. The scenario keeps the durable postgres contract visible to maintainers.
func TestPostgresGameDirectoryPersistsDrainingAdmissionBarrier(t *testing.T) {
	...
	control, err := NewControlPlane(
		ControlPlaneConfig{Accounts: store.Accounts, Characters: store.Characters, Games: store.Games},
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	reservation, err := store.Games.ReservePlayer(
		t.Context(),
		game.Entry.GameID,
		GamePlayer{
	...
	var state string
	if err := store.Pool.QueryRow(t.Context(), `SELECT state FROM realm_games WHERE id = $1`, game.Entry.GameID).
		Scan(&state); err != nil ||
		state != drainingRealmGameState {
...
// TestPostgresMembershipDepartureIsAtomicRetryableAndDurable verifies postgres membership departure is atomic
// retryable and durable. The scenario keeps the durable postgres contract visible to maintainers.
func TestPostgresMembershipDepartureIsAtomicRetryableAndDurable(t *testing.T) {
	...
	if repeated, err := restarted.Memberships.Depart(
		t.Context(),
		membership,
		canonical,
	); err != nil ||
		repeated.Record.Revision != 2 {
	...
	if repeated, err := restarted.Memberships.MarkWorkerRemoved(
		t.Context(),
		game.Entry.GameID,
		membership.PlayerID,
	); err != nil ||
		!repeated.WorkerRemoved {
...
// TestPostgresMembershipsAllowDifferentCharactersFromSameAccount verifies postgres memberships allow different
// characters from same account. The scenario keeps the durable postgres contract visible to maintainers.
...
// TestPostgresMembershipResumeRotatesLostLeaseTokenAtomically verifies postgres membership resume rotates lost lease
// token atomically. The scenario keeps the durable postgres contract visible to maintainers.
func TestPostgresMembershipResumeRotatesLostLeaseTokenAtomically(t *testing.T) {
	...
	if err := restarted.Pool.QueryRow(
		t.Context(),
		`SELECT token_digest FROM realm_character_leases WHERE character_id = $1`,
...
// TestPostgresVerifiedAccountRecoveryAndTransactionalMail verifies postgres verified account recovery and
// transactional mail. The scenario keeps the durable postgres contract visible to maintainers.
func TestPostgresVerifiedAccountRecoveryAndTransactionalMail(t *testing.T) {
	...
	if err := store.Accounts.CompletePasswordRecovery(
		t.Context(),
		recoveryToken,
		newPassword,
	); !errors.Is(
...
// accountMailToken supplies the test double's account mail token behavior, keeping the scenario deterministic and
...
// TestPostgresControlPlanePersistsCanonicalLeaveAndSessionAcrossRestart verifies postgres control plane persists
// canonical leave and session across restart. The scenario keeps the durable postgres contract visible to maintainers.
func TestPostgresControlPlanePersistsCanonicalLeaveAndSessionAcrossRestart(t *testing.T) {
	...
	if err := store.Pool.QueryRow(t.Context(), `SELECT event->>'operation', event->>'character_id'
		...
		Scan(&auditOperation, &auditCharacterID); err != nil ||
		auditOperation != AuditGameLeave ||
		auditCharacterID != created.Character.ID {
		store.Close()
...
// TestPostgresGameCheckpointSurvivesRestartRejectsRegressionAndDetectsTampering verifies postgres game checkpoint
// survives restart rejects regression and detects tampering. The scenario keeps the durable postgres contract visible
// to maintainers.
