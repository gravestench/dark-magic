...
func TestProcessAllocatorStartsRealPreparedRealmWorker(t *testing.T) {
	...
	allocation, err := allocator.Allocate(t.Context(), realm.GameSpec{GameID: "real-worker-acceptance", AllocationID: "real-worker-allocation"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestProcessAllocatorRealWorkerRestoresLiveClientSession(t *testing.T) {
	...
	control, err := realm.NewControlPlane(realm.ControlPlaneConfig{Allocator: allocator, CheckpointInterval: time.Nanosecond})
	if err != nil {
		t.Fatal(err)
	}
...
func TestProcessAllocatorRealWorkerCommitsExpiredTransportMembership(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		...
		if err == nil && committed.Revision == createdCharacter.Revision+1 {
			if games, listErr := control.ListGames(t.Context(), session.Token, realm.GameFilter{}); listErr != nil || len(games) != 0 {
