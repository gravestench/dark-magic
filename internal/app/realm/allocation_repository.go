...
// RestoreReady coordinates restore ready through the owning allocation repository synchronization boundary so shared
// state is published only after a complete transition.
func (store *MemoryAllocations) RestoreReady(
	ctx context.Context,
	...
	now := store.now().UTC()
	record.Endpoint, record.Runtime, record.LastHealthyAt, record.UpdatedAt = endpoint, cloneRuntimeIdentity(
		runtime,
	), &now, now
	record.LastError = ""
...
// NewMemoryAllocations constructs the allocation repository boundary and validates dependencies before callers can
...
// Request coordinates request through the owning allocation repository synchronization boundary so shared state is
...
// Ready decodes the allocation repository representation at one boundary so malformed data fails before it becomes
// shared state.
func (store *MemoryAllocations) Ready(
	ctx context.Context,
	gameID string,
	endpoint GameEndpoint,
	runtime simulation.RuntimeIdentity,
...
// Healthy contains healthy within the allocation repository boundary so callers do not duplicate its domain-specific
// policy.
...
// Fail contains fail within the allocation repository boundary so callers do not duplicate its domain-specific policy.
...
// Complete contains complete within the allocation repository boundary so callers do not duplicate its domain-specific
// policy.
...
// transition coordinates transition through the owning allocation repository synchronization boundary so shared state
// is published only after a complete transition.
func (store *MemoryAllocations) transition(
	ctx context.Context,
	gameID string,
	state AllocationState,
	message string,
...
// Active coordinates active through the owning allocation repository synchronization boundary so shared state is
// published only after a complete transition.
...
// Get coordinates get through the owning allocation repository synchronization boundary so shared state is published
...
// validGameEndpoint checks the allocation repository invariant before state changes, keeping invalid values off shared
// paths.
...
// allocationReadyMatches contains allocation ready matches within the allocation repository boundary so callers do not
// duplicate its domain-specific policy.
...
// cloneAllocationRecord returns an independent allocation repository value so callers cannot mutate repository-owned
