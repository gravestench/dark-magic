...
// Save executes save through the PostgreSQL checkpoint store so SQL encoding and database error translation remain
// centralized.
func (store *PostgresCheckpoints) Save(ctx context.Context, record GameCheckpoint) (GameCheckpoint, error) {
	...
	err = pgx.BeginFunc(ctx, store.pool, func(tx pgx.Tx) error {
		...
		var (
			existingAllocationID, existingChecksum string
			existingTick                           uint64
		)
...
// Latest executes latest through the PostgreSQL checkpoint store so SQL encoding and database error translation remain
// centralized.
...
// Remove executes remove through the PostgreSQL checkpoint store so SQL encoding and database error translation remain
// centralized.
...
// equalBytes executes equal bytes through the PostgreSQL checkpoint store so SQL encoding and database error
