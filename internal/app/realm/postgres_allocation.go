...
func (store *PostgresAllocations) Ready(ctx context.Context, gameID string, endpoint GameEndpoint, runtime simulation.RuntimeIdentity) (AllocationRecord, error) {
...
func (store *PostgresAllocations) RestoreReady(ctx context.Context, gameID, allocationID string, endpoint GameEndpoint, runtime simulation.RuntimeIdentity) (AllocationRecord, error) {
	if store == nil || store.pool == nil || !validGameEndpoint(endpoint) {
		return AllocationRecord{}, ErrAllocationRecord
	}
	if _, err := runtime.Digest(); err != nil {
...
func (store *PostgresAllocations) restoreReadyTransaction(ctx context.Context, gameID, allocationID string, endpoint GameEndpoint, runtime simulation.RuntimeIdentity) (AllocationRecord, error) {
...
func (store *PostgresAllocations) terminal(ctx context.Context, gameID string, state AllocationState, message string) error {
	if store == nil || store.pool == nil {
		return ErrAllocationRecord
	}
...
func scanPostgresAllocation(row postgresRowScanner) (AllocationRecord, error) {
	...
	var endpointJSON, runtimeJSON []byte
