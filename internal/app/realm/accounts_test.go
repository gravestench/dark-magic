...
// TestAccountsCreateAuthenticateAuthorizeAndLogout verifies accounts create authenticate authorize and logout. The
// scenario keeps the accounts contract visible to maintainers.
func TestAccountsCreateAuthenticateAuthorizeAndLogout(t *testing.T) {
	...
	attempts := []struct{ name, password string }{
		{"Alice_1", "wrong password"},
	...
	if session.ID == "" || session.Token == "" || session.Token == session.ID || session.Account.ID != account.ID ||
		!session.ExpiresAt.Equal(now.Add(time.Hour)) {
...
// TestAccountsExpireSessionsAndValidateInputs verifies accounts expire sessions and validate inputs. The scenario
// keeps the accounts contract visible to maintainers.
...
// TestAccountsAllowMulingButClaimEachCharacterOnce verifies accounts allow muling but claim each character once. The
// scenario keeps the accounts contract visible to maintainers.
...
// TestAccountsPruneExpiredReturnsFormerPrincipal verifies accounts prune expired returns former principal. The
// scenario keeps the accounts contract visible to maintainers.
...
// authenticatedFixture builds the shared authenticated fixture fixture so each test starts from the same explicit
// ownership and compatibility state.
