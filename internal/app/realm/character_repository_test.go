...
// TestMemoryCharactersEnforcesOwnerExclusiveRevisionedLease verifies memory characters enforces owner exclusive
// revisioned lease. The scenario keeps the character repository contract visible to maintainers.
...
// TestMemoryCharactersExpiresAndRejectsStaleLease verifies memory characters expires and rejects stale lease. The
// scenario keeps the character repository contract visible to maintainers.
func TestMemoryCharactersExpiresAndRejectsStaleLease(t *testing.T) {
	repository, err := NewMemoryCharacters(
		CharacterRecord{AccountID: "account", Revision: 1, Character: d2save.Character{ID: "character"}},
	)
	if err != nil {
		t.Fatal(err)
	}
...
// TestMemoryCharactersCommitsOnlyThroughActiveLease verifies memory characters commits only through active lease. The
// scenario keeps the character repository contract visible to maintainers.
func TestMemoryCharactersCommitsOnlyThroughActiveLease(t *testing.T) {
	...
	if _, err := repository.Commit(
		context.Background(),
		lease,
		d2save.Character{ID: "offline-character"},
	); !errors.Is(
		err,
		ErrCharacterCommit,
	) {
...
// TestMemoryCharactersListsOnlyOwnedDefensiveRecords verifies memory characters lists only owned defensive records.
// The scenario keeps the character repository contract visible to maintainers.
func TestMemoryCharactersListsOnlyOwnedDefensiveRecords(t *testing.T) {
	repository, err := NewMemoryCharacters(
		CharacterRecord{
			AccountID: "account:a",
			Revision:  1,
			Character: d2save.Character{ID: "character:b", Name: "B", Stats: &d2save.Stats{Health: 20}},
		},
...
// TestMemoryCharactersDeletesOnlyOwnedIdleCharacter verifies memory characters deletes only owned idle character. The
// scenario keeps the character repository contract visible to maintainers.
...
// TestMemoryCharactersReleaseGameFailsClosedWithoutCommitting verifies memory characters release game fails closed
// without committing. The scenario keeps the character repository contract visible to maintainers.
