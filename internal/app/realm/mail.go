...
func NewDevelopmentMailSender(mode string, lifecycle AccountLifecycle, logger *slog.Logger) (*DevelopmentMailSender, error) {
...
func (sender *SMTPMailer) Send(ctx context.Context, job MailJob) error {
	...
	if supported, _ := client.Extension("STARTTLS"); supported {
		if err := client.StartTLS(&tls.Config{MinVersion: tls.VersionTLS12, ServerName: sender.config.ServerName}); err != nil {
...
func renderAccountMail(job MailJob) (string, string, error) {
	...
	case "verify_email":
		...
		return "Verify your Dark Magic Realm account",
			fmt.Sprintf("Hello %s,\n\nVerify your Realm account:\n%s\n\nIf you did not request this, ignore this message.", account, link), nil
	case "reset_password":
