...
func (allocator *ProcessAllocator) Restore(ctx context.Context, spec GameSpec, recovery GameRecovery) (WorkerAllocation, error) {
...
func (allocator *ProcessAllocator) allocate(ctx context.Context, spec GameSpec, recovery *GameRecovery) (WorkerAllocation, error) {
...
func (allocator *ProcessAllocator) start(ctx context.Context, spec GameSpec, recovery *GameRecovery) (*processWorker, error) {
	...
	if err != nil || digest != description.IdentityHash || description.GameID != gameID ||
		(strings.TrimSpace(allocator.config.ExpectedAssetSetID) != "" && description.Runtime.Recipe.AssetSetID != allocator.config.ExpectedAssetSetID) {
		_ = allocator.stop(context.Background(), worker)
...
func (allocator *ProcessAllocator) waitReady(ctx context.Context, worker *processWorker, readyPath string) (WorkerProcessReady, error) {
	...
	for {
		...
		case <-worker.done:
			return WorkerProcessReady{}, fmt.Errorf("%w: worker exited before readiness: %v; logs: %s", ErrWorker, worker.result(), worker.logs.String())
		case <-ticker.C:
