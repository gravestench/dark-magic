...
// independent of external services.
func (worker *workerHTTPFixture) ProjectCharacter(
	_ context.Context,
	_ string,
...
// services.
func (tickets *ticketHTTPFixture) Issue(
	_ context.Context,
	principal AdmissionPrincipal,
	_ time.Duration,
) (string, error) {
...
// TestWorkerHTTPClientCompletesPrivateControlFlow verifies worker httpclient completes private control flow. The
// scenario keeps the worker http contract visible to maintainers.
func TestWorkerHTTPClientCompletesPrivateControlFlow(t *testing.T) {
	...
	worker := &workerHTTPFixture{
		description: WorkerDescription{Runtime: identity, IdentityHash: digest},
	...
	admission := WorkerAdmission{
		Character: d2save.Character{ID: "character", Name: "Hero"},
		PlayerID:  "player",
		Actor:     "realm:entry:player",
		Sequence:  1,
	}
...
// TestWorkerMembershipsReportReconnectExpiryUntilRealmRemoval verifies worker memberships report reconnect expiry
// until realm removal. The scenario keeps the worker http contract visible to maintainers.
...
// TestWorkerHTTPRejectsAuthenticationUnknownFieldsAndVersion verifies worker httprejects authentication unknown fields
// and version. The scenario keeps the worker http contract visible to maintainers.
...
// TestWorkerHTTPBoundsTicketLifetime verifies worker httpbounds ticket lifetime. The scenario keeps the worker http
...
func TestWorkerHTTPBoundsTicketLifetime(t *testing.T) {
	...
	if _, err := client.Issue(
		t.Context(),
		AdmissionPrincipal{AccountID: "a"},
		maximumWorkerTicketLifetime+time.Millisecond,
	); !errors.Is(
		err,
		ErrWorkerProtocol,
	) {
...
// TestWorkerHTTPPreservesRuntimeCompatibilityFailure verifies worker httppreserves runtime compatibility failure. The
// scenario keeps the worker http contract visible to maintainers.
...
// TestWorkerHTTPRejectsOversizedCheckpointResponse verifies worker httprejects oversized checkpoint response. The
// scenario keeps the worker http contract visible to maintainers.
...
// workerHTTPIdentity supplies the test double's worker httpidentity behavior, keeping the scenario deterministic and
// independent of external services.
