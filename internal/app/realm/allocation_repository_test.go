...
// TestMemoryAllocationsPreserveLifecycleAndDefensiveIdentity verifies memory allocations preserve lifecycle and
// defensive identity. The scenario keeps the allocation repository contract visible to maintainers.
func TestMemoryAllocationsPreserveLifecycleAndDefensiveIdentity(t *testing.T) {
	...
	if repeated, err := store.Request(
		t.Context(),
		"game",
		"allocation",
	); err != nil ||
		repeated.AllocationID != requested.AllocationID {
