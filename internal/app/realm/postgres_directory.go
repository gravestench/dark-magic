...
func (directory *PostgresGameDirectory) Create(ctx context.Context, principal AuthenticatedPrincipal, request CreateGameRequest) (GameDetail, error) {
	...
	_, err = directory.pool.Exec(ctx, `INSERT INTO realm_games
		(id, state, specification, revision, normalized_name, owner_account_id, created_by, password_hash, created_at, updated_at)
...
func (directory *PostgresGameDirectory) ReservePlayer(ctx context.Context, gameID string, player GamePlayer) (GamePlayerReservation, error) {
	if directory == nil || directory.pool == nil {
...
func (directory *PostgresGameDirectory) CommitPlayer(ctx context.Context, reservation GamePlayerReservation) (GameDetail, error) {
...
func (directory *PostgresGameDirectory) CancelPlayer(ctx context.Context, reservation GamePlayerReservation) error {
	...
	digest := sha256.Sum256([]byte(reservation.Token))
	command, err := directory.pool.Exec(ctx, `DELETE FROM realm_game_reservations WHERE game_id = $1 AND token_digest = $2`,
		strings.TrimSpace(reservation.GameID), digest[:])
	if err != nil {
...
func (directory *PostgresGameDirectory) SetPlayers(ctx context.Context, gameID string, players []GamePlayer) error {
	...
	return pgx.BeginFunc(ctx, directory.pool, func(tx pgx.Tx) error {
		...
		_, err := tx.Exec(ctx, `UPDATE realm_games SET revision = revision + 1, updated_at = CURRENT_TIMESTAMP WHERE id = $1`, gameID)
		return err
	})
}
...
func (directory *PostgresGameDirectory) BeginDrain(ctx context.Context, gameID string) error {
	...
	return pgx.BeginFunc(ctx, directory.pool, func(tx pgx.Tx) error {
		var state string
		if err := tx.QueryRow(ctx, `SELECT state FROM realm_games WHERE id = $1 FOR UPDATE`, strings.TrimSpace(gameID)).Scan(&state); err != nil {
			if errors.Is(err, pgx.ErrNoRows) {
				return ErrGameNotFound
			}
			return err
...
func (directory *PostgresGameDirectory) RemovePlayer(ctx context.Context, gameID, characterID string) (GameDetail, error) {
	...
	err := pgx.BeginFunc(ctx, directory.pool, func(tx pgx.Tx) error {
		...
		if command.RowsAffected() != 1 {
			var exists bool
			if queryErr := tx.QueryRow(ctx, `SELECT EXISTS (SELECT 1 FROM realm_games WHERE id = $1 AND state <> 'completed')`, gameID).Scan(&exists); queryErr != nil {
				return queryErr
			}
			if !exists {
				return ErrGameNotFound
...
func (directory *PostgresGameDirectory) resolve(ctx context.Context, reference string, idOnly bool) (postgresGame, error) {
...
func (directory *PostgresGameDirectory) resolveQuery(ctx context.Context, reference string, idOnly bool, baseQuery string) (postgresGame, error) {
	if err := contextErr(ctx); err != nil {
...
func (directory *PostgresGameDirectory) players(ctx context.Context, gameID string) ([]GamePlayer, error) {
	rows, err := directory.pool.Query(ctx, `SELECT player FROM realm_game_players WHERE game_id = $1 ORDER BY joined_at, character_id`, gameID)
	if err != nil {
...
func gameEntry(gameID string, revision uint64, request CreateGameRequest, createdBy string, passwordRequired bool, players int, createdAt time.Time) GameDirectoryEntry {
