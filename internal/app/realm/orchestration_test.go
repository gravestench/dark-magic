...
// newOrchestrationAllocator builds the shared new orchestration allocator fixture so each test starts from the same
...
// Allocate supplies the test double's allocate behavior, keeping the scenario deterministic and independent of
// external services.
func (allocator *orchestrationAllocator) Allocate(_ context.Context, spec GameSpec) (WorkerAllocation, error) {
	...
	worker := &orchestrationWorker{
		description: WorkerDescription{GameID: spec.GameID, Runtime: identity, IdentityHash: digest,
			EntryDestination: destination},
		admitErr: allocator.admitErr,
	}
...
// Game supplies the test double's game behavior, keeping the scenario deterministic and independent of external
// services.
...
// Release supplies the test double's release behavior, keeping the scenario deterministic and independent of external
// services.
...
// independent of external services.
func (*orchestrationWorker) ProjectCharacter(
	_ context.Context,
	_ string,
...
// orchestratedControlConfig supplies the test double's orchestrated control config behavior, keeping the scenario
...
// orchestrationIdentity supplies the test double's orchestration identity behavior, keeping the scenario deterministic
...
func orchestrationIdentity() simulation.RuntimeIdentity {
	return simulation.RuntimeIdentity{Recipe: simulation.RuntimeRecipe{
		...
		GameDataGenerationID: simulation.GameDataGenerationIDForAssetSet(
			simulation.EmptyAssetSetID,
		),
...
// testCheckpoint supplies the test double's test checkpoint behavior, keeping the scenario deterministic and
// independent of external services.
