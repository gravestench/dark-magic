...
// OpenPostgres executes open postgres through the PostgreSQL postgres store so SQL encoding and database error
...
// Close executes close through the PostgreSQL postgres store so SQL encoding and database error translation remain
// centralized.
...
// initializeRealmPostgres executes initialize realm postgres through the PostgreSQL postgres store so SQL encoding and
...
func initializeRealmPostgres(ctx context.Context, pool *pgxpool.Pool) error {
	...
	if _, err := tx.Exec(ctx, realmSchema); err != nil {
		return fmt.Errorf(
			"realm: initialize PostgreSQL schema (run make realm-fresh-install after incompatible pre-production changes): %w",
			err,
		)
	}
...
// Record emits the canonical durable postgres representation so persisted and transported values retain one stable
// shape.
...
// Create executes create through the PostgreSQL postgres store so SQL encoding and database error translation remain
// centralized.
...
// Authenticate executes authenticate through the PostgreSQL postgres store so SQL encoding and database error
// translation remain centralized.
...
// Authorize executes authorize through the PostgreSQL postgres store so SQL encoding and database error translation
// remain centralized.
...
// SelectCharacter executes select character through the PostgreSQL postgres store so SQL encoding and database error
// translation remain centralized.
...
// SelectedCharacter executes selected character through the PostgreSQL postgres store so SQL encoding and database
...
// Logout executes logout through the PostgreSQL postgres store so SQL encoding and database error translation remain
// centralized.
...
// PruneExpired executes prune expired through the PostgreSQL postgres store so SQL encoding and database error
// translation remain centralized.
...
// Delete executes delete through the PostgreSQL postgres store so SQL encoding and database error translation remain
// centralized.
func (repository *PostgresCharacters) Delete(ctx context.Context, accountID, characterID string) error {
	...
	var owner string
	if err := tx.QueryRow(ctx, `SELECT account_id FROM realm_characters WHERE id = $1 FOR UPDATE`, characterID).
		Scan(&owner); errors.Is(
		err,
		pgx.ErrNoRows,
	) {
		return ErrCharacterNotFound
...
// Get executes get through the PostgreSQL postgres store so SQL encoding and database error translation remain
// centralized.
func (repository *PostgresCharacters) Get(ctx context.Context, accountID, characterID string) (CharacterRecord, error) {
	record, err := scanPostgresCharacter(
		repository.pool.QueryRow(ctx, `SELECT account_id, revision, character, compatibility
...
// List executes list through the PostgreSQL postgres store so SQL encoding and database error translation remain
// centralized.
...
// Acquire executes acquire through the PostgreSQL postgres store so SQL encoding and database error translation remain
// centralized.
func (repository *PostgresCharacters) Acquire(
	ctx context.Context,
...
// BindCompatibility emits the canonical durable postgres representation so persisted and transported values retain one
// stable shape.
func (repository *PostgresCharacters) BindCompatibility(
	ctx context.Context,
	...
	if _, err := tx.Exec(
		ctx,
		`UPDATE realm_characters SET compatibility = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2`,
		encoded,
		lease.CharacterID,
	); err != nil {
		return CharacterRecord{}, err
...
// Renew executes renew through the PostgreSQL postgres store so SQL encoding and database error translation remain
// centralized.
func (repository *PostgresCharacters) Renew(
	ctx context.Context,
	lease CharacterLease,
	lifetime time.Duration,
...
// Release executes release through the PostgreSQL postgres store so SQL encoding and database error translation remain
// centralized.
...
// ReleaseGame executes release game through the PostgreSQL postgres store so SQL encoding and database error
// translation remain centralized.
...
// Commit emits the canonical durable postgres representation so persisted and transported values retain one stable
// shape.
func (repository *PostgresCharacters) Commit(
	ctx context.Context,
	lease CharacterLease,
	...
	if _, err := tx.Exec(
		ctx,
		`DELETE FROM realm_character_leases WHERE character_id = $1`,
		lease.CharacterID,
	); err != nil {
		return CharacterRecord{}, err
	}
...
// lockLeasedCharacter executes lock leased character through the PostgreSQL postgres store so SQL encoding and
// database error translation remain centralized.
func (repository *PostgresCharacters) lockLeasedCharacter(
	ctx context.Context,
	tx pgx.Tx,
	lease CharacterLease,
) (CharacterRecord, error) {
...
// scanPostgresCharacter decodes the durable postgres representation at one boundary so malformed data fails before it
// becomes shared state.
func scanPostgresCharacter(row postgresRow) (CharacterRecord, error) {
	var (
		record                   CharacterRecord
		character, compatibility []byte
	)
...
// encodePostgresCharacter emits the canonical durable postgres representation so persisted and transported values
...
// postgresUniqueViolation executes postgres unique violation through the PostgreSQL postgres store so SQL encoding and
// database error translation remain centralized.
