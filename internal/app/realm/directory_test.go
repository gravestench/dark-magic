...
func TestGameDirectorySeparatesNamesFromOpaqueIDs(t *testing.T) {
	...
	if _, err := directory.Create(t.Context(), owner, CreateGameRequest{Name: "trist run", Difficulty: DifficultyNormal, Maximum: 8}); !errors.Is(err, ErrGameExists) {
...
func TestGameDirectoryProjectsPlayersCapacityAndFilters(t *testing.T) {
	...
	players := []GamePlayer{{CharacterID: "one", Name: "Alice", Class: "Assassin", Level: 80}, {CharacterID: "two", Name: "Bob", Class: "Barbarian", Level: 79}}
	...
	if err := directory.SetPlayers(t.Context(), game.Entry.GameID, []GamePlayer{{CharacterID: "same", Name: "A", Class: "Amazon", Level: 1}, {CharacterID: "same", Name: "B", Class: "Amazon", Level: 1}}); !errors.Is(err, ErrGameDirectoryInput) {
...
func TestGameDirectoryReservationsArePrivateAtomicAndCancelable(t *testing.T) {
	...
	owner := authenticatedFixture(t, accounts, "Owner")
	directory := NewGameDirectory()
	game, err := directory.Create(t.Context(), owner, CreateGameRequest{Name: "Two Seats", Difficulty: DifficultyNormal, Maximum: 2})
	if err != nil {
		t.Fatal(err)
	}
	first, err := directory.ReservePlayer(t.Context(), game.Entry.GameID, GamePlayer{CharacterID: "one", Name: "One", Class: "Amazon", Level: 1})
	if err != nil {
		t.Fatal(err)
	}
	second, err := directory.ReservePlayer(t.Context(), game.Entry.GameID, GamePlayer{CharacterID: "two", Name: "Two", Class: "Barbarian", Level: 1})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := directory.ReservePlayer(t.Context(), game.Entry.GameID, GamePlayer{CharacterID: "three", Name: "Three", Class: "Amazon", Level: 1}); !errors.Is(err, ErrGameFull) {
	...
	if _, err := directory.ReservePlayer(t.Context(), game.Entry.GameID, GamePlayer{CharacterID: "three", Name: "Three", Class: "Amazon", Level: 1}); err != nil {
...
func TestRealmServicesRejectCancellationAndInvalidPrincipals(t *testing.T) {
	...
	if control.Version() != RealmControlPlaneVersion || control.accounts == nil || control.channels == nil || control.games == nil || control.characters == nil {
		t.Fatalf("control plane = %#v", control)
	}
	if _, err := control.games.Create(t.Context(), AuthenticatedPrincipal{}, CreateGameRequest{Name: "Game", Difficulty: DifficultyNormal, Maximum: 8}); !errors.Is(err, ErrGameDirectoryInput) {
