...
func TestComponentDiscoveryKeepsEachResolvedPackageRoot(t *testing.T) {
	...
	contentFS, err := content.New(content.Layer{Name: "first", FS: first}, content.Layer{Name: "second", FS: second})
	if err != nil {
		t.Fatal(err)
	}
...
func discoveryManifest(id string) modcache.Manifest {
	return modcache.Manifest{Schema: modcache.ManifestSchema, ID: id, Name: id, Version: "1.0.0", Kind: "game",
		EngineAPI: modcache.EngineAPI, Redistributable: true, Entrypoints: modcache.Entrypoints{ClientComponents: []string{id + ".boot"}}}
}
