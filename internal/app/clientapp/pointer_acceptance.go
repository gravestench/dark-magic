...
func newPointerMovementAcceptance(world *gameworld.Map, spawnX, spawnY float64, screenWidth, screenHeight int) *pointerMovementAcceptance {
...
func acceptanceMovementTarget(world *gameworld.Map, spawnX, spawnY float64) (float64, float64) {
	for radius := 18; radius >= 6; radius -= 3 {
		...
		for _, offset := range [][2]float64{{distance, 0}, {0, distance}, {-distance, 0}, {0, -distance}, {distance, distance}, {-distance, distance}} {
			x, y, found := world.OpenPointNearSubtile(spawnX+offset[0], spawnY+offset[1])
			if !found {
				continue
			}
			path, err := world.FindPath(gameworld.PathRequest{Start: gameworld.Point{X: spawnX, Y: spawnY}, Goal: gameworld.Point{X: x, Y: y}, Radius: 1.5})
...
func (fixture *pointerMovementAcceptance) Frame(frame inputstate.Frame, x, y float64, playerPresent bool) inputstate.Frame {
...
func (app *application) controlledPlayerPosition() (float64, float64, bool) {
	...
	controls, found := akara.GetDynamicStore(app.entitySimulation.World(), "d2.world.player_control")
	if !found {
		return 0, 0, false
	}
	positions, found := akara.GetDynamicStore(app.entitySimulation.World(), "d2.world.position")
	if !found {
		return 0, 0, false
	}
