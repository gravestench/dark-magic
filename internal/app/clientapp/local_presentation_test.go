...
// TestLocalPresentationMovesImmediatelyAndSmoothsOnlyCorrectionError keeps
// prediction responsive while hiding small drift.
func TestLocalPresentationMovesImmediatelyAndSmoothsOnlyCorrectionError(t *testing.T) {
	var state localPresentation
	if got := state.Project(
		"player",
		playeradapter.HUDPosition{X: 10},
	...
	if got := state.Project(
		"player",
		playeradapter.HUDPosition{X: 11},
		playeradapter.HUDPosition{},
		false,
		16*time.Millisecond,
	); got.X != 11 {
	...
	if got := state.Project(
		"player",
		playeradapter.HUDPosition{X: 10.5},
		playeradapter.HUDPosition{X: .5},
		true,
		16*time.Millisecond,
	); got.X != 11 {
...
// TestLocalPresentationSnapsLargeCorrectionAndResetsForNewOwner prevents
// smoothing across teleports or identity changes.
func TestLocalPresentationSnapsLargeCorrectionAndResetsForNewOwner(t *testing.T) {
	...
	if got := state.Project(
		"one",
		playeradapter.HUDPosition{X: 2},
		playeradapter.HUDPosition{X: 8},
		true,
		0,
	); got.X != 2 {
	...
	if got := state.Project(
		"two",
		playeradapter.HUDPosition{X: 40},
		playeradapter.HUDPosition{X: 2},
		true,
		0,
	); got.X != 40 {
...
// TestMergeInputHistoryRetainsAcknowledgedAndNewCommandsForCorrectionComparison preserves replay evidence by sequence.
