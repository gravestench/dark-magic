...
// Invalidate is inert because fixture rows are immutable for the lifetime of a test.
func (fixtureRecords) Invalidate(string) {}

// Loaded reports every fixture table ready so runtime startup never falls back to external assets.
func (fixtureRecords) Loaded(string) bool { return true }

// Load returns the minimum record rows required to start and reconcile the authority fixture.
func (fixtureRecords) Load(path string) ([]map[string]string, error) {
	switch path {
	case "data/global/excel/charstats.txt":
		return fixtureCharacterStats(), nil
	...
	case "data/global/excel/skills.txt":
		return fixtureSkills(), nil
	case "data/global/excel/Missiles.txt":
		return fixtureMissiles(), nil
	default:
		return nil, nil
	}
}

// fixtureCharacterStats supplies movement and stamina inputs whose values become part of runtime identity.
func fixtureCharacterStats() []map[string]string {
	return []map[string]string{{
		"class": "Amazon", "StartSkill": "Fire Bolt", "WalkVelocity": "6", "RunVelocity": "9",
		"vit": "20", "stamina": "84", "RunDrain": "20",
		"StaminaPerLevel": "4", "StaminaPerVitality": "4",
	}}
}

// fixtureSkills supplies one command-capable skill without introducing unrelated gameplay records.
func fixtureSkills() []map[string]string {
	return []map[string]string{{
		"Id": "36", "skill": "Fire Bolt", "skilldesc": "firebolt", "leftskill": "1",
		"general": "0", "passive": "0", "srvmissile": "firebolt", "etype": "fire", "interrupt": "1",
		"srvstfunc": "", "srvdofunc": "", "mana": "5", "manashift": "7", "emin": "3", "emax": "6",
		"HitShift": "8",
	}}
}

// fixtureMissiles provides the visual and collision facts referenced by the fixture skill.
func fixtureMissiles() []map[string]string {
	return []map[string]string{{
...
// TestHeadlessHostPinsRunningAuthorityToAdmissionAndReconnect keeps allocation, replay, and
// correction identity aligned.
func TestHeadlessHostPinsRunningAuthorityToAdmissionAndReconnect(t *testing.T) {
	...
	assertHostIdentityPinned(t, host)
	token := assertHostAdmissionAndCorrection(t, host)
	assertHostRejectsIdentityDrift(t, host, token)
}

// assertHostIdentityPinned proves allocation and replay use the runtime identity registered by the live authority.
func assertHostIdentityPinned(t *testing.T, host *Host) {
	t.Helper()
...
// assertHostAdmissionAndCorrection exercises compatible admission and restores a divergent predicted ECS.
func assertHostAdmissionAndCorrection(t *testing.T, host *Host) gamesession.AdmissionToken {
	t.Helper()
	...
	t.Cleanup(func() { _ = predictedEngine.Close() })
...
// assertHostRejectsIdentityDrift covers changed runtime packages and stale session tokens.
func assertHostRejectsIdentityDrift(t *testing.T, host *Host, token gamesession.AdmissionToken) {
	t.Helper()
...
// TestHeadlessHostRejectsUnknownPredictionContract prevents unimplemented client-authority contracts at startup.
...
// TestSharedHostRunsStandaloneListenAndRealmModes proves hosting mode does not alter authority composition or identity.
