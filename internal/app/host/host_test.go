...
func TestHostStartsDependenciesAndStopsInReverse(t *testing.T) {
	...
	var calls []string
	h := New()
	register := func(id string, dependencies ...string) {
		t.Helper()
		err := h.Register(Definition{
			ID:        id,
			DependsOn: dependencies,
			Component: componentFunc{
				start: func(context.Context) error { calls = append(calls, "start "+id); return nil },
	...
	register("game", "scripts", "renderer")
	register("renderer", "assets")
...
func TestHostRollsBackFailedStartAndCanRetry(t *testing.T) {
	...
	var calls []string
	h := New()
	if err := h.Register(Definition{ID: "one", Component: componentFunc{
		start: func(context.Context) error { calls = append(calls, "start one"); return nil },
		stop:  func(context.Context) error { calls = append(calls, "stop one"); return nil },
	}}); err != nil {
		t.Fatal(err)
	}
	if err := h.Register(Definition{ID: "two", DependsOn: []string{"one"}, Component: componentFunc{
		start: func(context.Context) error {
		...
		stop: func(context.Context) error { calls = append(calls, "stop two"); return nil },
	}}); err != nil {
		t.Fatal(err)
	}
...
func TestHostValidatesDependencyGraph(t *testing.T) {
	...
	t.Run("missing", func(t *testing.T) {
		h := New()
		if err := h.Register(Definition{ID: "game", DependsOn: []string{"renderer"}, Component: componentFunc{}}); err != nil {
			t.Fatal(err)
		}
...
func TestHostHonorsCancellationAndJoinsStopErrors(t *testing.T) {
	...
	h := New()
	for _, def := range []Definition{
		{ID: "one", Component: componentFunc{stop: func(context.Context) error { return firstErr }}},
		{ID: "two", DependsOn: []string{"one"}, Component: componentFunc{stop: func(context.Context) error { return secondErr }}},
	} {
		if err := h.Register(def); err != nil {
			t.Fatal(err)
		}
