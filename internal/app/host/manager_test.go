...
func TestManagerEnablesDependenciesAndRejectsUnsafeDisable(t *testing.T) {
	...
	if err := m.Disable(context.Background(), "assets"); err == nil || !strings.Contains(err.Error(), "active dependents") {
...
func TestManagerRestartAndEvents(t *testing.T) {
	...
	wantStates := []State{StateEnabling, StateEnabled, StateDisabling, StateDisabled, StateEnabling, StateEnabled}
...
func TestManagerRejectsCyclesAndMissingDependencies(t *testing.T) {
	...
	if err := m.Register(ManagedDefinition{ID: "a", DependsOn: []string{"b"}, New: noop}); err != nil {
		t.Fatal(err)
	}
	if err := m.Enable(context.Background(), "a"); err == nil || !strings.Contains(err.Error(), "not registered") {
		t.Fatalf("Enable error = %v", err)
	}
	if err := m.Register(ManagedDefinition{ID: "b", DependsOn: []string{"a"}, New: noop}); err == nil || !strings.Contains(err.Error(), "cycle") {
...
func TestManagerTransactionallyReplacesEnabledComponent(t *testing.T) {
	...
	if err := m.Register(ManagedDefinition{ID: "scripts", New: func(context.Context) (Component, error) { return old, nil }}); err != nil {
		t.Fatal(err)
	}
	if err := m.Enable(context.Background(), "scripts"); err != nil {
		t.Fatal(err)
	...
	if err := m.Replace(context.Background(), ManagedDefinition{ID: "scripts", New: func(context.Context) (Component, error) { return newComponent, nil }}); err != nil {
		t.Fatal(err)
	}
...
func TestManagerKeepsExistingInstanceWhenReplacementFails(t *testing.T) {
	...
	if err := m.Replace(context.Background(), ManagedDefinition{ID: "scripts", New: func(context.Context) (Component, error) { return broken, nil }}); err == nil {
