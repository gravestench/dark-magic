...
the level-scaled radius into relationship entities that own keyed ordinary
...
inactive-aura rule.
Resist Fire is the fourth exact selected aura and the first whose active
relationship owns multiple stat sources. The owned row and official Expansion
level table define diminishing fire resistance, +1 maximum fire resistance per
hard point while active, and only half that maximum-resistance bonus rounded
down on the Paladin while inactive; soft skill bonuses do not increase either
hard-point contribution. Aura relationships now reconcile a keyed set of
ordinary stat-source entities, allowing one state/lifecycle to own both active
values without adding parallel aura components. The learned-passive system
...
similarity.
Resist Lightning is the sixth exact selected aura and completes the three
single-element defensive-aura consumers. It independently admits skill ID 110,
states 5/183, lightning-specific localized text, sound, Overlay, and DCC facts;
matching Fire/Cold formulas are implementation-reuse evidence rather than
admission evidence. Canonical/effective lightning resistance and maximum
lightning resistance now flow from player entry through ordinary stat sources
into the shared channel-indexed elemental mitigation boundary. Level-3
checkpoint coverage pins 76 active lightning resistance, +3 active maximum,
+1 inactive maximum, and 1000 raw lightning damage reduced to 240.
Salvation is the seventh exact selected aura and the first admitted row whose
single relationship owns three same-progression stats. Its independently pinned
ID 125 record, `resistall` state, localized text, sound, Overlay, and DCC facts
declare fire, cold, and lightning resistance—never poison—with no passive or
maximum-resistance effect. The generic decoder now accepts a state whose
representative stat matches any authored aura stat rather than incorrectly
...
- [x] Add the first selected-right party-aura family and admit exact Expansion
  ...
  self/party target relationships with keyed ordinary stat sources.
...
- [x] Admit both record-declared halves of exact Expansion 1.14d Blessed Aim.
  ...
  as hard points.
- [x] Reuse the selected-aura and learned-passive families for exact Expansion
  1.14d Resist Fire. Generalize an aura relationship from one co-composed
  modifier to a stable keyed set of ordinary stat sources; decode active
  `fireresist=dm34`, active `maxfireresist=self.blvl`, and inactive
  `maxfireresist=floor(self.blvl/2)` without an ID/name/element branch. Pin the
  staged integer `dm34` level vector, soft-level exclusion, 95% mitigation cap,
...
- [x] Admit exact Expansion 1.14d Resist Lightning through the same generic
  consumer boundary. Add canonical/effective lightning and max-lightning fields,
  preserve the character-entry fact, resolve ordinary keyed stat sources, and
  select them through the channel-indexed mitigation policy. Reuse the reviewed
  `dm34`, self-hard-level, and rounded-half passive recipes while independently
  pinning ID 110, states 5/183, localized TBL intent, sound, Overlay, and DCC
  evidence. Level-3 checkpoint coverage proves 76/+3/+1 and 240/1000 lightning
  mitigation without deriving admission from either neighboring aura row.
- [x] Admit exact Expansion 1.14d Salvation as a three-stat configuration of the
  same selected-aura family. Generalize state validation so a state may name any
  one of its row's authored stats, then decode three `dm34` recipes into keyed
  fire/cold/lightning resistance sources without a multi-element system branch.
  Pin the independent 60..108 official vector, no-passive boundary, ID 125,
...
unassignable. It currently grants the 18 manifest-backed Expansion
...
Resist Fire is the fourth exact `aura.selected-party-stat` configuration. Exact
skill ID 100 retains the right-only, immediate, zero-mana, filter-73731,
level-scaled-radius, and 50-tick aura contract, but its active state has two
authored contributions: `fireresist=dm34` and
`maxfireresist=skill('Resist Fire'.blvl)`. The first uses Params 3/4 (35/150)
with staged integer arithmetic: truncate `110*level/(level+6)`, then interpolate
between the parameters. This reproduces the official level-1..20 vector
52,66,76,85,92,98,102,106,110,113,116,118,121,123,124,127,128,129,130,131.
The second adds one maximum-fire-resistance point per hard point.

Its inactive `passive_resistfire` state uses
`maxfireresist=skill('Resist Fire'.blvl)/2`. The [official Expansion defensive-
aura reference](https://classic.battle.net/diablo2exp/skills/paladin-defense.shtml)
confirms floor rounding, no passive ordinary fire resistance, the 95% maximum,
and exclusion of item-granted soft levels. The generic learned-passive recipe
therefore stores an integer numerator/divisor instead of assuming every passive
is a per-level multiple. At level three it produces +1 maximum fire resistance
while inactive; selection removes that source and the active aura supplies 76
fire resistance plus +3 maximum resistance. Shared fire mitigation already
caps the effective maximum at 95%.

Because a single aura state can now affect multiple stats, the relationship no
longer relies on one co-composed modifier. Its stable source key owns a
deterministically reconciled set of ordinary stat-source entities, keyed by
target, aura source, and stat. Losing range, party, level, life, room activity,
or selection removes stale keyed sources in the same pass; duplicate sources
are collapsed deterministically. Presentation and connected projection still
observe only the aura relationship's target/state/period and never receive the
stat set. Owned tests pin the active/passive states, SkillDesc/TBL intent, sound
key, persistent/cast Overlay rows, and DCC members. Hireling/summon inclusion,
...
mitigation policy uses a channel-to-stat table for fire, cold, and lightning,
clamps current resistance to −100..effective maximum and maximum to 0..95, and retains the
...
Resist Lightning is the sixth exact `aura.selected-party-stat` configuration.
Exact skill ID 110 independently pins `lightresist=dm34`,
`maxlightresist=skill('Resist Lightning'.blvl)`, and inactive
`maxlightresist=skill('Resist Lightning'.blvl)/2`, plus active/passive states
5/183, the localized lightning-protection description, aura sound, and
persistent/cast Overlay/DCC members. Its Params 3/4 are 35/150, producing the
same official 52..131 active vector; Blizzard's Expansion reference separately
pins +1 active maximum resistance per hard point, floor-half while inactive,
the 95% cap, and exclusion of item-granted soft levels. The decoder and systems
do not recognize ID 110, its name, or the lightning element as a skill identity.

The shared defense component, player-entry adapter, and derived-stat pass now
carry `lightning_resist` and `max_lightning_resist`. The mitigation policy maps
the lightning channel to those fields through the same data table as fire and
cold, retaining the −100/current-maximum/95% clamps and integer percentage
stage. At level three, active resistance 76 and maximum 78 reduce 1000 raw
lightning damage to 240; switching away restores only the +1 inactive maximum
source. Lightning absorb, PvP conversion, item-source ordering, soft-level
integration, and exact maximum-resistance stacking order remain explicit
Expansion 1.14d evidence work.

Salvation is the seventh exact `aura.selected-party-stat` configuration. Its
owned ID 125 row independently declares `fireresist=dm34`, `coldresist=dm34`,
and `lightresist=dm34` with Params 3/4 of 50/120, no passive state/stat/formula,
and the shared right-only, zero-mana selected-aura lifecycle. State 8 names
`lightresist` even though fire is the row's first stat, so generic state
validation now requires the representative state stat to match any authored
stat. It does not weaken validation by accepting an empty or unrelated state.

The staged progression produces Blizzard's Expansion level-1..20 vector
60,68,75,80,85,88,91,93,96,97,99,101,102,103,104,106,106,107,108,108. Owned
SkillDesc and locale TBL text say “Resist All” and explicitly enumerate fire,
cold, and lightning, excluding poison by both record and player-visible intent.
At level three, one `resistall` relationship owns three keyed ordinary stat
sources worth 75 each. Existing derived stats and table-driven mitigation reduce
1000 damage to 250 in every supported elemental channel; changing selection
removes all three sources in the same reconciliation pass. The connected view
still projects only target/state/period presentation facts. Owned state, sound,
three Overlay rows, and DCC members pin the purple front/back/cast presentation.
