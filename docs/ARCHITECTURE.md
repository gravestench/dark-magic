...
`d2legacy` mod requests immutable rows through `engine.records/v1`, reads typed
fixed-point animation records from the same pinned generation through
`engine.animdata/v1`, loads its package-owned target declarations through
read-only `engine.data/v1`, and owns
