...
| bundled Lua content | The first-party `d2legacy` mod, `d2.*` presentation, and labs | `d2legacy` is the canonical package; `d2.*` is its concise runtime namespace; neither is a second mod |
