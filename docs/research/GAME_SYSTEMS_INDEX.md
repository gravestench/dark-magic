...
| 11 | Skills, missiles, states, and combat actions | [SKILLS_STATES_AND_MISSILES.md](SKILLS_STATES_AND_MISSILES.md) | baseline | M21.6 immutable cast requests and M21.7 checkpointed cast lifecycle are implemented; straight/radial/area-impact/on-hit-state missile families use ordinary ECS composition, Glacial Spike composes area damage/control, and Teleport adds signed mana plus generic point relocation/result composition while exact resistance/range/invalid-target ordering remains probe-gated | timing, stats, combat, missiles/states | 22 |
...
- G9 direct-control tranche: active; exact-ID Ice Blast snapshots record/TBL-
  backed damage and freeze-length synergies, then emits a generic monster-cold
  timed-state request after nonlethal contact. Resistance/immunity,
  monster-class effectiveness, cross-source replacement, PvP chill conversion,
  and exact action timing remain probes.
- G9 area-control tranche: active; exact-ID Glacial Spike composes the shared
  stable area-impact query, cold damage, and monster-cold state request using
  record/TBL-backed radius, duration, Blizzard duration synergy, and three
  damage synergies. Secondary ejecta presentation, resistance/immunity,
  monster-class effectiveness, PvP conversion, rounding/RNG, and exact action
  timing remain probes.
- G9 point-movement tranche: active; exact-ID Teleport decodes signed mana and
  Levels policy, validates static/dynamic target occupancy, atomically moves the
...
- G9 selected-aura tranche: active; exact-ID Might, Prayer, Defiance, Blessed
  ...
  probes.
  Blessed Aim additionally decodes its self-hard-level passive into a separate
  generic ECS reconciler: the learned-skill entity owns a 5%-per-hard-point
  attack-rating source while the aura is not selected, and selecting the aura
  yields to the active party source. Soft-level and external-aura overlap
  ordering remain target-version probes.
  Resist Fire generalizes one aura relationship to a keyed set of ordinary stat
  sources: active `dm34` fire resistance and +1 max fire resistance per hard
  ...
  remain probes.
  Salvation independently admits ID 125/state 8 and lets one relationship own
