...
| 11 | Skills, missiles, states, and combat actions | [SKILLS_STATES_AND_MISSILES.md](SKILLS_STATES_AND_MISSILES.md) | baseline | M21.6 immutable cast requests and M21.7 checkpointed cast lifecycle are implemented; straight/radial/area-impact/on-hit-state missile families use ordinary ECS composition, Fire Bolt/Fire Ball snapshot modifiers, Ice Blast routes direct freeze, and Glacial Spike composes area impact with one timed-state request per surviving target while exact resistance/class/PvP ordering remains probe-gated | timing, stats, combat, missiles/states | 22 |
