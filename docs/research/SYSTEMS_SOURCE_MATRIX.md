...
| --- | --- | --- | --- | --- |
| Dark Magic repository | current `main` at each research/consolidation pass | MIT project source | implementation baseline | typed game data, 25 Hz session/replay, stat-source authority, fixed-point combat, monster materialization/AI/melee, item authority, loot, generated world, transitions, audio backend/catalog, Lua snapshot/intent boundary, server/realm composition roots, roadmap |
