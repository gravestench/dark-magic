...
func (s *Service) InitRoutes(group *gin.RouterGroup) {
	...
	group.GET("sprite/png/:palette/*path", s.handleCreatePngSpriteAtlas)
	group.GET("sprite/gif/:palette/*path", s.createAnimatedGifFromSpriteAtlas)
}
...
func (s *Service) handleCreateAnimatedGifFromDc6(c *gin.Context) {
	...
	data, err := s.LoadDc6ToPngSpriteAtlas(path, pathPL2)
	if err != nil {
		s.logger.Error().Msgf("loading file: %v", err)
		c.JSON(http.StatusBadRequest, err)
		return
	...
	cacheKey := fmt.Sprintf("gif: %s", path)
	cachedData, isCached := s.spriteCache.Retrieve(cacheKey)
	if isCached {
		gifImage = cachedData.([]byte)
	} else {
		gifImage, err = generateAnimatedGifFromPngSpriteAtlasData(data)
		if err != nil {
			s.logger.Error().Msgf("creating animated GIF from png sprite atlas: %v", err)
