...
const (
	intVal = StatValueInt
	sum    = StatValueCombineSum
	static = StatValueCombineStatic
)
...
func (s *diablo2Stat) init(numbers ...float64) { //nolint:funlen,gocyclo // can't reduce
	...
	case 1:
		...
		s.values = make([]StatValue, oneValue)
	...
	case 24:
		...
		s.values = make([]StatValue, fourValue)
...
func (s *diablo2Stat) Values() []StatValue {
	return s.values
}

// SetValues sets the stat values
func (s *diablo2Stat) SetValues(values ...StatValue) {
	s.values = make([]StatValue, len(values))
...
func (s *diablo2Stat) Clone() Stat {
	...
	for idx := range s.values {
		...
		if len(clone.values) < len(s.values) {
			clone.values = make([]StatValue, len(s.values))
		}
...
func (s *diablo2Stat) Copy(from Stat) Stat {
	srcValues := from.Values()
	s.values = make([]StatValue, len(srcValues))
...
func (s *diablo2Stat) Combine(other Stat) (result Stat, err error) {
	...
	for idx := range result.Values() {
		...
		if combinationRule == StatValueCombineStatic {
		...
		if combinationRule == StatValueCombineSum {
...
func (s *diablo2Stat) canBeCombinedWith(other Stat) bool {
	...
	for idx := range values1 {
		...
		// the same.
		if values1[idx].CombineType() == StatValueCombineStatic {
...
func (s *diablo2Stat) descFn1() string {
	...
	stringTableString := s.factory.records.TranslateString(stringTableKey)
...
func (s *diablo2Stat) descFn6() string {
	...
	str1 := s.factory.records.TranslateString(stringTableKey)
	str2 := s.factory.records.TranslateString(s.record.DescStr2)
...
func (s *diablo2Stat) descFn14() string {
	...
	heroIndex := hero.Int()
	classRecord := s.factory.records.Records.Character.Stats[heroMap[heroIndex]]
	...
	skillTabStr := s.factory.records.TranslateString(skillTabKey)
...
func (s *diablo2Stat) descFn15() string {
	...
	chanceToCastStr := s.factory.records.TranslateString(s.record.DescStrPos)
...
func (s *diablo2Stat) descFn16() string {
	...
	format := s.factory.records.TranslateString(s.record.DescStrPos)
...
func (s *diablo2Stat) descFn22() string {
	...
	arVersus := s.factory.records.TranslateString(s.record.DescStrPos)
...
func (s *diablo2Stat) descFn24() string {
	...
	chargeStr := s.factory.records.TranslateString(s.record.DescStrPos)
