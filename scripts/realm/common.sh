...
realm_load_environment() {
    ...
    while IFS= read -r realm_line || [ -n "$realm_line" ]; do
        ...
        case "$realm_key" in ''|[0-9]*|*[!A-Za-z0-9_]*) realm_fail "invalid variable name in $realm_environment_path: $realm_key" ;; esac
        eval "realm_is_exported=\${$realm_key+x}"
        [ "$realm_is_exported" = x ] && continue
        case "$realm_value" in
            \"*\") realm_value=${realm_value#\"}; realm_value=${realm_value%\"}; realm_value=$(printf '%b' "$realm_value") ;;
            \'*\') realm_value=${realm_value#\'}; realm_value=${realm_value%\'} ;;
        esac
...
realm_validate_managed_postgres() {
    ...
    for realm_identifier in "$REALM_POSTGRES_DATABASE" "$REALM_POSTGRES_USER"; do
        case "$realm_identifier" in
            ''|[0-9]*|*[!A-Za-z0-9_]*) realm_fail "managed PostgreSQL names must be simple SQL identifiers" ;;
        esac
    done
...
'*) realm_fail "managed PostgreSQL password cannot contain a newline" ;;
    ...
    case "$REALM_POSTGRES_SOCKET" in
        ''|*[!A-Za-z0-9_./-]*) realm_fail "managed PostgreSQL socket path contains unsupported characters" ;;
    esac
}
