...
case "$mailpit_name" in ''|*[!A-Za-z0-9_.-]*) printf '%s\n' 'realm mailpit: invalid container name' >&2; exit 1 ;; esac
for mailpit_port in "$mailpit_smtp_port" "$mailpit_http_port"; do
    case "$mailpit_port" in ''|*[!0-9]*) printf '%s\n' 'realm mailpit: ports must be numeric' >&2; exit 1 ;; esac
...
while [ "$mailpit_attempt" -lt 60 ]; do
    if curl --silent --fail "http://127.0.0.1:$mailpit_http_port/readyz" >/dev/null 2>&1; then
        printf '%s\n' "realm mailpit: ready (SMTP 127.0.0.1:$mailpit_smtp_port, inbox http://127.0.0.1:$mailpit_http_port)"
        exit 0
    fi
